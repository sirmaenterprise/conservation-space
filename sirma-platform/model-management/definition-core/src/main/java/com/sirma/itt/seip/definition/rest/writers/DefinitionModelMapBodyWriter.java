...
public class DefinitionModelMapBodyWriter extends AbstractMessageBodyWriter<Map<String, DefinitionModelObject>> {
	...
	public void writeTo(Map<String, DefinitionModelObject> objects, Class<?> clazz, Type type, Annotation[] annotations,
		...
		if (CollectionUtils.isEmpty(objects)) {
			throw new ResourceException(NOT_FOUND, new ErrorData().setMessage("There are no request object."), null);
		}
		...
		for (Entry<String, DefinitionModelObject> entry : objects.entrySet()) {
			...
			if (model != null) {
				...
				if (isEmpty(requestedFields)) {
					...
					serializer.serialize(model, value.getInstance(), value.getOperation(), getProjection(requestedFields, model), generator);
				}
				generator.writeEnd();
			}
		}
	...
	private static Set<String> getProjection(List<String> requestedFields, DefinitionModel model) {
		return model
