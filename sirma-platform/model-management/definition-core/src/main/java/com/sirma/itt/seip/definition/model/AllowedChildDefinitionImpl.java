 ...
 * @author BBonev
 */
public class AllowedChildDefinitionImpl extends MergeableBase<AllowedChildDefinitionImpl>
		implements Serializable, AllowedChildDefinition, BidirectionalMapping, Cloneable, Sealable {
	...
	@Override
	public AllowedChildDefinitionImpl clone() {
		try {
			AllowedChildDefinitionImpl clone = (AllowedChildDefinitionImpl) super.clone();
			clone.sealed = false;
			...
			for (AllowedChildConfiguration configuration : getPermissions()) {
				clone.getPermissions().add(((AllowedChildConfigurationImpl) configuration).clone());
			}
			clone.setFilters(new ArrayList<>(getFilters().size()));
			for (AllowedChildConfiguration configuration : getFilters()) {
				clone.getFilters().add(((AllowedChildConfigurationImpl) configuration).clone());
			}
