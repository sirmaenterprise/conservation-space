 ...
 * @author BBonev
 */
public class RegionDefinitionImpl extends BaseDefinition<RegionDefinitionImpl>implements RegionDefinition, Cloneable {
	...
	private void mergeFields(RegionDefinitionImpl source) {
		...
		if (!getTemplate().booleanValue()) {
			if (source.getTemplate().booleanValue()) {
	...
	public RegionDefinitionImpl clone() {
		...
		if (conditions != null) {
			...
			for (Condition condition : conditions) {
				copy.getConditions().add(((ConditionDefinitionImpl) condition).clone());
			}
		}
