...
public class ModelServiceImplTest {
	...
	/**
	 * Tests {@link ModelService#getClassesForOntology(String)}. Verifies that the attributes of the returned clases are properly
	 * collected and converted to {@link ClassInfo}.
	 */
	@Test
	public void testGetClassesForOntology()
		...
		ClassInfo culturalActivity = new ClassInfo()
				...
				.setOntology("http://www.sirma.com/ontologies/2013/10/culturalHeritageDomain").setSuperClasses(
						Arrays.asList("http://ittruse.ittbg.com/ontology/enterpriseManagementFramework#Activity"));
		ClassInfo concept = new ClassInfo().setId("http://www.w3.org/2004/02/skos/core#Concept")
				.setLabel("Concept")
				.setOntology("http://www.w3.org/2004/02/skos/core");
		ClassInfo activity = new ClassInfo()
				.setId("http://ittruse.ittbg.com/ontology/enterpriseManagementFramework#Activity").setLabel("Activity")
	...
	private void mockReturnedClassesForOntology() {
		...
		when(markType.getOwningInstance()).thenReturn(concept);
		...
		when(culturalActivity.getString(eq("ontology")))
				.thenReturn("http://www.sirma.com/ontologies/2013/10/culturalHeritageDomain");
		when(culturalActivity.getOwningInstance()).thenReturn(activity);
	...
	private void mockReturnedOntologies(boolean returnEmptyResult)
		...
		TransactionalRepositoryConnection repositoryConnectionMock = Mockito
				.mock(TransactionalRepositoryConnection.class);
		TupleQuery query = Mockito.mock(TupleQuery.class);
		when(repositoryConnectionMock.prepareTupleQuery(Mockito.any(QueryLanguage.class), Mockito.anyString()))
				.thenReturn(query);
		when(repositoryConnection.get()).thenReturn(repositoryConnectionMock);
