...
public class LDAPUserRegistry implements UserRegistry, LDAPNameResolver, InitializingBean, ActivateableBean {
	...
	private LDAPTenantTools ldapTenantTools;

	/**
	 * Instantiates a new lDAP user registry.
	 */
	public LDAPUserRegistry() {
		// Default to official LDAP generalized time format (unfortunately not
	...
	/**
	 * Setter method for ldapTenantTools.
	 *
	 * @param ldapTenantTools
	 *            the ldapTenantTools class to use
	 */
	public void setldapTenantToolsClass(Class<?> ldapTenantTools) {
		try {
			this.ldapTenantTools = (LDAPTenantTools) ldapTenantTools.newInstance();
		} catch (Exception e) {
	...
	public void afterPropertiesSet() throws Exception {
		...
		// Include a range restriction for the multi-valued member attribute if
		// this is enabled
		...
		this.groupKeys = initKeys(this.groupAttributeMapping, this.attributeBatchSize > 0
	...
	public Collection<String> getPersonNames() {
			...
			public void process(SearchResult result) throws NamingException, ParseException {
				...
				if (nameAttribute == null) {
					if (LDAPUserRegistry.this.errorOnMissingUID) {
						...
						LDAPUserRegistry.logger.warn("User missing user id attribute DN =" + result.getNameInNamespace()
					...
					if (!tenantId.isEmpty()) {
						personNames.add((String) nameAttribute.get() + "@" + tenantId);
					} else {
		...
		}, getUserSearchBase(), this.personQuery, new String[] { this.userIdAttributeName });
		return personNames;
	}
	...
	public Collection<String> getGroupNames() {
		...
		}, getGroupSearchBase(), this.groupQuery, new String[] { this.groupIdAttributeName });
		return groupNames;
	}

	@Override
	public Collection<NodeDescription> getGroups(Date modifiedSince) {
		// Work out whether the user and group trees are disjoint. This may
		// allow us to optimize reverse DN
		// resolution.
		final LdapName groupDistinguishedNamePrefix;
		final LdapName userDistinguishedNamePrefix;
		try {
			groupDistinguishedNamePrefix = fixedLdapName(getGroupSearchBase().toLowerCase());
			userDistinguishedNamePrefix = fixedLdapName(getUserSearchBase().toLowerCase());
		...
		processQuery(new SearchCallback() {
			// We get a whole new context to avoid interference with cookies
			// from paged results
			private DirContext ctx = LDAPUserRegistry.this.ldapInitialContextFactory.getDefaultIntialDirContext();
			private Map<String, String> nameToOUMap = new HashMap<String, String>();

			public void process(SearchResult result) throws NamingException, ParseException {
				Attributes attributes = result.getAttributes();
				String name = result.getName();
				String ou = nameToOUMap.get(name);
				if (ou == null) {
					ou = ldapTenantTools.getOrganizationUnit(result.getNameInNamespace());
					nameToOUMap.put(name, ou);
				}
				...
				NodeDescription group = lookup.get(name);
				if (group == null) {
					...
					if (SEIPTenantIntegration.isValidTenant(ou)) {
						group.getProperties().put(ContentModel.PROP_ORGANIZATION, ou);
					}
				...
				// Get the repeating (and possibly range restricted) member
				// attribute
				...
				while (memAttribute != null) {
					for (int i = 0; i < memAttribute.size(); i++) {
						...
						if (attribute != null && attribute.length() > 0) {
							try {
								// Attempt to parse the member attribute as a
								// DN. If this fails we have a fallback
								// in the catch block
								LdapName distinguishedNameForComparison = fixedLdapName(attribute.toLowerCase());
								Attribute nameAttribute;

								// If the user and group search bases are
								// different we may be able to recognize user
									...
									if (distinguishedNameForComparison.startsWith(groupDistinguishedNamePrefix)
										...
										if (LDAPUserRegistry.logger.isDebugEnabled()) {
											LDAPUserRegistry.logger
													.debug("Group DN recognized: " + "GROUP_" + nameAttribute.get());
										}
								...
								// If we can't determine the name and type from
								// the DN alone, try a directory lookup
								if (distinguishedNameForComparison.startsWith(userDistinguishedNamePrefix)
									...
									try {
										...
										if (hasAttributeValue(objectClass, LDAPUserRegistry.this.personType)) {
											...
											String uid = (String) nameAttribute.get();
											if (SEIPTenantIntegration.isValidTenant(ou) && uid.indexOf('@') == -1) {
												uid += ("@" + ou);
											}
											if (LDAPUserRegistry.logger.isDebugEnabled()) {
												LDAPUserRegistry.logger.debug("User DN recognized by directory lookup: "
														+ nameAttribute.get());
											}
											childAssocs.add(uid);
											continue;
											...
											if (LDAPUserRegistry.logger.isDebugEnabled()) {
												LDAPUserRegistry.logger
														.debug("Group DN recognized by directory lookup: " + "GROUP_"
																+ nameAttribute.get());
											}
											childAssocs.add("GROUP_" + nameAttribute.get());
											continue;
								...
								// The member attribute didn't parse as a DN. So
								// assume we have a group class like
								// posixGroup (FDS) that directly lists user
								// names
								if (LDAPUserRegistry.logger.isDebugEnabled()) {
					...
					// If we are using attribute matching and we haven't got to
					// the end (indicated by an asterisk),
		...
		}, getGroupSearchBase(), query, this.groupKeys.getFirst());
	...
	private String getGroupSearchBase() {
		...
		if (logger.isTraceEnabled()) {
			logger.trace("Request group search base for tenant: " + tenantId);
		}
		return ldapTenantTools.prepareLDAPGroupSearchBase(groupSearchBase, tenantId);
	}

	private String getUserSearchBase(String userId) {
		String tenantId = SEIPTenantIntegration.getTenantId(userId);
		if (logger.isTraceEnabled()) {
			logger.trace("Request user search base for tenant: " + tenantId);
		}
		return ldapTenantTools.prepareLDAPUserSearchBase(userSearchBase, tenantId);
	}
	...
	public String resolveDistinguishedName(String userId) throws AuthenticationException {
		...
		// Although we don't actually need any attributes, we ask for the UID
		// for compatibility with Sun Directory Server. See ALF-3868
		...
		try {
			...
			// Execute the user query with an additional condition that ensures
			// only the user with the required ID is
			// returned. Force RFC 2254 escaping of the user ID in the filter to
			// avoid any manipulation

			Pair<String, String> baseAndUserIdPair = ldapTenantTools
					.prepareLDAPSpecificUserSearchArgument(getUserSearchBase(userId), userId);
			NamingEnumeration<SearchResult> searchResults = ctx.search(baseAndUserIdPair.getFirst(),
					"(&" + this.personQuery + "(" + this.userIdAttributeName + "={0}))",
					new Object[] { baseAndUserIdPair.getSecond() }, userSearchCtls);
	...
	private Pair<String[], Set<QName>> initKeys(Map<String, String> attributeMapping, String... extraAttibutes) {
		// Compile a complete array of LDAP attribute names, including
	...
	public class PersonCollection extends AbstractCollection<NodeDescription> {
		...
		public PersonCollection(Date modifiedSince) {
			...
			// Estimate the size of this collection by running the entire query
			// once, if progress
			...
			if (LDAPUserRegistry.this.enableProgressEstimation) {
				class CountingCallback implements SearchCallback {
					 ...
					 * (non-Javadoc)
					 * 
					 * @see org.alfresco.repo.security.sync.ldap.LDAPUserRegistry.
					 * SearchCallback#process(javax.naming.directory .SearchResult)
					 */
					public void process(SearchResult result) throws NamingException, ParseException {
						this.count++;
						if (LDAPUserRegistry.logger.isDebugEnabled()) {
				...
				processQuery(countingCallback, getUserSearchBase(), this.query, new String[] {});
			...
			private NodeDescription fetchNext() throws NamingException {
				...
				do {
					...
					while (!readyForNextPage && this.searchResults.hasMore()) {
						...
						String ou = ldapTenantTools.getOrganizationUnit(result.getNameInNamespace());
						...
						// Apply the mapped properties to the node description
						NodeDescription mapToNode = mapToNode(LDAPUserRegistry.this.personAttributeMapping,
								LDAPUserRegistry.this.personAttributeDefaults, result);
						if (SEIPTenantIntegration.isValidTenant(ou)) {
							mapToNode.getProperties().put(ContentModel.PROP_ORGANIZATION, ou);
						}
						mapToNode.getProperties().put(ContentModel.PROP_USERNAME, uid);
						return mapToNode;
					}

					// Examine the paged results control response for an
					// indication that another page is available
					if (!readyForNextPage) {
					...
					if (readyForNextPage) {
						this.searchResults = this.ctx.search(getUserSearchBase(), PersonCollection.this.query,
								this.userSearchCtls);
					}
