...
public class AlfrescoFacade {
	...
	protected void populateSession(HttpSession httpSess, User user) {
		// added user ticket to session because share can't parse User Object
		httpSess.setAttribute("alfTicket", user.getTicket());
	...
	/**
	 * Register a session to its idp index. If the same index exists it is
	 ...
	 * @param session
	 *            the session to link
	 * @return null if the value is not added or if the index has not been
	 ...
	 * @param sessionIndex
	 *            the session index
	 * @return the session for the key or null if not found or sessionIndex is
	 *         null
	 */
	 ...
	 * @param sessionIndex
	 *            the session index
	 * @return the session that has been linked to the key or null if not found
	 *         or sessionIndex is null
	 */
