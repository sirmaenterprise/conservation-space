...
export class DeactivateTemplateAction extends InstanceAction {
  ...
  execute(actionDefinition, context) {
    return this.templateService.deactivateTemplate(context.currentObject.getId()).then(() => {
      let message = this.translateService.translateInstant('idoc.template.deactivation.success');
