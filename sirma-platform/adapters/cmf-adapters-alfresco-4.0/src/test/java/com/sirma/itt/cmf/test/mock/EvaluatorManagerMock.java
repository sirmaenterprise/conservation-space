...
public class EvaluatorManagerMock extends ExpressionEvaluatorManager {
	...
	public Map<String, Serializable> evaluateRules(Set<PropertyDefinition> definitions, ExpressionContext context,
		...
		Map<String, Serializable> mapping = new LinkedHashMap<String, Serializable>((int) (definitions.size() * 1.1),
				0.95f);
	...
	public Serializable evaluateRule(PropertyDefinition propertyDefinition, ExpressionContext context,
		...
		if (evaluator != null) {
			Serializable serializable = evaluator.evaluate(propertyDefinition.getRnc(), context, target);
			Serializable convert = (Serializable) typeConverter.convert(propertyDefinition.getDataType().getJavaClass(),
					serializable);
			return convert;
		}
		return null;
	...
	public Serializable ruleConvertTo(PropertyDefinition definition, Serializable value) {
		if (definition == null || StringUtils.isNullOrEmpty(definition.getRnc()) || value == null) {
			return value;
		}
