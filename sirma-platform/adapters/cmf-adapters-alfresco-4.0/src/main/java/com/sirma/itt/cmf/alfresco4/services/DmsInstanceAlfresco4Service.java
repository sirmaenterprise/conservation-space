...
public class DmsInstanceAlfresco4Service implements DMSInstanceAdapterService, AlfrescoCommunicationConstants {
	...
	public DMSInstance attachFolderToInstance(DMSInstance parent, DMSInstance child) throws DMSException {
		AlfrescoUtils.validateExistingDMSInstance(parent);
		if (child == null) {
			throw new DMSException("Invalid element 'null' is provided for child!");
		}
		try {
			DMSTypeConverter typeConverter = converterFactory.getConverter(child.getClass());
			Map<String, Serializable> dmsProperties = typeConverter.convertCMFtoDMSProperties(child.getProperties(),
					child, DMSTypeConverter.EDITABLE_HIDDEN_MANDATORY_LEVEL);
			JSONObject request = new JSONObject();
			...
			throw new DMSException("Faild to attach '" + child + "' as child to parent '" + parent + "'", e);
		} catch (Exception e) {
	...
	public FileAndPropertiesDescriptor attachDocumentToInstance(DMSInstance docInstance, FileDescriptor descriptor,
		...
		if (docInstance.getDmsId() == null) {
			...
			if (dmsId == null) {
				dmsId = parentNode.getString(SectionProperties.DMS_ID);
			}
	...
	public String dettachDocumentFromInstance(DMSInstance documentInstance) throws DMSException {
		...
		return documentAdapter.get().deleteAttachment(documentInstance);
	}

	@Override
	public boolean linkAsChild(DMSInstance parent, DMSInstance child, DMSInstance parentToRemove, String assocName) {
		if (parent == null || parent.getDmsId() == null) {
			throw new IllegalArgumentException("Invalid element id is provided for parent!");
		}
		if (child == null || child.getDmsId() == null) {
			throw new IllegalArgumentException("Invalid element id is provided for child!");
		}
		try {
			JSONObject request = new JSONObject();
			request.put(KEY_NODEID, parent.getDmsId());
			...
			throw new RollbackedRuntimeException("Faild to link '" + child + "' to parent '" + parent + "'", e);
		} catch (Exception e) {
			throw new RollbackedRuntimeException(
					"Faild to link '" + child + "' to parent '" + parent + "'! " + AlfrescoErrorReader.parse(e), e);
		}
		throw new RollbackedRuntimeException("Faild to link '" + child + "' to parent '" + parent + "'");
	}
