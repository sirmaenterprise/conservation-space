...
public class ImageManifestServiceImpl implements ImageManifestService {
	...
	private JsonObject buildManifest(String imageWidgetId, Collection<String> instanceIds) {
		...
		currentSequence.add(CANVASES, buildCanvases(uploadedImagesInfo));
	...
	private JsonArrayBuilder buildCanvases(Collection<ContentInfo> imagesContent) {
		...
		for (ContentInfo contentInfo : imagesContent) {
			if (contentInfo.exists()) {
				...
				JsonObjectBuilder resource = buildResourceInfo(counter, currentImageDimension, service);
				JsonObjectBuilder image = buildImageSection(counter, resource, contentInfo.getName());

				canvases.add(buildCanvas(contentInfo.getInstanceId().toString(), contentInfo.getName(),
						currentImageDimension, image));
			} else {
				useNoContentFallbackImage(canvases, counter);
			}
		}
