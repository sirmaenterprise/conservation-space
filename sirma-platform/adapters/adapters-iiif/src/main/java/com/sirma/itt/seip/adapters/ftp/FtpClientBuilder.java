...
public class FtpClientBuilder {
	...
	public FTPClient buildClient(FTPConfiguration configuration) throws DMSException {
		...
		try {
			...
			if (!FTPReply.isPositiveCompletion(reply)) {
				...
				throw new DMSException("Could not connect to ftp server. Recieved a reply code: " + reply);
			}
			...
			if (configuration.getUsername() != null && configuration.getPassword() != null) {
				...
				LOGGER.debug("Succesfully connected to the FTP server.");
			}
		} catch (IOException e) {
			throw new DMSException("An error occured while trying to connect to the server.", e);
		}
		return ftpClient;
	}
	...
	public void closeClient(FTPClient client) {
		...
		try {
			...
			LOGGER.error("Error occured while trying to disconnect from the FTP server.", e);
		}
	}
