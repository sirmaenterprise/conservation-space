...
public class AlfrescoRESTClient implements RESTClient {
	 ...
	 */
	private static Map<String, String> sessions = new HashMap<>(32);
	...
	public HttpMethod rawRequest(HttpMethod method, URI uri) throws DMSClientException {
		try {
			...
			LOGGER.debug("Received response with status code: {} @ {}", statusCode, uri);
			if (statusCode == 200) {
				return method;
			}
	...
	private void setAuthentication(HttpMethod method, Object uri, HttpClient client) {
		...
		if (method.getRequestHeader(SAML_TOKEN) != null) {
			...
			com.sirma.itt.seip.security.User authentication = securityContext.getEffectiveAuthentication();
			if (authentication == null) {
				authentication = securityContext.getAuthenticated();
			}
		...
		if (ticket != null) {
			...
			LOGGER.debug("INVOKING WEBSCRIPT @ {} user: {} token: PROTECTED", uri, username);
		} else if (username != null && password != null) {
			setDefaultCredentials(client, username, password);
			LOGGER.debug("INVOKING WEBSCRIPT @ {} user: {} pass: PROTECTED", uri, username);
		} else {
	 ...
	 */
	private static String getResponse(HttpMethod method) throws DMSClientException {
