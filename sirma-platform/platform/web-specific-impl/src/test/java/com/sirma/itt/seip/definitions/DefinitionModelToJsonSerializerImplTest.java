...
public class DefinitionModelToJsonSerializerImplTest {
	...
	public void serialize() throws IOException {
		...
		try (ByteArrayOutputStream stream = new ByteArrayOutputStream();
			...
			JsonObject expected = Json.createReader(classLoader.getResourceAsStream(DEFINITION_MODEL_JSON))
					.readObject();
	...
	private static EmfInstance prepareInstance() {
		...
		currentInstance.setOwningInstance(parentInstance);
	...
	private void prepareStubs(DefinitionModel model, Instance instance) {
		...
		DataTypeDefinitionMock dataTypeDefinition = new DataTypeDefinitionMock(instance);
		when(dictionaryService.getDataTypeDefinition(instance.getClass().getName())).thenReturn(dataTypeDefinition);
	}
	...
	private static List<Ordinal> buildDefinitionFields() {
		List<Ordinal> fields = Arrays.asList(buildIdentifierField(), buildTypeField(), buildTitleField(),
				buildCreatedOnField(), buildContentField(), buildDepartmentField(), buildRegion());
	...
	private static ControlDefinition buildControlDefinition() {
		...
		ControlParam inclusive = buildControlParam("INCLUSIVE", "true");
		departmentControlDefinition.getControlParams().addAll(Arrays.asList(inclusive,
				buildControlParam("FILTER_SOURCE", "extra1"), buildControlParam("RERENDER", "functional")));
		return departmentControlDefinition;
	}

	private static ControlParam buildControlParam(String name, String value) {
