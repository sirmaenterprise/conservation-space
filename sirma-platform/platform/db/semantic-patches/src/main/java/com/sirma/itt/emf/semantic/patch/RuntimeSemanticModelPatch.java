...
public class RuntimeSemanticModelPatch implements SemanticSchemaPatches {
	...
	public static final String CHANGELOG_XML_SUFFIX = "-changelog.xml";
	public static final String[] ALLOWED_FILES = { NAMESPACES_FILE_EXTENSION, NS_FILE_EXTENSION,
			SPARQL_FILE_EXTENSION };
	public static final String CHANGE_SET_TEMPLATE = "<changeSet id=\"%s\" author=\"system\">\r\n"
	...
	private static File generateChangelogFile(File location) {
		...
		try {
			...
			for (File file : files) {
				...
				String fileType;
				if (RDFFormat.forFileName(fileName) != null) {
			...
			FileUtils.writeStringToFile(changeLogFile,
					template.replace(CHANGELOG_TEMPLATE_ZONE, changeSetBuilder.toString()));
