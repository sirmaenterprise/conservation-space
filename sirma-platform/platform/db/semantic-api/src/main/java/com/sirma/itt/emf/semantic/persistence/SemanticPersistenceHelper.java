...
public class SemanticPersistenceHelper {
	 ...
	 */
	public static void saveModel(RepositoryConnection connection, Model model, URI graph) {
		try {
	 ...
	 *            Graph to insert the statements from the model
	 */
	public static void updateModel(RepositoryConnection connection, Model model, URI graph) {
		try {
			if (CollectionUtils.isNotEmpty(model)) {
	 ...
	 *            Graph to insert the statements from the model
	 */
	public static void removeModel(RepositoryConnection connection, Model model, URI graph) {
		try {
			if (CollectionUtils.isNotEmpty(model)) {
	...
	private static Statement createStatement(Object subjectUri, Object predicate, Serializable value, boolean isLiteral,
		...
		URI subject = createURI(subjectUri, registryService);
		URI predicateURI = createURI(predicate, registryService);
	...
	/**
	 * Creates literal or URI for the given value
	 *
	 * @param value
	 *            the value
	 * @param isLiteral
