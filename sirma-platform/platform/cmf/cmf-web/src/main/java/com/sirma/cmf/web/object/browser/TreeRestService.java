...
public class TreeRestService extends EmfRestService {
	...
	protected static final String OBJECTINSTANCE = "objectinstance";
	...
	public Response getRoots(@QueryParam("rootType") String rootType, @QueryParam("limit") Integer limit,
		...
		for (Instance instance : roots) {
			...
			String instanceHeader = (String) instance.getProperties().get(
					StringUtils.isNotNullOrEmpty(header) ? header : DefaultProperties.HEADER_COMPACT);
	...
	private List<? extends Instance> loadRootInstances(String actualRootType, Integer limit, String term) {
		...
		if (StringUtils.isNotNullOrEmpty(term)) {
			context.put("term", " AND _all_text:\"*" + term + "*\"");
		}
	...
	public Response tree(@QueryParam("node") String node, @QueryParam("type") String type,
			@QueryParam("currId") String currentInstanceId, @QueryParam("currType") String currentInstanceType,
		...
		LOG.debug(
				"EMFWeb: ObjectsBrowserRestService.load node={}, type={}, currentInstanceId={}, currentInstanceType={}, allowSelection={}, filters={}, clickableLinks={}, clickOpenWindow={}, manualExpand={}",
				node, type, currentInstanceId, currentInstanceType, allowSelection, filters, clickableLinks,
				clickOpenWindow, manualExpand);
		...
		// If the tree is manually expanded by user, we may skip node path building process.
		if (!manualExpand && StringUtils.isNotNullOrEmpty(currentInstanceId)) {
			currentNodePath = buildPathToCurrentNode(currentInstanceId, currentInstanceType);
		}

		// buid JSON response
		JSONArray nodes = buildData(grouped, rootInstance, node, shouldBeGrouped(rootInstance), currentNodePath,
				allowSelection, clickableLinks, clickOpenWindow, filters, header);
	...
	protected LinkSearchArguments buildSearchArguments(Instance rootInstance, Integer page) {
		...
		// avoid empty result if not having READ/WRITE permissions over all prior elements
	 ...
	 * @return the string
	 */
	protected String buildPathToCurrentNode(String currentInstanceId, String currentInstanceType) {
		Instance currentInstanceNode = fetchInstance(currentInstanceId, currentInstanceType);
		if (currentInstanceNode == null) {
			return "";
		}

		instanceContextInitializer.restoreHierarchy(currentInstanceNode);
