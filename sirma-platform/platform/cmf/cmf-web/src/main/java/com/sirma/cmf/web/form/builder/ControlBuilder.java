...
public class ControlBuilder extends FormBuilder {
	 ...
	 */
	protected List<ControlParam> getParametersByName(String requestedParameterName, List<ControlParam> controlParams) {
	 ...
	 */
	protected ControlParam getControlParameter(String requestedParameterName, List<ControlParam> controlParams) {
	 ...
	 */
	public String getControlParameterValue(String requestedParameterName, List<ControlParam> controlParams) {
		ControlParam controlParam = null;
		if (controlParams != null && StringUtils.isNotNullOrEmpty(requestedParameterName)) {
			for (ControlParam currentControlParam : controlParams) {
				if (currentControlParam.getName().endsWith(requestedParameterName)) {
	...
	protected void addGroupLabel(UIComponent wrapper, String additionalStyleClass) {
		...
		if (StringUtils.isNotNullOrEmpty(labelAttribute)) {
			...
			HtmlOutputText label = (HtmlOutputText) builderHelper.getComponent(ComponentType.OUTPUT_TEXT);
