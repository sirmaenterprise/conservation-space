...
public class FormBuilderHelper {
	...
	public UIComponent getComponent(ComponentType componentType) {
		...
		if (componentType == ComponentType.COMMAND_LINK) {
			...
			component = createComponentInstance(JAVAX_FACES_HTML_OUTPUT_LINK, JAVAX_FACES_LINK_RENDERER);
		} else if (componentType == ComponentType.A4J_COMMAND_LINK) {
			component = createComponentInstance("org.richfaces.CommandLink", "org.richfaces.CommandLinkRenderer");
			...
			component = createComponentInstance("org.richfaces.MediaOutput", "org.richfaces.MediaOutputRenderer");
		} else if (componentType == ComponentType.SELECT_MANY_CHECKBOX) {
			component = createComponentInstance("javax.faces.HtmlSelectManyCheckbox", "javax.faces.Checkbox");
		} else if (componentType == ComponentType.SELECT_BOOLEAN_CHECKBOX) {
			component = createComponentInstance("javax.faces.HtmlSelectBooleanCheckbox", "javax.faces.Checkbox");
			...
			component = createComponentInstance("javax.faces.HtmlSelectOneRadio", "javax.faces.Radio");
			...
			component = createComponentInstance(JAVAX_FACES_HTML_INPUT_TEXTAREA, JAVAX_FACES_TEXTAREA_RENDERER);
		} else if (componentType == ComponentType.SELECT_ONE_MENU) {
			component = createComponentInstance(JAVAX_FACES_HTML_SELECT_ONE_MENU, JAVAX_FACES_MENU_RENDERER);
			...
			component = createComponentInstance("org.richfaces.OutputPanel", "org.richfaces.OutputPanelRenderer");
	 ...
	 */
	public List<ControlParam> getParametersByName(String parameterName, List<ControlParam> controlParams) {
	...
	/**
	 * Gets the parameter by name. If is found requested parameter, then it is returned. If more than one parameters
	 * with given name exists, then the first one is returned. Returns null if no parameter with given name is found.
	 *
	 * @param parameterName
	 *            the parameter name
	 * @param controlParams
	...
	public UICommandButton createAjaxButton(String id, MethodExpression actionExpression,
			ValueExpression disabledExpression, String oncompleteAttribute, String buttonValue, String render,
			String execute, String styleClass, Boolean rendered, Boolean immediate) {

		UICommandButton commandButton = (UICommandButton) createComponentInstance(ORG_RICHFACES_COMMAND_BUTTON,
				ORG_RICHFACES_COMMAND_BUTTON_RENDERER);
	 ...
	 */
	public UICommandLink createAjaxLink(String linkId, MethodExpression actionExpression, String linkValueExpression,
			ValueExpression disabledExpression, Boolean immediate, String render, String execute) {
		...
		HtmlOutputText linkValue = (HtmlOutputText) createComponentInstance(JAVAX_FACES_HTML_OUTPUT_TEXT,
				JAVAX_FACES_TEXT);
		linkValue.setValueExpression("value", createValueExpression(linkValueExpression, String.class));
	...
	public HtmlOutputLink createOutputLink(String fieldName, String valueExpressionString,
		...
		HtmlOutputLink link = (HtmlOutputLink) createComponentInstance(JAVAX_FACES_HTML_OUTPUT_LINK,
		...
		if (StringUtils.isNotNullOrEmpty(valueExpressionString)) {
			link.setValueExpression("value", createValueExpression(valueExpressionString, String.class));
		}

		if (StringUtils.isNotNullOrEmpty(disabledExpressionString)) {
			link.setValueExpression("disabled", createValueExpression(disabledExpressionString, Boolean.class));
		} else {
			link.setDisabled(false);
		}

		HtmlOutputText outputText = (HtmlOutputText) createComponentInstance(JAVAX_FACES_HTML_OUTPUT_TEXT,
				JAVAX_FACES_TEXT);
		outputText.setEscape(false);

		if (StringUtils.isNotNullOrEmpty(linkLabelExpressionString)) {
			outputText.setValueExpression("value", createValueExpression(linkLabelExpressionString, String.class));
		} else {
	...
	public HtmlGraphicImage createGraphicImage(String imageName) {
		...
		image.setValueExpression("value", createValueExpression("../images:" + imageName + "", String.class));
	...
	private ValueExpression createValueExpression(String stringValueExpression, Class<?> valueType) {
		...
		return facesContext.getApplication().getExpressionFactory().createValueExpression(facesContext.getELContext(),
				stringValueExpression, valueType);
	}
}
