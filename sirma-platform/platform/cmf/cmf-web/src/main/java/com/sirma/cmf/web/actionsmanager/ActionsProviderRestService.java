...
public class ActionsProviderRestService extends EmfRestService {
	...
	public Response evaluateActions(@QueryParam("instanceId") String instanceId,
			@QueryParam("instanceType") String instanceType, @QueryParam("contextId") String contextId,
			@QueryParam("contextType") String contextType, @QueryParam("placeholder") String placeholder) {
		...
		List<Action> actionsByInstance = actionsProvider.evaluateActionsByInstance(instance, placeholder, context);
		JSONObject response = new JSONObject();
		Action.addActions(response, actionsByInstance);
		Response responseObject = buildResponse(Status.OK, response);

		LOG.debug("Evaluating actions for instanceId[{}], instanceType[{}] and placeholder[{}] took {} sec", instanceId,
				instanceType, placeholder, timeTracker.stopInSeconds());
