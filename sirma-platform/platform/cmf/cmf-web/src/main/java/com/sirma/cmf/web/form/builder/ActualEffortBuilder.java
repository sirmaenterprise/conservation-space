...
/**
 * Builder class for actual effort field. Used to convert field value from minutes to user readable string of type d h m
 * (example: 2d 4h 30m)
 */
public class ActualEffortBuilder extends FormBuilder {
	 ...
	 */
	public ActualEffortBuilder(LabelProvider labelProvider, CodelistService codelistService) {
	...
	public UIComponent getComponentInstance() {
		HtmlOutputText output = (HtmlOutputText) builderHelper.getComponent(ComponentType.OUTPUT_TEXT);
		...
		output.setValueExpression("value", createValueExpression(valueExpressionForUser, String.class));
	...
	public List<UIComponent> buildOutputField() {
		...
		componentsList.add(uiComponent);
