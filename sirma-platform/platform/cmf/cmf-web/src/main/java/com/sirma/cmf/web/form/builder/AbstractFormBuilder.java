...
public abstract class AbstractFormBuilder {
	 ...
	 *            The component.
	 * @param validatorData
	 *            the validator data contains the type to be used as regex pattern and message if validation fails.
	 */
	 ...
	 * @return EL expression as string.
	 */
	protected String getValueExpressionStringForCodelist(String instanceName, String propertyName, int clNumber) {
		// TODO: try to use LabelBuilder.getCodelistDisplayValue in order to have consistent value
		String expression = MessageFormat.format("#'{'cls.getDescription({0},{1}.properties[''{2}''])'}'", clNumber,
				instanceName, propertyName);
		return expression;
	}

	/**
	 * Assembles an EL expression for getting the code of given property as string. * @param instanceName The object
	 * instance name.
	 *
	 * @param propertyName
	 *            The instance's property name.
	 ...
	 * @return EL expression as string.
	 */
	protected String getCodeExpressionStringForCodeList(int clNumber, String instanceName, String propertyName) {
		String expression = MessageFormat.format("#'{'cls.getCodeValue({0},{1}.properties[''{2}'']).getValue()'}'",
				clNumber, instanceName, propertyName);
		return expression;
	}
	...
	protected String getValueExpressionForUser(String fieldname) {
		return MessageFormat.format("#'{'labelBuilder.getUserDisplayName(documentContext,''{0}'')'}'", fieldname);
	}
	 ...
	 * @return the method expression
	 */
	protected MethodExpression createMethodExpression(String stringExpression, Class<?>[] argumentsType) {
	 ...
	 * @return the method expression
	 */
	private MethodExpression _createMethodExpression(String stringExpression, Class<?>[] argumentsType) {
		FacesContext facesContext = FacesContext.getCurrentInstance();
		return facesContext.getApplication().getExpressionFactory().createMethodExpression(facesContext.getELContext(),
				stringExpression, String.class, argumentsType);
	}
