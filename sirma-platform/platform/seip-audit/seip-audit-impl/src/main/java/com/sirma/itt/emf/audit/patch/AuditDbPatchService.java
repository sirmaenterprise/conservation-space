...
public class AuditDbPatchService {
	...
	@Startup(phase = StartupPhase.BEFORE_APP_START, order = 3.5, async = true)
	...
	@Startup(phase = StartupPhase.BEFORE_APP_START, order = 3.4, async = true)
	...
	public void patchRecentActivities(AuditConfiguration configuration) throws RollbackedException {
		...
		LOGGER.info("recent activities audit patching completed in " + tracker.stop() + " ms");
	}
}
