...
public class AuditRelationObserverTest {
	...
	public void testAddRelation() {
		...
		ObjectPropertyAddEvent event = new AddRelationEvent(mockStatement(), uri -> uri.stringValue());
	...
	public void testRemoveRelation() {
		...
		ObjectPropertyRemoveEvent event = new RemoveRelationEvent(mockStatement(), uri -> uri.stringValue());
	...
	private static LocalStatement mockStatement() {
		...
		URI sourceId = getURI("sourceId");
		Mockito.when(statement.getSubject()).thenReturn(sourceId);

		URI predicate = getURI("pred");
	...
	private static URI getURI(String value) {
		URI rdfValue = Mockito.mock(URI.class);
