...
/**
 * <strong> Copied original code with minor modifications as best working solution.</strong> Utility class for HTML form
 ...
 * the HTML <A HREF="http://www.w3.org/TR/html4/">specification</A>. <br>
 * When encoding a String, the following rules apply: <br>
 * <ul>
 * <li>The alphanumeric characters &quot;<code>a</code>&quot; through &quot; <code>z</code>&quot;, &quot;<code>A</code>
 * &quot; through &quot;<code>Z</code> &quot; and &quot;<code>0</code>&quot; through &quot;<code>9</code>&quot; remain
 * the same.
...
public class StringEncoder {
	...
	static {
		...
		/*
		 * The list of characters that are not encoded has been determined as follows: RFC 2396 states: ----- Data
		 * characters that are allowed in a URI but do not have a reserved purpose are called unreserved. These include
		 * upper and lower case letters, decimal digits, and a limited set of punctuation marks and symbols. unreserved
		 * = alphanum | mark mark = "-" | "_" | "." | "!" | "~" | "*" | "'" | "(" | ")" Unreserved characters can be
		 * escaped without changing the semantics of the URI, but this should not be done unless the URI is being used
		 * in a context that does not allow the unescaped character to appear. ----- It appears that both Netscape and
		 * Internet Explorer escape all special characters from this list with the exception of "-", "_", ".", "*".
		 * While it is not clear why they are escaping the other characters, perhaps it is safest to assume that there
		 * might be contexts in which the others are unsafe if not escaped. Therefore, we will use the same list. It is
		 * also noteworthy that this is consistent with O'Reilly's "HTML: The Definitive Guide" (page 164). As a last
		 * note, Intenet Explorer does not encode the "@" character which is clearly not unreserved according to the
	...
	/**
	 * Translates a string into <code>x-www-form-urlencoded</code> format. This method uses the platform's default
	 * encoding as the encoding scheme to obtain the bytes for unsafe characters.
	 *
	 * @param s
	...
	public static String encode(String s) {
		...
		try {
			str = encode(s, dfltEncName, dontNeedEncoding);
		} catch (UnsupportedEncodingException e) {
			// The system should always have the platform default
			LOGGER.trace("The system should always have the platform default but obviously not", e);
		}
	...
	/**
	 * Encode with the default confing
	 *
	 * @param s
	 ...
	 * @param dontNeedEncodingParam
	 *            is the set with not convertable chars.
	 * @return the translated <code>String</code>.
	 ...
	 * @since 1.4
	 */
	public static String encode(String s, String enc, BitSet dontNeedEncodingParam)
			throws UnsupportedEncodingException {
		...
		try {
			...
			LOGGER.trace("Invalid charset name {}", enc, e);
			throw new UnsupportedEncodingException(enc);
		} catch (UnsupportedCharsetException e) {
			LOGGER.trace("No such charset {}", enc, e);
			throw new UnsupportedEncodingException(enc);
		}

		for (int i = 0; i < s.length();) {
			int c = s.charAt(i);
			if (dontNeedEncodingParam.get(c)) {
				if (c == ' ') {
					c = '+';
					needToChange = true;
				}
				...
				do {
					...
					i++;
				} while (i < s.length() && !dontNeedEncodingParam.get(c = s.charAt(i)));
