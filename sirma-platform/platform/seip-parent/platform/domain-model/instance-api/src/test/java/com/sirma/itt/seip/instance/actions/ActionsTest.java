...
public class ActionsTest {
	...
	public void beforeMethod() {
		...
		when(transactionSupport.invokeInNewTx(any(Callable.class))).then(
				a -> a.getArgumentAt(0, Callable.class).call());
		doAnswer(a -> a.getArgumentAt(0, Executable.class).asSupplier().get()).when(transactionSupport)
				.invokeInNewTx(any(Executable.class));
		when(securityContext.getRequestId()).thenReturn(REQUEST_UUID);
	}

	@Test
	public void callAction_actionValidationFailed() {
	...
	@Test
	public void callAction_shouldLockInstanceWhenActionAllows() {
		ActionRequest request = new Request("actionName");
		request.setUserOperation("doSomething");
		request.setTargetId("emf:instance");
	...
	public void callAction_shouldFailIfFailToAcquireLockInTime() {
		ActionRequest request = new Request("actionName");
		request.setUserOperation("doSomething");
		request.setTargetId("emf:instance");
	...
	public void callAction_missingInstance() {
		...
		when(resolver.resolveReference("emf:missingInstance"))
				.then(a -> getReference(a.getArgumentAt(0, String.class), null));
		actions.callAction(request);
		verify(action).perform(request);
		verify(eventService, never()).fire(any());
		verify(eventService).fireNextPhase(eq(null));
