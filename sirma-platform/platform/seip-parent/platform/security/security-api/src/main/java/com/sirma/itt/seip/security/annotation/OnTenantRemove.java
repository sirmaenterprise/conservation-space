package com.sirma.itt.seip.security.annotation;

import java.lang.annotation.Documented;
import java.lang.annotation.ElementType;
import java.lang.annotation.Inherited;
import java.lang.annotation.Retention;
import java.lang.annotation.RetentionPolicy;
import java.lang.annotation.Target;

/**
 * Marks a method to be called after new tenant has been removed or deactivated in the system.
 * <p>
 * This should be used to initialize components that have some specific logic that need to be executed for each tenant.
 * This is intended for use in combination of {@link RunAsAllTenantAdmins} when something must called and for new
 * tenants during live application without the need to be restarted.
 * <p>
 * If tenant module is not present then this annotation does not affect anything.
 * <p>
 * The annotated method will only be called if tenant is removed while application is running
 *
 * @author BBonev
 */
@Inherited
@Documented
@Target({ ElementType.METHOD })
@Retention(RetentionPolicy.RUNTIME)
public @interface OnTenantRemove {

	/**
	 * Defines an order between all other observers for the tenant deletion
	 * @return the observer order. The bigger the order the later in invocation
	 */
	double order() default Integer.MAX_VALUE;
}
