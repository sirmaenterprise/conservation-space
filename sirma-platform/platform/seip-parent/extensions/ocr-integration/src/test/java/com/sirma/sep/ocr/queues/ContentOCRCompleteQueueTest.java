...
public class ContentOCRCompleteQueueTest {
	...
	public void should_saveOCRedContent_onOCRCompleted() throws JMSException, IOException, URISyntaxException {
		...
		String instanceVersionId = "instanceVersionId";
		...
		when(message.getStringProperty(InstanceCommunicationConstants.INSTANCE_VERSION_ID)).thenReturn(
				instanceVersionId);
		when(message.getStringProperty(OCRContentMessageAttributes.OCRED_VERSION_CONTENT_ID))
				.thenReturn("ocredVersionContentId");
		...
		verify(contentPersister).saveOcrContent(instanceVersionId, oceredText);
	}
}
