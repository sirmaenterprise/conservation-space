...
class UserPreferencesImpl implements UserPreferences {
	...
	@Inject
	@Configuration
	@ConfigurationPropertyDefinition(name = "session.timeout.period", type = Integer.class, defaultValue = "30", label = "Default user session timeout time in minutes.")
	private ConfigurationProperty<Integer> sessionTimeoutPeriod;

	@Inject
	@Configuration
	@ConfigurationPropertyDefinition(name = "session.timeout.redirect", type = Boolean.class, defaultValue = "false", label = "Should the user be redirected to login page when session times out.")
