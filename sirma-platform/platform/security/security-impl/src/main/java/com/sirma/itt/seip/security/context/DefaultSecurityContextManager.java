...
class DefaultSecurityContextManager implements SecurityContextManager {
	...
	public void beginContextExecution(User user) {
		if (!SecurityContextHolder.isSet()) {
			throw new ContextNotActiveException(
					"Security context not initialized. Call SecurityContextManager.initializeExecution(User) method first before calling SecurityContextManager.beginContextExecution(User)!");
		}
	...
	public boolean initializeExecution(AuthenticationContext authenticationContext) {
		...
		if (SecurityContextHolder.isSet()) {
			...
			if (!nullSafeEquals(authenticated, SecurityContextHolder.getContext().getAuthenticated())) {
				throw new SecurityException("Secure context already active for "
						+ SecurityContextHolder.getContext().getAuthenticated() + ". Cannot activate again!");
			}
			LOGGER.warn("Requested authentication for {} user twice", authenticated.getIdentityId());
		}

		if (!authenticated.canLogin()) {
			throw new AuthenticationException(authenticated.getIdentityId(),
					"User " + authenticated.getDisplayName() + " not allow to log in!");
		}
	...
	public void endExecution() {
		...
		try {
			...
			LOGGER.trace("End security session for user: {}@{}", currentContext.getAuthenticated().getIdentityId(),
					currentContext.getAuthenticated().getTenantId());
		} finally {
			clearLoggingContext();
		}
	...
	public void initializeExecutionAsSystemAdmin() {
		...
		if (checkIsSystem(tenantId) != null) {
			return;
		}
	...
	@Override
	public void initializeTenantContext(String tenantId) {
		if (checkIsSystem(tenantId) != null) {
			return;
		}
		String requestId = getOrGenerateRequestId();
		LOGGER.trace("Activating tenant context for {}", tenantId);
		setLoggingContext(tenantId, "System@" + tenantId, requestId);
	...
	private SecurityContext checkIsSystem(String tenantId) {
