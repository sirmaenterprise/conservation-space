...
public class SecurityConfigurationImpl implements SecurityConfiguration {
	...
	private static final String TRUST_STORE_PWD = "security.trustStore.password";
	...
	static final String SYSTEM_USER_PWD = "security.user.system.password";
	...
	static final String ADMIN_PWD = "admin.password";
	...
	@Inject
	@Configuration
	@ConfigurationPropertyDefinition(name = "admin.groupname", label = "The admin group name.The default is null, so no group is considered as admin.")
	...
	static User buildSystemUser(GroupConverterContext context, UserStore userStore, SecurityContext securityContext,
		...
		String systemUserName = context.get(SYSTEM_USER_NAME);
	...
	public User getSystemAdminUser() {
		if (systemAdminUserCached == null) {
			systemAdminUserCached = systemAdminUser.get();
		}
	...
	private abstract static class PredefinedUser implements User {
		...
		PredefinedUser(String identity, String tenantId, String ticket, Object credential,
