 ...
 * instances.
 * <p>
 * If the authenticated user should be changed during the session the methods {@link #executeAs(User)} could be used.
 *
 * @author BBonev
 */
public interface SecurityContextManager {
	 ...
	 *            the authentication context used to initialize the execution
	 ...
	 *
	 * @param user
	 *            the user permissions to use for executions
	 */
	 ...
	 *            the tenant id
	 */
	void initializeTenantContext(String tenantId);
	...
	default boolean isSystemUser(User user) {
		return user == null ? false : getSystemUser().getSystemId().equals(user.getSystemId());
	}
	 ...
	 *
	 * @param user
	 *            to use to change the current security context
