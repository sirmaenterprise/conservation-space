...
class DefaultLockService implements LockService {
	...
	public LockInfo lockStatus(InstanceReference reference) {
		...
		List<LockEntity> entities = dbDao.fetchWithNamed(LockEntity.QUERY_LOCK_INFO_FOR_INSTANCE_KEY,
				Arrays.asList(new Pair<>("instanceId", reference.getIdentifier())));
	...
	@Override
	@Transactional
	public LockInfo tryLock(InstanceReference reference) {
		return lockInternal(reference, true, "", Function.identity());
	}

	@Override
	@Transactional
	public LockInfo forceLock(InstanceReference reference) {
		return lockInternal(reference, false, "", Function.identity());
	}
	...
	public LockInfo tryUnlock(InstanceReference reference) {
		return unlockInternal(reference, true, Function.identity());
	}
	...
	private LockInfo unlockInternal(InstanceReference reference, boolean checkPermissions,
		...
		if (!lockStatus.isLockedByMe()) {
			if (checkPermissions && !hasPermissionsToUnlock(lockStatus.getLockedInstance())) {
		...
		int unlocked = dbDao.executeUpdate(LockEntity.UNLOCK_INSTANCE_KEY, Arrays.asList(
				new Pair<>("instanceId", reference.getIdentifier()), new Pair<>("lockedBy", lockStatus.getLockedBy())));
		if (unlocked == 0) {
			LOGGER.warn("Tried to unlock already unlocked instance: {}", reference.getIdentifier());
		}
		return createNotLockedInfo(reference);
	}
