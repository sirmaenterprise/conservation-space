...
@org.hibernate.annotations.Table(appliesTo = "seip_lockedentity", indexes = {})
@AssociationOverrides(value = {
		@AssociationOverride(name = "lockedInstance.sourceType", joinColumns = @JoinColumn(name = "lockedinstancetype", nullable = false) ) })
...
public class LockEntity extends BaseEntity {
	...
	static final String UNLOCK_INSTANCE = "delete from LockEntity where lockedInstance.sourceId = :instanceId and lockedBy = :lockedBy";

	static final String QUERY_LOCK_INFO_FOR_INSTANCE_KEY = "QUERY_LOCK_INFO_FOR_INSTANCE";
	static final String QUERY_LOCK_INFO_FOR_INSTANCE = "from LockEntity where lockedInstance.sourceId = :instanceId";

	static final String QUERY_LOCK_INFO_FOR_INSTANCES_KEY = "QUERY_LOCK_INFO_FOR_INSTANCES";
	static final String QUERY_LOCK_INFO_FOR_INSTANCES = "from LockEntity where lockedInstance.sourceId in (:instanceId)";

	@AttributeOverrides(value = {
			@AttributeOverride(name = "sourceId", column = @Column(name = "lockedinstanceid", length = 128, nullable = false) ) })
	...
	@Column(name = "locktype", length = 16)
