package com.sirmaenterprise.sep.export.services;
...
public class HtmlTableAnnotationService {
	...
	private int addRow(HtmlTableBuilder table, String instanceHeader, Annotation annotation, int rowIndex,
		...
		int result = rowIndex;
		table.addTdValue(rowIndex, FIRST_COLUMN_INDEX, new HtmlValueHyperlinkBuilder(instanceHeader));
	...
	private void addAnnotationInformation(HtmlTableBuilder table, Annotation annotation, int rowIndex,
		...
		if (createdBy instanceof EmfUser) {
			...
			table.addTdValue(rowIndex, columnIndex++, new HtmlValueHyperlinkBuilder(hyperlink));
		}
	...
	private int fetchStatusCodelist() {
		DefinitionModel instanceDefinition = dictionaryService.getInstanceDefinition(new Annotation());
