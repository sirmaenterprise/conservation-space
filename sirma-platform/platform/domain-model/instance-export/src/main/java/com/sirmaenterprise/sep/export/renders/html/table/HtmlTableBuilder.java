...
public class HtmlTableBuilder implements HtmlBuilder {
	 ...
	 */
	private List<HtmlTrBuilder> tableTrs = new LinkedList<>();
	...
	/**
	 * Build row with title of table.
	 * 
	 * @param columnCount
	 *            - column count.
	 */
	private void buildTitleColumn(int columnCount) {
		if (StringUtils.isNotBlank(title)) {
			HtmlTrBuilder header = new HtmlTrBuilder();
			HtmlTdBuilder td = header.getTd(0);
			td.addValue(new HtmlValueTextBuilder(title));
			td.setColspan(columnCount);
			header.build(table, columnCount);
		}
	}
	...
	public static void addNoResultRow(HtmlTableBuilder htmlTableBuilder, String rowValue) {
		...
		htmlTableBuilder.addTdValue(htmlTableBuilder.getRowCount(), htmlTableBuilder.getColumnCount(), 1, htmlValueTextBuilder);
	}
	 ...
	 * @param tdIndex the td index
	 * @param style additional style to be added.
	 */
