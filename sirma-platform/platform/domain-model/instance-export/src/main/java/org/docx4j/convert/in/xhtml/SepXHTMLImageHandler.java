 ...
 */
public class SepXHTMLImageHandler implements XHTMLImageHandler {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(SepXHTMLImageHandler.class);
	 ...
	 */
	public SepXHTMLImageHandler(XHTMLImporterImpl importer, Function<String, byte[]> embeddedImageLoader) {
	...
	public void addImage(Docx4jUserAgent docx4jUserAgent, WordprocessingMLPackage wordMLPackage, P p, Element e,
		...
		try {
			...
			if (e.hasAttribute(EMBEDDED_ID)) {
				...
				imagePart = imagePartCache.get(embeddedId);
				if (imagePart != null) {
					// found embedded image in the cache -> get out
					addImageToDocument(p, e, cx, cy, imagePart);
					return;
				}
				// first time this image -> load data and set the cache key the embedded id
