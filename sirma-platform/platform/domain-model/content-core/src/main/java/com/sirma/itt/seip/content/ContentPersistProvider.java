...
class ContentPersistProvider {
	...
	static class ContentPersister {
		...
		protected ContentInfo uploadContentAndPersist(Serializable instance, Content content, ContentEntity entity,
			...
			try {
				...
				afterContentSave(instance, previousVersion.getPreviousVersion(), updatedView, isNew && !isNewVersion);
			} finally {
		...
		/**
		 * Called after content save was saved. The default implementation fires events that the content was
		 * added/updated
		 *
		 * @param instance
		 *            the target instance
		 ...
		 */
		protected void afterContentSave(Serializable instance, ContentInfo previousVersion, Content content,
				boolean isNew) {
			if (isNew) {
				eventService.fire(new ContentAddEvent(instance, content));
			} else {
				eventService.fire(new ContentUpdatedEvent(instance, content, previousVersion));
			}
		}
		...
		protected Collection<ContentInfo> toContentInfo(List<ContentEntity> entities) {
			return entities.stream().map(info -> toContentInfo(info)).collect(Collectors.toList());
		}
		...
		protected ContentInfo toContentInfo(ContentEntity info) {
			return toContentInfo(info, (contentStore, storeInfo) -> contentStore.getReadChannel(storeInfo));
		}
		...
		protected Supplier<ContentMetadata> getContentMetadataProvider(ContentEntity entity) {
			...
			return () -> {
				...
				if (storeInfo.isModified()) {
					...
					dao.persistEntity(entity, false);
				}
				return metadata;
			};
		}
	...
	static class ViewPersister extends ContentPersister {
		...
		@Override
		protected void afterContentSave(Serializable instance, ContentInfo previousVersion, Content updatedView,
				boolean isNew) {
			if (isNew) {
	...
	private static class ContentInfoProxy implements ContentInfo {
		...
		private final Supplier<FileDescriptor> descriptorSupplier;
		...
		private final Supplier<ContentMetadata> metadataResolver;
	...
	static class PreviousVersion {
		...
		private final ContentInfo previousVersion;
