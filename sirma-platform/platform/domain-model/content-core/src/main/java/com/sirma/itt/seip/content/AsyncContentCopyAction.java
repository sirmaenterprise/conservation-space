...
public class AsyncContentCopyAction extends SchedulerActionAdapter {
	...
	public void execute(SchedulerContext context) throws Exception {
		...
		ContentStore contentStore = storeProvider.getStore(sourceInfo.getRemoteSourceName());
		if (contentStore == null) {
			// this will also trigger a retry but probably will not recover from it
			throw new EmfRuntimeException(
					MessageFormat.format("Target content store {0} not found!", sourceInfo.getRemoteSourceName()));
		}
