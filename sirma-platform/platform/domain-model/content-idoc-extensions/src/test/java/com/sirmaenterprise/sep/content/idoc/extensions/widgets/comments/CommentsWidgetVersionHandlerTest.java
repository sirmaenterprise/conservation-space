package com.sirmaenterprise.sep.content.idoc.extensions.widgets.comments;
...
import com.sirmaenterprise.sep.content.idoc.nodes.widgets.comments.CommentsWidget;
import com.sirmaenterprise.sep.content.idoc.nodes.widgets.comments.CommentsWidgetConfiguration;
...
public class CommentsWidgetVersionHandlerTest {
	...
	public void processResults_withoutDateRagne() {
		...
		handler.processResults(widget, Optional.of(buildSearchResults(new Object())), new Date());
	...
	public void processResults_withDateRagne_withoutAfterDate() {
		...
		node.attr("config", "e30=");
		CommentsWidget widget = new CommentsWidget(node);
		handler.processResults(widget, Optional.of(buildSearchResults(new DateRange(null, new Date()))), new Date());
		CommentsWidgetConfiguration configuration = widget.getConfiguration();
		assertNotNull(configuration);
		assertEquals(4, configuration.getSelectedObjects().size());
		JsonObject filterCriteria = configuration.getFilterCriteria();
	...
	public void processResults_withDateRagne_withBothaDates() {
		...
		Optional<Object> searchResults = Optional.of(buildSearchResults(new DateRange(new Date(), new Date())));
		handler.processResults(widget, searchResults, new Date());
		CommentsWidgetConfiguration configuration = widget.getConfiguration();
		assertNotNull(configuration);
		assertEquals(4, configuration.getSelectedObjects().size());
		JsonObject filterCriteria = configuration.getFilterCriteria();
