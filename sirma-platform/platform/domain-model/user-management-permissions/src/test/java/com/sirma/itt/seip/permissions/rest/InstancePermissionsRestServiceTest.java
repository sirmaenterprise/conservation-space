...
import com.sirma.itt.seip.permissions.role.AuthorityRoleAssignment;
...
public class InstancePermissionsRestServiceTest {
	...
	public void savePermissions() throws Exception {
		...
		when(instanceAccessEvaluator.isAtLeastRole(any(InstanceReference.class),
				eq(SecurityModel.BaseRoles.COLLABORATOR)))
		...
		entityPermission.setInheritFromLibrary(false);

		entityPermission.getAssignments().add(new AuthorityRoleAssignment("admin", "manager"));
		entityPermission.getAssignments().add(new AuthorityRoleAssignment("regular_user", "collaborator"));

		when(entityPermissionDao.loadWithAssignments(INSTANCE_ID)).thenReturn(Optional.of(entityPermission));
	...
	private void mockExistingInstance(String instanceId) {
		...
		InstanceReferenceMock referenceMock = new InstanceReferenceMock(instanceId, mock(DataTypeDefinition.class),
				instance);
		referenceMock.setType(instanceType);
		instance.setReference(referenceMock);
