...
public class ResourceEntity extends BaseStringIdEntity implements Identity {
	...
	static final String CHECK_IF_RESOURCE_EXISTS = "select count(p.id) from ResourceEntity p where p.identifier = :identifier or p.id = :identifier";
	...
	static final String QUERY_RESOURCES_BY_NAMES_AND_TYPE = "select p from ResourceEntity p where p.identifier in (:identifier) and p.type=:type";
