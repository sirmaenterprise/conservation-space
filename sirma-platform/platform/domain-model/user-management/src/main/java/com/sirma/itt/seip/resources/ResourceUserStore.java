...
public class ResourceUserStore implements UserStore {
	...
	public User loadByIdentityId(String identity, String tenantId) {
		...
		if (resource == null) {
			...
			// try to search the resource from some of the providers and if
			// found - add it
			// to emf
			resource = findUserUsingProviders(identity);
		}
	...
	// REVIEW: change people retrieval to be from LDAP directly not via DMS
	private Resource findUserUsingProviders(String userId) {
		for (ResourceProviderExtension nextProvider : resourceProviders) {
			if (nextProvider.isApplicable(ResourceType.USER)) {
				...
				if (synchedUser instanceof UserWithCredentials) {
					return synchedUser;
				}
			}
		}
