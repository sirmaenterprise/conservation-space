...
class ResourceStore {
	...
	@CacheConfiguration(eviction = @Eviction(maxEntries = 100) , expiration = @Expiration(maxIdle = 1800000, interval = 60000) , doc = @Documentation(""
	...
	private Plugins<ResourceProviderExtension> resourceProviders;
	@Inject
	...
	protected List<Resource> getAllResourcesReadOnly(ResourceType resourceType) {
		return allResources.computeIfAbsent(resourceType, type -> loadAllResources(type));
	}
	...
	protected Resource updateResource(Resource resource) {
		...
		entity.setActive(Boolean.valueOf(resource.isActive()));
		if (resource.getIdentifier() == null) {
			DefinitionModel model = dictionaryService.getInstanceDefinition(resource);
			if (model != null) {
	...
	protected Resource importResource(Resource resource) {
		// initialize
		OperationExecutedEvent event = new OperationExecutedEvent(SYNCHRONIZE, resource);
	 ...
	 * @param operation
	 *            the operation that triggered the new user - create/synchronize
	 * @return the updated resource
	 */
	protected Resource persistNewResource(Resource resource, Operation operation) {

		if (resource.getId() == null) {
			String resourceDbId = entityDao.generateResourceDbId(resource.getName());
	...
	private void addDefaultProperties(Resource resource) {
		DefinitionModel definition = dictionaryService.getInstanceDefinition(resource);
		if (definition != null) {
	...
	protected boolean resourceExists(Serializable resourceId) {
		if (StringUtils.isNullOrEmpty(Objects.toString(resourceId, null))) {
			return false;
		}
		return entityDao.resourceExists(resourceId);
	}
	 ...
	 *
	 * @param entity
	 *            the entity that represents a valid resource {@link Resource} or {@link ResourceEntity}
	 */
	...
	private void setDefinitionId(Resource resource) {
		if (resource.getIdentifier() == null) {
			String definitionId = dictionaryService.getDefaultDefinitionId(resource);
	...
	protected class ResourceLookupDao extends BaseEntityLookupDao<ResourceEntity, Pair<String, ResourceType>, String> {
		...
		protected Pair<String, ResourceType> getValueKeyInternal(ResourceEntity value) {
			...
			return new Pair<>(value.getIdentifier(), ResourceType.getById(value.getType()));
		}

		@Override
		public Pair<String, ResourceEntity> createValue(ResourceEntity value) {
			/*
			 * if the id is registered but we came here means the resource is created in the same transaction when
			 * searched and not found. No need to continue persisting. We better return the resource if generated id
			 */
			if (value.getId() == null) {
				value.setId(entityDao.generateResourceDbId(value.getIdentifier()));
				if (idManager.isIdRegistered(value.getId())) {
					return new Pair<>(value.getId(), value);
				}
				idManager.register(value);
			} else if (idManager.isIdRegistered(value.getId())) {
				return new Pair<>(value.getId(), value);
			}
			Pair<String, ResourceEntity> pair = super.createValue(value);
			if (pair != null) {
				eventService.fire(new ResourcePersistedEvent(convertToResource(pair.getSecond()), null, null));
			}
			return pair;
		}
		...
		protected List<ResourceEntity> fetchEntityByValue(Pair<String, ResourceType> key) {
			...
			List<ResourceEntity> list = entityDao.findResourcesByNameAndType(Arrays.asList(identifier), type);
			if (isNotEmpty(list)) {
				return list;
			}
			List<ResourceEntity> newResources = resourceProviders
					.select(extension -> extension.isApplicable(type))
						.map(extension -> extension.getResource(identifier))
						.map(newResource -> importResource(newResource))
						.map(resource -> convertToEntity(resource))
						.map(Arrays::asList)
