 ...
 * @author BBonev
 */
public abstract class BaseDmsToSeipResourceSynchronizationConfig
	...
	protected abstract SynchronizationProvider<Collection<? extends Resource>> loadLocalResources();
	...
	protected Resource copyResource(Resource resource) {
		if (resource instanceof GenericProxy<?>) {
			return (Resource) ((GenericProxy<?>) resource).clone();
		}
		return resource;
	}
	 ...
	 */
	@SuppressWarnings("squid:S1452")
	protected abstract SynchronizationProvider<Collection<? extends Resource>> loadRemoteResources();
	...
	public Resource merge(Resource oldValue, Resource newValue) {
		...
		resource.addAllProperties(newValue.getProperties());
	...
	/**
	 * {@inheritDoc}<br>
	 * Comparator that uses custom hash functions provided via {@link HashCalculator}
	 */
	@Override
	public BiPredicate<Resource, Resource> getComparator() {
		return (newValue, currentValue) -> {
			// cleans the null properties from the new value where not present in the current entry
			removeNotPresentProperties(newValue, currentValue);
			return hashCalculator.equalsByHash(newValue, currentValue);
		};
	}

	private static void removeNotPresentProperties(Resource newValue, Resource currentValue) {
		for (Iterator<Entry<String, Serializable>> it = newValue.getProperties().entrySet().iterator(); it.hasNext();) {
			Entry<String, Serializable> entry = it.next();
			if (entry.getValue() == null && !currentValue.isPropertyPresent(entry.getKey())) {
				it.remove();
			}
		}
	}
	 ...
	 *
	 * @param newResources
	 *            map of new resources which should be saved
	 */
