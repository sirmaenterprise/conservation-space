...
public class FavouriteServiceImplTest {
	...
	public void setup() {
		...
		when(typeConverter.convert(eq(InstanceReference.class), any(EmfUser.class))).then(a -> {
			...
			return new LinkSourceId(instance.getId().toString(), new DataTypeDefinitionMock(instance), instance);
		});
	...
	private EmfInstance prepareInstance() {
		...
		ReflectionUtils.setField(instance, "reference",
				new LinkSourceId(instance.getId().toString(), new DataTypeDefinitionMock(instance), instance));
		LinkReference linkReference = new LinkReference();
