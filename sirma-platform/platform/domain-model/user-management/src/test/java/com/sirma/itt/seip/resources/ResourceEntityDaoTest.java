...
public class ResourceEntityDaoTest {
	...
	public void testAddMembers() {
		...
		assertFalse(entityDao.addMembers("emf:group", null));
		assertFalse(entityDao.addMembers(null, new HashSet<>(Arrays.asList("emf:newMember"))));

		when(dbDao.fetchWithNamed(eq(GroupMembershipEntity.GET_ALL_MEMBERS_IDS_KEY), anyList()))
				.thenReturn(Arrays.asList("emf:oldMember"));
		assertTrue(entityDao.addMembers("emf:group", new HashSet<>(Arrays.asList("emf:newMember", "emf:oldMember"))));
		verify(dbDao).saveOrUpdate(new GroupMembershipEntity("emf:group", "emf:newMember"));
	}

	@Test
	public void testRemoveMembers() {
		assertFalse(entityDao.removeMembers("emf:groupId", null));
		assertFalse(entityDao.removeMembers(null, null));
		...
		entityDao.removeMembers("emf:groupId", members);
	...
	public void testGenerateResourceDbId() {
		...
		String resourceDbId = entityDao.generateResourceDbId(value.getIdentifier());
