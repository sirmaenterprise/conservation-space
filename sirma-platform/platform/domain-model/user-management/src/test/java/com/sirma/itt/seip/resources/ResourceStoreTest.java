...
public class ResourceStoreTest {
	...
	@Mock
	private ResourceProviderExtension resourceProviderExtension;

	private List<ResourceProviderExtension> plugins = new ArrayList<>();
	@Spy
	private Plugins<ResourceProviderExtension> resourceProviders = new Plugins<>("", plugins);
	...
	public void beforeMethod() {
		...
		plugins.clear();
		plugins.add(resourceProviderExtension);

		when(entityDao.generateResourceDbId(any())).then(a -> "emf:" + a.getArgumentAt(0, String.class));
		...
		when(dictionaryService.getDefaultDefinitionId(any(Resource.class))).thenReturn("userDefinition");
	}
	...
	public void updateResource_notFound() throws Exception {
		ResourceEntity entity = createEntity("test", USER);
		Resource resource = resourceStore.updateResource(mapper.map(entity, EmfUser.class));
	}
	...
	@Test
	public void fileResourceFromExtension() throws Exception {
		when(resourceProviderExtension.isApplicable(USER)).thenReturn(Boolean.TRUE);
		Resource value = mapper.map(createEntity("user1", USER), EmfUser.class);
		value.setId(null);
		when(resourceProviderExtension.getResource("user1")).thenReturn(value);
