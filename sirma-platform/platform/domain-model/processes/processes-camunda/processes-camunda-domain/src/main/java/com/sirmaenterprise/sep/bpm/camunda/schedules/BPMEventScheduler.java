...
public class BPMEventScheduler extends SchedulerActionAdapter {
	...
	public void execute(SchedulerContext context) throws Exception {
		...
		if (CreateOrUpdateRequest.OPERATION_NAME.equalsIgnoreCase(serverAction) && ActionTypeConstants.CREATE.equalsIgnoreCase(userAction)
			...
			for (LinkReference linkReference : links) {
				...
				if (targetDefId.equalsIgnoreCase(instance.getIdentifier())) {
					notifyCamundaCatchEvent(executionId, eventName, schedulerEntry);
					break;
				}
			}
		} else {
	...
	private void notifyCamundaCatchEvent(String executionId, String eventName, SchedulerEntry schedulerEntry) {
		...
		for (EventSubscription eventSubscription : eventSubscriptions) {
			...
			if ("signal".equalsIgnoreCase(eventType)) {
				runtimeService.signalEventReceived(eventSubscription.getEventName(), executionId);
				schedulerEntry.getConfiguration().setRemoveOnSuccess(true);
			} else if ("message".equalsIgnoreCase(eventType)) {
				runtimeService.messageEventReceived(eventSubscription.getEventName(), executionId);
				schedulerEntry.getConfiguration().setRemoveOnSuccess(true);
			}
		}
	}
	 ...
	 *
	 * @param event
	 * 		the {@link MultiEngineBusinessProcessEvent} from which to take camunda extension properties and add them to new {@link SchedulerContext}
	...
	public static SchedulerContext createExecutorContext(MultiEngineBusinessProcessEvent event) {
		...
		if (StringUtils.isNotBlank(eventName)) {
			...
			if (camundaProperties == null || camundaProperties.getCamundaProperties() == null) {
				throw new CamundaIntegrationRuntimeException(
						"Can not create ScheduleContext, because of missing properties for event: " + event);
			}
		...
		throw new CamundaIntegrationRuntimeException(
				"Can not create ScheduleContext, because of missing event name for event: " + event);
	}

	/**
	 * Creates the {@link EmfEvent} that will be scheduled by {@link SchedulerService}.
	 *
	 * @param context the {@link SchedulerContext} with {@link BPMEventScheduler#USER_ACTION} and {@link BPMEventScheduler#SERVER_ACTION}
	 * @param contextInstance the context instance to observe for
	...
	public static EmfEvent createConfigurationEvent(SchedulerContext context, Instance contextInstance) {
		String userAction = context.getIfSameType(USER_ACTION, String.class);
		String serverAction = context.getIfSameType(SERVER_ACTION, String.class);
		if (CreateOrUpdateRequest.OPERATION_NAME.equalsIgnoreCase(serverAction) && ActionTypeConstants.CREATE.equalsIgnoreCase(
				userAction)) {
	 ...
	 *
	 * @param context the current {@link SchedulerContext} with properties filled from {@link MultiEngineBusinessProcessEvent}
	 * @param event the {@link MultiEngineBusinessProcessEvent}
	...
	public Collection<Instance> findContextInstances(SchedulerContext context, MultiEngineBusinessProcessEvent event) {
		...
		throw new CamundaIntegrationRuntimeException("Can not find context instance for event: " + event);
	}
}
