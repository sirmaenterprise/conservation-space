...
public class BPMTransitionDefinitionModelRestServiceTest {
	...
	public void testGetInstanceDefinitionModelForMultipleActivities() throws Exception {
		...
		when(dictionaryService.getInstanceDefinition(eq(newActivity1))).thenReturn(model1);
		when(dictionaryService.getInstanceDefinition(eq(newActivity2))).thenReturn(model2);
		when(dictionaryService.getInstanceDefinition(eq(instance))).thenReturn(instanceModel);
