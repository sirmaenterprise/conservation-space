...
public class SepRuntimeCamundaContainerDelegate extends RuntimeContainerDelegateImpl {
	...
	public void deployProcessApplication(AbstractProcessApplication processApplication) {
		...
		allTenantsInfo.forEach(tenantInfo -> deployEngine(processApplication, tenantInfo.getTenantId()));
		completeInitialization(processApplication);

	}

	private void completeInitialization(AbstractProcessApplication processApplication) {
		if (!isDeployed) {
			tenantManager
					.addOnTenantAddedListener(tenantInfo -> deployEngine(processApplication, tenantInfo.getTenantId()));
			tenantManager.addOnTenantRemoveListener(
					tenantInfo -> undeployEngine(processApplication, tenantInfo.getTenantId()));
			isDeployed = true;
		}
	}
	...
	public void undeployProcessApplication(AbstractProcessApplication processApplication) {
		Stream<TenantInfo> allTenantsInfo = tenantManager.getActiveTenantsInfo(false);
		allTenantsInfo.forEach(tenantInfo -> undeployEngine(processApplication, tenantInfo.getTenantId()));
	}

	private void deployEngine(final AbstractProcessApplication processApplication, final String processAppName) {
		try {
			securityContextManager.initializeTenantContext(processAppName);
			// invoke in tx in the new thread context
			transactionSupport.invokeInNewTx(() -> deployEngineInternal(processApplication, processAppName));
		} catch (Exception e) {
	...
	protected void deployEngineInternal(AbstractProcessApplication processApplication, String processAppName) {
		ensureNotNull("Process application", processApplication);
	...
	private void undeployEngine(final AbstractProcessApplication processApplication, final String processAppName) {
		ensureNotNull("Process application", processApplication);
		try {
			securityContextManager.initializeTenantContext(processAppName);
			transactionSupport.invokeInNewTx(() -> undeployEngineInternal(processApplication, processAppName));
		} finally {
			securityContextManager.endContextExecution();
		}
	}

	protected void undeployEngineInternal(AbstractProcessApplication processApplication, String processAppName) {
		String engineName = camundaConfiguration.getEngineName().get();
