...
public abstract class BaseRoleEvaluator<E> implements RoleEvaluator<E> {
	...
	protected static final Set<Action> NO_ACTIONS = new HashSet<>(5);

	protected static final RoleIdentifier[] ROLE_IDENTIFIERS_EMPTY_ARRAY = new RoleIdentifier[0];
	...
	public Pair<Role, RoleEvaluator<E>> evaluate(E target, Resource resource, RoleEvaluatorRuntimeSettings settings) {
		...
		Pair<Role, RoleEvaluator<E>> highestRole = calculateHighestRoleFromChain(target, resource, role, settings,
				getPreferredHighPriorityRoles());
		return highestRole;
	}

	/**
	 * Checks if the given user has permissions to unlock given instance. The method checks, if the user is admin,
	 * manager, if the instance is locked by this user or if the user is the instance creator.
	 *
	 * @param instance
	 *            the instance for which the checks are performed
	 * @param resource
	 *            the resource which permissions are checked
	 * @return <b>TRUE</b> if the instance is locked by the user, or the user is admin, or the user is manager, or the
	 *         user is creator of the instance, <b>FALSE</b> otherwise
	 */
	protected boolean hasUnlockPermissions(Instance instance, Resource resource) {
		return resourceService.areEqual(instance.get(LOCKED_BY), resource) || isAdminOrSystemUser(resource)
				|| isRootManager(instance, resource) || resourceService.areEqual(instance.get(CREATED_BY), resource);
	}
	...
	protected Set<Action> getAllowedActions(Instance target, Resource authority, Role localRole) {
		...
		RoleActionEvaluatorContext context = buildContext(target, localRole, authority);
		Set<Action> allowedActions = transitionManager.getAllowedActions(target, getInstanceStatus(target),
				localRole.getAllowedActions(context));
		context = null;
