...
public class CopyObjectPropertiesOnRevertStep implements RevertStep {
	...
	public void invoke(RevertContext context) {
		...
		Serializable currentId = currentInstance.getId();
		DefinitionModel definition = dictionaryService.getInstanceDefinition(currentInstance);
		if (definition == null) {
			throw new EmfRuntimeException("Missing definition for instance - " + currentId);
		}
