...
public class SyncThumbnailPublishStep extends SchedulerActionAdapter implements PublishStep {
	...
	public void execute(PublishContext publishContext) {
		...
		String thumbnail = renditionService.getThumbnail(thumbnailSource.getIdentifier());
		if (isValidThumbnail(thumbnail)) {
			thumbnailService.addThumbnail(revision, thumbnail);
			LOGGER.debug("Assigned thumbnail {} to revision {}", thumbnailSource.getIdentifier(),
					revision.getIdentifier());
		} else {
			LOGGER.debug("The thumbnail of {} is not fetched, yet. Will try to fetch it later for {}",
					thumbnailSource.getIdentifier(), revision.getIdentifier());
			scheduleAsyncThumbnailCheck(thumbnailSource.getIdentifier(), revision.getIdentifier());
		}
	}
	...
	private void scheduleAsyncThumbnailCheck(String thumbnailSource, String thumbnailTarget) {
		...
		SchedulerConfiguration configuration = schedulerService
				.buildEmptyConfiguration(SchedulerEntryType.TIMED)
					...
					.setMaxRetryCount(50)
					.setRetryDelay(60L)
					.setSynchronous(false).setScheduleTime(calendar.getTime());
	...
	public void execute(SchedulerContext context) throws Exception {
		...
		InstanceReference revision = instanceTypeResolver
				.resolveReference(revisionid)
					.orElseThrow(() -> new SchedulerRetryException("Revision " + revisionid + " does not exist, yet"));
		thumbnailService.addThumbnail(revision, thumbnail);
