...
public abstract class AbstractInstanceToInstanceReferenceConverterProvider implements TypeConverterProvider {
	...
	public class EntityInstanceToLinkSourceConverter<E extends Instance, I extends InstanceReference>
		...
		public I convert(E source) {
			InstanceType type = instanceTypes.from(source).orElse(null);
			LinkSourceId reference = new LinkSourceId(String.valueOf(source.getId()), getType(targetClass), type,
					source);
			if (source instanceof OwnedModel) {
				reference.setParent(((OwnedModel) source).getOwningReference());
			}
			return (I) reference;
		}
	}
	...
	protected DataTypeDefinition getType(Class<?> clazz) {
		DataTypeDefinition typeDefinition = dictionaryService.getDataTypeDefinition(clazz.getName());
		if (typeDefinition == null) {
