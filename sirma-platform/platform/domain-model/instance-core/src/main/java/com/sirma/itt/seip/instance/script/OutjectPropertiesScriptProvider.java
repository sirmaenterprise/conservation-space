...
/**
 * Extension which provides functionality for outjecting(coping) properties from source instance to destination
 * instance.
 *
 * @author A. Kunchev
 */
...
public class OutjectPropertiesScriptProvider implements GlobalBindingsExtension {
	...
	public Map<String, Object> getBindings() {
		return Collections.<String, Object>singletonMap(OUTJECT, this);
	}
	 ...
	 *
	 * @param source
	 *            the source ScriptNode object, which contains the source instance from which the properties will be
	 *            copied
	 * @param destination
	 *            the destination ScriptNode object, which contains the destination instance to which the properties
	 ...
	 *            the properties, which should be copied as string in JSON format (the example above)
	...
	public boolean outjectProperties(ScriptNode source, ScriptNode destination, String propertiesObject) {
		...
		instanceContextInitializer.restoreHierarchy(current);
		...
		List<String> outject = fetchAsListOfStrings(propertiesJson, OUTJECT);
		List<String> outjectNotEmpty = fetchAsListOfStrings(propertiesJson, OUTJECT_NOT_EMPTY);
	 ...
	 */
	private List<String> fetchAsListOfStrings(JsonObject json, String arraysKey) {
	 ...
	 *
	 * @param current
	 *            the instance from which the properties values are get
	 * @param parent
	 *            the parent instance to which the properties are add
	 * @param outjectNotEmpty
	 *            array with the properties which should be transfered, if they are not null or empty
	 */
