 ...
 * @author BBonev
 */
@Extension(target = InstanceTypeResolver.TARGET_NAME, enabled = true, order = 10)
public class SolrInstanceTypeResolver implements InstanceTypeResolver {
	...
	public Map<Serializable, InstanceType> resolve(Collection<Serializable> ids) {
		...
		return list.stream()
					...
					.map(instance -> resolveInstanceType(instance))
					.filter(instance -> instance.type() != null)
					.collect(Collectors.toMap(Instance::getId, Instance::type));
	}

	@Override
	public Collection<InstanceReference> resolveReferences(Collection<? extends Serializable> ids) {
		if (CollectionUtils.isEmpty(ids)) {
		...
		return CollectionUtils.transformToList(list, inst -> toReference(inst));
	}
	...
	@Override
	public Collection<Instance> resolveInstances(Collection<? extends Serializable> ids) {
		if (CollectionUtils.isEmpty(ids)) {
			return Collections.emptyList();
		}
		...
		list.forEach(this::resolveInstanceType);
		return BatchEntityLoader.loadDistinctInstances(list, serviceRegistry, executor);
	}
	...
	private static Collection<Serializable> filter(Collection<? extends Serializable> ids) {
	...
	private Supplier<List<Instance>> resolve(Supplier<String> querySupplier) {
		...
		return () -> {
			...
			if (StringUtils.isNullOrEmpty(arguments.getStringQuery())) {
				// query is not valid
				return Collections.emptyList();
			}
	...
	private Supplier<String> buildQuery(Collection<? extends Serializable> ids) {
		return () -> {
			...
			if (StringUtils.isNullOrEmpty(idQuery)) {
