 ...
 * @author BBonev
 */
@InstanceType(type = ScriptInstance.TYPE)
public class ScriptNode implements ScriptInstance {
	...
	public ScriptNode[] getLinks(String linkId) {
		...
		return linkReferences.stream()
				.map(linkReference -> linkReference.getTo().toInstance())
				.map(instance -> toScriptNode(instance))
	...
	public ScriptNode[] getLinksTo(String linkId) {
		...
		return linkReferences.stream()
				.map(linkReference -> linkReference.getFrom().toInstance())
				.map(instance -> toScriptNode(instance))
				.toArray(ScriptNode[]::new);
	}
	...
	public boolean linkToReference(InstanceReference to, String linkId, String reverseLinkId) {
		if (to == null || StringUtils.isNullOrEmpty(linkId) && StringUtils.isNullOrEmpty(reverseLinkId)) {
			return false;
		}
		Map<String, Serializable> properties = new HashMap<>(5);
		properties.put(DefaultProperties.CREATED_ON, new Date());
		properties.put(DefaultProperties.CREATED_BY, securityContext.getAuthenticated());
		Pair<Serializable, Serializable> pair = linkService.link(getTarget().toReference(), to, linkId, reverseLinkId,
				properties);
	...
	/**
	 * Resolve the given path into instance.
	 *
	 * @param path
	 *            the path
	 * @return the script node
	 */
	public ScriptNode resolvePath(String path) {
		return null;
	}
	...
	public ScriptNode getParent() {
		if (target instanceof OwnedModel) {
			return toScriptNode(((OwnedModel) target).getOwningInstance());
		}
		return null;
	}
	...
	public ScriptNode getParent(String type) {
		DataTypeDefinition dataTypeDefinition = typeProvider.getDataType(type);
		Instance parent = Instance.class.cast(InstanceUtil.getParent(dataTypeDefinition.getJavaClass(), target));
		if (parent == null) {
			return null;
		}
	...
	public boolean is(String value) {
		if (target == null || target.type() == null || StringUtils.isNullOrEmpty(value)) {
			return false;
		}
	...
	public String[] getArray(String property) {
		Predicate<Serializable> isString = value -> value instanceof String;
