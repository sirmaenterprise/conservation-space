...
public class PropertiesServiceImpl implements PropertiesService {
	 ...
	 *            the id manager
	 * @param taskExecutor
	 *            used for executing asynchronous operations
	 * @param serviceRegistry
	 *            used to resolve and get correct instance service
	 * @param mapper
	 *            used to transfer mapped object properties
	 */
	public PropertiesServiceImpl(PropertiesStorageAccess storageAccess, PropertiesDao propertiesDao,
			PropertyModelCallback<PropertyModel> defaultInstanceCallback,
			javax.enterprise.inject.Instance<PropertyModelCallback<? extends PropertyModel>> callbacks,
			Instance<InstanceDao> instanceDao, DatabaseIdManager idManager, TaskExecutor taskExecutor,
			ServiceRegistry serviceRegistry, ObjectMapper mapper) { // NOSONAR
	...
	public void saveProperties(Entity entity, PathElement path, Map<String, ? extends Serializable> properties,
		...
		propertiesDao.saveProperties(entity, path, properties, addOnly, getCallback(null, entity, true, true), storageAccess);
	}
	...
	private <E extends PropertyModel> void loadPropertiesInternal(List<E> instances, boolean loadAll) {
		...
		if (TRACE) {
			...
			LOGGER.trace("Loading properties for " + instances.get(0).getClass().getSimpleName() + " (" + list.size()
					+ "): " + list);
		}
	...
	public <I extends com.sirma.itt.seip.domain.instance.Instance> void loadPropertiesBatch(Collection<I> instances) {
		Map<Pair<Object, Serializable>, com.sirma.itt.seip.domain.instance.Instance> mapping = BatchEntityLoader
				.loadInstancesAsMapping(instances, serviceRegistry, taskExecutor);
		for (I instance : instances) {
			com.sirma.itt.seip.domain.instance.Instance loaded = mapping
					.get(BatchEntityLoader.instanceToPair(instance));
			if (loaded != null) {
	...
	private <I extends com.sirma.itt.seip.domain.instance.Instance> void syncInstances(I instance,
		...
		if (StringUtils.isNullOrEmpty(instance.getIdentifier())) {
			if (instance instanceof OwnedModel) {
				...
				ownedModel.setOwningReference(((OwnedModel) loaded).getOwningReference());
			}
	...
	protected <E extends PropertyModel> PropertyModelCallback<E> getCallback(E instance, Entity<?> entity,
