...
public class ObjectRoleEvaluatorTest extends EmfTest {
	...
	public void testEvaluateInternal_stateDeleted() {
		...
		objectInstance.getProperties().put(DefaultProperties.STATUS, PrimaryStates.DELETED_KEY);

		Mockito.when(stateService.isInStates(objectInstance, PrimaryStates.DELETED)).thenReturn(true);
	...
	public void testEvaluateInternal_stateIsInStates() {
		...
		Mockito.when(stateService.isInStates(objectInstance, states)).thenReturn(true);
		...
		Mockito
				.when(permissionService.getPermissionAssignment(objectInstance.toReference(), createResource.getId()))
					.thenReturn(mock);
		Mockito.when(registry.find(BaseRoles.CONSUMER)).thenReturn(role);

		Pair<Role, RoleEvaluator<ObjectInstance>> actual = objectEvaluator.evaluate(objectInstance, createResource,
				null);
	...
	public void testFilterInternal_removeAllAfterConditions() {
		...
		PrimaryStates[] states = new PrimaryStates[] { PrimaryStates.DELETED, PrimaryStates.COMPLETED,
				PrimaryStates.STOPPED };
		EmfInstance ci = new EmfInstance();
		ci.setId("emf:user");
		ci.setReference(new InstanceReferenceMock(ci));
	...
	private static ObjectInstance createInstance() {
		...
		objectInstance
				.setReference(new InstanceReferenceMock("object", mock(DataTypeDefinition.class), objectInstance));
	...
	private static Set<Action> getActions() {
		Set<Action> listOfActions = new LinkedHashSet<>(
				Arrays.asList(ObjectRoleEvaluator.EDIT_DETAILS, ObjectRoleEvaluator.DELETE, ObjectRoleEvaluator.UNLOCK,
