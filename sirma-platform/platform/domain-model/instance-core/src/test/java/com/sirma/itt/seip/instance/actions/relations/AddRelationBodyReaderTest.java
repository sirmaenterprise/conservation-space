 ...
 * @author BBonev
 */
public class AddRelationBodyReaderTest {

	@InjectMocks
	private AddRelationBodyReader bodyReader;
	@Mock
	...
	public void isReadable_wrongType() {
		assertFalse(bodyReader.isReadable(RemoveRelationRequest.class, null, null, null));
	}

	@Test
	public void isReadable_AddRelationRequestForType() {
		assertTrue(bodyReader.isReadable(AddRelationRequest.class, null, null, null));
	}
	...
	public void readRequest_singleRelation() throws Exception {
		AddRelationRequest request = bodyReader.readFrom(null, null, null, null, null,
				getResourceStream("addRelation.json"));
	...
	public void readRequest_multipleRelation() throws Exception {
		AddRelationRequest request = bodyReader.readFrom(null, null, null, null, null,
				getResourceStream("addTwoRelations.json"));
		assertNotNull(request);
		assertEquals("addParent", request.getUserOperation());
		assertNotNull(request.getRelations());
		assertFalse(request.getRelations().isEmpty());
		assertTrue(request.getRelations().containsKey("emf:addParent"));
	...
	private static InputStream getResourceStream(String string) {
		return AddRelationBodyReaderTest.class.getClassLoader().getResourceAsStream(
				AddRelationBodyReaderTest.class.getPackage().getName().replace('.', '/') + "/" + string);
	}
}
