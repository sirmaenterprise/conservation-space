...
import com.sirma.itt.seip.instance.security.RelationRoleEvaluator;
...
public class RelationRoleEvaluatorTest {
	...
	public void test_notPersistedSource_fromReference() {
		...
		EmfInstance instance = new EmfInstance();
		instance.setId("emf:source");
		linkInstance.setFrom(new LinkSourceId("emf:source", mock(DataTypeDefinition.class), instance));
	...
	public void test_sourceInstanceRole() {
		...
		EmfInstance instance = new EmfInstance();
		instance.setId("emf:source");
		InstanceReference reference = new InstanceReferenceMock("emf:source", mock(DataTypeDefinition.class), instance);
		...
		when(permissionService.getPermissionAssignment(reference, user.getId())).thenReturn(role);
		Role expectedDefaultRole = new Role(BaseRoles.MANAGER);

		RoleEvaluator<Instance> parentEvaluator = mock(RoleEvaluator.class);
		when(parentEvaluator.evaluate(instance, user, null)).thenReturn(new Pair<>(expectedDefaultRole, null));
