...
@NamedQueries({
		...
		@NamedQuery(name = InstanceEntity.QUERY_INSTANCE_ID_BY_INSTANCE_TYPES_AND_OWNING_INSTANCE_KEY, query = InstanceEntity.QUERY_INSTANCE_ID_BY_INSTANCE_TYPES_AND_OWNING_INSTANCE),
		@NamedQuery(name = InstanceEntity.QUERY_INSTANCE_ID_BY_INSTANCE_TYPE_AND_OWNING_INSTANCE_KEY, query = InstanceEntity.QUERY_INSTANCE_ID_BY_INSTANCE_TYPE_AND_OWNING_INSTANCE),
...
public class InstanceEntity extends BaseStringIdEntity {
	...
	static final String QUERY_INSTANCE_ENTITIES_BY_ID = "select c from InstanceEntity c left join fetch c.owningInstance where c.id in (:id)";

	/** Query {@link InstanceEntity} by dms id. */
	public static final String QUERY_INSTANCE_BY_DMS_KEY = "QUERY_INSTANCE_BY_DMS";
	static final String QUERY_INSTANCE_BY_DMS = "select t from InstanceEntity t left join fetch t.owningInstance where t.dmsId=:dmsId";
	...
	static final String QUERY_INSTANCE_ENTITIES_BY_DMS_ID = "select c from InstanceEntity c left join fetch c.owningInstance where c.dmsId in (:dmsId)";
	...
	static final String QUERY_INSTANCE_ENTITIES_BY_CM_ID = "select c from InstanceEntity c left join fetch c.owningInstance where c.cmId in (:cmId)";

	/**
	 * Query instance ids by instance type and owning instance id. Effectively fetches the children of particular type
	 * for given instance
	 */
	public static final String QUERY_INSTANCE_ID_BY_INSTANCE_TYPE_AND_OWNING_INSTANCE_KEY = "QUERY_INSTANCE_ID_BY_INSTANCE_TYPE_AND_OWNING_INSTANCE";
	static final String QUERY_INSTANCE_ID_BY_INSTANCE_TYPE_AND_OWNING_INSTANCE = "select ie.id from InstanceEntity ie left join fetch ie.owningInstance where ie.instanceType =:instanceType and ie.owningInstance.sourceId =:owningInstanceId";
	...
	static final String QUERY_INSTANCE_ID_BY_INSTANCE_TYPES_AND_OWNING_INSTANCE = "select ie.id from InstanceEntity ie left join fetch ie.owningInstance where ie.instanceType in (:instanceType) and ie.owningInstance.sourceId =:owningInstanceId";
	...
	@AttributeOverrides(value = {
			@AttributeOverride(name = "sourceId", column = @Column(name = "owninginstanceid", length = 128, nullable = true) ) })
