...
public interface LinkService {
	...
	/**
	 * Creates an association link of type parent between two instances in DMS. This is helpful method to link instances
	 * as context dependent of custom scope
	 *
	 * @param from
	 *            the link from element
	 * @param to
	...
	/**
	 * Removes an association link of type parent between two instances in DMS. The method will remove all associations
	 * between the provided instances that match the given association name if present or all if not.
	 *
	 * @param parent
	 *            the link from element
	 * @param child
	...
	/**
	 * Creates an association link of type parent between two instances in DMS and removed an existing one. This is
	 * helpful method to link instances as context dependent of custom scope
	 *
	 * @param from
	 *            the link from element
	 * @param to
	...
	/**
	 * Removes all links for the give instance. The method will remove the links where the given object is source or
	 * destination of the link.
	 *
	 * @param instance
	 *            the instance
	 * @return true, if removed at least one link. Will return <code>false</code> if the object was never part of a link
	 */
	 ...
	 * @param to
	 *            the to
	 * @return true, if removed at least one link. Will return <code>false</code> if the objects were never linked.
	 */
	...
	/**
	 * Gets all relations associated to the instance that are defined as object relations in the instance model
	 *
	 * @param instance
	 *            the reference
	 * @return the instance relations
	 */
	List<LinkReference> getInstanceRelations(Instance instance);
	...
	/**
	 * Gets the relations diff for the given instance and returns link references that need to be added and one that
	 * need to be removed. The method does not actually add or remove anything.
	 *
	 * @param source
	 *            the source instance to use as base for relations diff
	 * @param changes
	 *            the relation changes to save
	 * @return a pair that has as it's first value the relations that need to be added and as second the one that need
	 *         to be removed. If nothing is changed then both lists should be empty
	 */
	Pair<List<LinkReference>, List<LinkReference>> getRelationsDiff(InstanceReference source,
			Map<String, ? extends Collection<InstanceReference>> changes);

	/**
	 * Save relation changes. The methods adds the given relations and removes the other in a single operation. The
	 * created/removed relations are normal and not 'simple'. The arguments may be obtained from
	 * {@link #getRelationsDiff(InstanceReference, Map)} method. <br>
	 * When adding relations the inverse relation should be determined based on the model
	 *
	 * @param toAdd
	 ...
	 * @param onSuccessAdd
	 *            the on success add
	 * @param onSuccessRemove
	 *            the on success remove
	 */
	void saveRelations(List<LinkReference> toAdd, List<LinkReference> toRemove, Consumer<LinkReference> onSuccessAdd,
			Consumer<LinkReference> onSuccessRemove);
}
