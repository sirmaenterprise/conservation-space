...
class ThumbnailDao {
	...
	ThumbnailMappingEntity getOrCreateThumbnailMappingEntity(InstanceReference instance, String purpose) {
		...
		args.add(new Pair<String, Object>("id", instance.getIdentifier()));
		args.add(new Pair<String, Object>(PURPOSE, purpose));
		...
		if (list.isEmpty()) {
			...
			entity.setInstanceId(instance.getIdentifier());
	...
	<S extends Serializable> int scheduleThumbnailChecks(Collection<S> ids) {
		...
		args.add(new Pair<String, Object>("instanceId", ids));
		args.add(new Pair<String, Object>(PURPOSE, RenditionService.DEFAULT_PURPOSE));
		args.add(new Pair<String, Object>("thumbnail", ThumbnailService.MAX_RETRIES));
		args.add(new Pair<String, Object>("lastFailTimeThreshold", getOneHourBack().getTime()));
	...
	int deleteThumbnail(Serializable thumbnailId) {
		List<Pair<String, Object>> args = new ArrayList<>(1);
		args.add(new Pair<String, Object>("id", thumbnailId));
	...
	int updateThumbnailEntity(String id, Integer retries, String thumbnail, Date lastFailTime) {
		...
		args.add(new Pair<String, Object>("lastFailTime", lastFailTime));
	...
	List<Object[]> getThumbnailsForSynchronization(Integer maxRetryCount) {
		List<Pair<String, Object>> args = new ArrayList<>(2);
		args.add(new Pair<String, Object>("retries", maxRetryCount));
