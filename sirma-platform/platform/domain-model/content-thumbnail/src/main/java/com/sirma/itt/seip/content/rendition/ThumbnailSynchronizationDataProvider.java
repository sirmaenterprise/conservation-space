 ...
 */
@ApplicationScoped
public class ThumbnailSynchronizationDataProvider {

	private static final Logger LOGGER = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

	private static final long WARNIG_TIMEOUT = TimeUnit.HOURS.toMillis(1);
	...
	@Schedule(identifier = "ThumbnailSynchronization", transactionMode = TransactionMode.NOT_SUPPORTED)
	...
	public void synchThumbnails() {
		if (!thumbnailLoaderEnabled.get().booleanValue()) {
			return;
		}
		...
		if (!list.isEmpty()) {
			...
			} else if (last > 0 && System.currentTimeMillis() - last > WARNIG_TIMEOUT) {
				LOGGER.warn(
