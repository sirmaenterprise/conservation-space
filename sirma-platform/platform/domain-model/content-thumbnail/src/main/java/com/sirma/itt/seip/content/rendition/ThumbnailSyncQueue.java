...
public class ThumbnailSyncQueue {
	...
	@ConfigurationGroupDefinition(properties = { WORKER_THREADS, STORE_CAPACITY }, type = SyncQueueWorkerGroup.class)
	private static final String WORKER_GROUP = "thumbnail.loader.group";

	@Inject
	@Configuration(WORKER_GROUP)
	private ConfigurationProperty<SyncQueueWorkerGroup> workerGroup;
	...
	static SyncQueueWorkerGroup buildWorkerGroup(GroupConverterContext context,
		...
		return new SyncQueueWorkerGroup(capacity.intValue(), parallelism.intValue(), securityContextManager,
				thumbnailLoader);
	}
	...
	@PostConstruct
	void initQueue() {
		workerGroup.addValueDestroyListener(group -> group.stopWork());
		workerGroup.addConfigurationChangeListener(group -> group.get().startWorkers());
	}
	...
	/**
	 * This is internal method for starting the workers for all tenants
	 */
	@RunAsAllTenantAdmins
	@Startup(phase = StartupPhase.BEFORE_APP_START)
	void enableForAllTenants() {
		workerGroup.get().startWorkers();
	}
	 ...
	 */
	int size() {
		return workerGroup.get().getStore().size();
	}
	...
	private boolean scheduleForProcessing(String id, String endPoint, String providerName, Integer retries) {
		...
		if (!workerGroup.get().markForProcessing(endPoint)) {
	...
	private boolean addToQueue(ThumbnailSyncEntry entry) {
		SyncQueueWorkerGroup queueWorkerGroup = workerGroup.get();
		try {
	...
	private static class SyncQueueWorkerGroup {
		...
		private final List<ThumbnailSyncWorker> workers;
		 ...
		 *            the thumbnail loader
		 */
		SyncQueueWorkerGroup(int storeCapacity, int workersCount,
				SecurityContextManager securityContextManager, ThumbnailLoader thumbnailLoader) {
			...
			executorFactory = new CachingSupplier<>(() -> createExecutorService(workersCount, securityContextManager));
			workerFactory = () -> buildWorker(securityContextManager, waiting, thumbnailLoader);
		}
		...
		private ThumbnailSyncWorker buildWorker(SecurityContextManager securityManager, Set<String> waitingQueue,
				ThumbnailLoader loader) {
			return new ThumbnailSyncWorker(store, securityManager, entry -> loadThumbnail(entry, waitingQueue, loader),
		...
		public boolean equals(Object obj) {
			...
			if (storeCapacity != other.storeCapacity) {
				return false;
			}
			if (workersCount != other.workersCount) {
				return false;
			}
			return true;
		}
	...
	private static class ThumbnailSyncWorker extends SecureRunnable implements JsonRepresentable {
		 ...
		 */
		public ThumbnailSyncWorker(BlockingDeque<ThumbnailSyncEntry> storeRef,
	...
	private static class ThumbnailSyncEntry implements Serializable, JsonRepresentable {
		 ...
		 *            the retries
		 */
		public ThumbnailSyncEntry(String id, String endPoint, String providerName, Integer retries) {
			this.id = id;
