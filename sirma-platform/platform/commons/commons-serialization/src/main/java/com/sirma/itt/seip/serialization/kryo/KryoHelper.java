...
public class KryoHelper {
	...
	private final List<Pair<Class<?>, Integer>> classRegister = new LinkedList<>();
	...
	protected void registerClasses(Kryo kryo) {
		for (Pair<Class<?>, Integer> objects : classRegister) {
			...
			// several positions
			int registerId = objects.getSecond().intValue() + KRYO_ENGINE_CLASS_OFFSET;
	...
	public void initializeKryoRegister() {
		Set<Pair<Class<?>, Integer>> temp = new TreeSet<>((o1, o2) -> o1.getSecond().compareTo(o2.getSecond()));
		...
		classRegister.addAll(temp);

		// initialize the static instance that will be produced by CDI
		INSTANCE.classRegister.addAll(temp);
	}
	...
	public Integer getClassRegistration(Class<?> clazz) {
		...
		return getSerializationPool().run(kryo -> {
			...
			return registration == null ? null : Integer.valueOf(registration.getId());
		});
	}
	...
	public Class<?> getRegisteredClass(Integer registerId) {
		...
		return getSerializationPool().run(kryo -> {
			Registration registration = kryo.getClassResolver().getRegistration(registerId.intValue());
