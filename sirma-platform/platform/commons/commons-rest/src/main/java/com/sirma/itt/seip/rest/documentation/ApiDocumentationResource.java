...
public class ApiDocumentationResource {
	...
	private Swagger getSwagger(ServletConfig servletConfig, Application application, ServletContext context) {
		if (swagger == null) {
			synchronized (sync) {
				// we check if someone already initialized the value if so just use it
				if (swagger == null) {
