...
public final class JSON {
	 ...
	 *            Value to check.
	 * @param expected
	 *            Expected {@link ValueType}.
	 * @return {@code true} if the value is not {@code null} and it's type is the expected.
	 */
	public static boolean isType(JsonValue v, ValueType expected) {
	...
	public static boolean isBlank(JsonValue value) {
		...
		switch (value.getValueType()) {
			...
			case STRING:
				return StringUtils.isNullOrEmpty(((JsonString) value).getString());
			default:
				return false;
		}
	}
	...
	public static boolean addIfNotNull(JsonObjectBuilder builder, String key, Object value) {
		if (StringUtils.isNullOrEmpty(key) || Objects.isNull(value)) {
			return false;
		}
	...
	private static void addTypeSafeValue(JsonObjectBuilder jsonBuilder, String key, Object value) {
		if (value instanceof String) {
			...
			jsonBuilder.add(key, ((Boolean) value).booleanValue());
	...
	private static void addToObjectIfNumber(JsonObjectBuilder jsonBuilder, String key, Object value) {
		if (value instanceof Integer) {
			jsonBuilder.add(key, ((Integer) value).intValue());
		} else if (value instanceof Long) {
			jsonBuilder.add(key, ((Long) value).longValue());
		} else if (value instanceof Double) {
			jsonBuilder.add(key, ((Double) value).doubleValue());
	...
	public static JsonArray convertToJsonArray(Collection<?> collection) {
		...
		for (Object value : collection) {
			if (value == null) {
				...
				arrayBuilder.add(((Uri) value).toString());
			} else if (value instanceof Map) {
	...
	private static void addToArrayIfNumber(JsonArrayBuilder arrayBuilder, Object value) {
		if (value instanceof Integer) {
			arrayBuilder.add(((Integer) value).intValue());
		} else if (value instanceof Long) {
			arrayBuilder.add(((Long) value).longValue());
		} else if (value instanceof Double) {
			arrayBuilder.add(((Double) value).doubleValue());
		} else if (value instanceof Float) {
			// its done this way, because if we work with float directly, it will be casted to double, which adds
			// additional precision to the number(adds more numbers after the floating point) and we don't want that
	...
	public static Map<String, Serializable> jsonToMap(JsonObject json) {
		...
		for (Entry<String, JsonValue> entry : json.entrySet()) {
			addNonNullValue(result, entry.getKey(), readJsonValue(entry.getValue()));
		}
		return result;
	}
	...
	public static DateRange getDateRange(JsonObject json) {
		...
		if (StringUtils.isNullOrEmpty(firstDate) && StringUtils.isNullOrEmpty(secondDate)) {
			return null;
		}
