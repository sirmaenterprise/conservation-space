...
class AsyncTaskExecutor implements TaskExecutor {
	...
	private static int getThreadPoolSize(ConfigurationProperty<Integer> poolSizeConfig) {
		int config = poolSizeConfig.get().intValue();
	...
	private void setPerUserPoolSize(ConfigurationProperty<Integer> perUserPoolSizeConfig) {
		int config = perUserPoolSizeConfig.get().intValue();
	...
	public void waitForAll(Collection<? extends Future<?>> futures) {
		...
		while (isNotEmpty(copy)) {
			sleepForAWhile();
			Iterator<? extends Future<?>> it = copy.iterator();
			while (it.hasNext()) {
	...
	private <T extends GenericAsyncTask> void processCompletedTask(String user, List<Throwable> errors,
		...
		if (asyncTask.isDone()) {
			...
			release(user);
			if (TRACE) {
				LOGGER.trace("Releasing worker for user " + user + " available threads now: "
	...
	private static <T extends GenericAsyncTask> void processOnFail(List<T> tasks, boolean rollbackOnError,
		...
		if (rollbackOnError) {
			LOGGER.warn("There are " + fails.size() + " failed tasks out of " + tasks.size() + ". Rolling back "
					+ successfullTasks.size() + " successful tasks.");
	...
	/**
	 * Async task that executes the given {@link Executable}s at the different task lifecycle
	 *
	 * @author BBonev
	 */
	static class AsyncTask extends GenericAsyncTask {
		private static final long serialVersionUID = 7556656205375568992L;
		private final Supplier<Object> action;
		private final Executable onSuccess;
		private final Executable onFail;
		private final Executable onRollback;
		private final CompletableOperation<Object> future;
		 ...
		 *            the future
		 */
		public AsyncTask(Executable action, Executable onSuccess, Executable onFail, Executable onRollback,
				CompletableOperation<Object> future) {
			super();
			this.future = future;
			this.action = action.asSupplier();
		 ...
		 */
		@SuppressWarnings("unchecked")
		public <T> AsyncTask(Supplier<T> action, Executable onSuccess, Executable onFail, Executable onRollback,
				CompletableOperation<T> future) {
			super();
			this.action = (Supplier<Object>) action;
			this.onSuccess = onSuccess;
			this.onFail = onFail;
			this.onRollback = onRollback;
			this.future = (CompletableOperation<Object>) future;
		}
		...
		protected boolean executeTask() throws Exception {
			if (action != null) {
				try {
					Object resut = action.get();
					if (future != null) {
						future.completed(resut);
					}
				} catch (Exception e) {
					if (future != null) {
						future.failed(e);
