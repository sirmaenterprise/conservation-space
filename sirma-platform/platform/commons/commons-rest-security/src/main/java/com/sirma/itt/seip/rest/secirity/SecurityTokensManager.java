...
public class SecurityTokensManager {
	...
	public String generate(User user) {
		...
		if (user.getTicket() != null) {
			tokensHolder.addToken(jwtCompactSerialization, user.getTicket(),
					user.getProperties().get(JwtUtil.SESSION_INDEX).toString());
		}
	...
	public String getSamlToken(JwtClaims claims) throws JoseException {
		...
		return tokensHolder.getSamlToken(jws.getCompactSerialization());
	}

	/**
	 * Removes token from the cache on user logout. Listens for {@link BeginLogoutEvent}.
	 *
	 * @param event
	 *            the fired event for logout
	 */
	void onBeginLogout(@Observes BeginLogoutEvent event) {
