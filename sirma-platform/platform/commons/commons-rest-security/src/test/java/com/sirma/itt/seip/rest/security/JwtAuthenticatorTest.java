...
public class JwtAuthenticatorTest {
	...
	protected void init() throws NoSuchAlgorithmException {
		...
		ReflectionUtils.setField(authenticator, "jwtConfig", jwtConfig);
		ReflectionUtils.setField(authenticator, "securityContextManager", securityContextManager);
		ReflectionUtils.setField(authenticator, "userStore", userStore);
		ReflectionUtils.setField(authenticator, "securityTokensManager", securityTokensManager);
	}
	...
	public void testSuccessfulAuthentication() throws JoseException {
		Mockito.when(securityTokensManager.getSamlToken(Matchers.any(JwtClaims.class))).thenReturn("samlToken");

		User authenticate = authenticator
				.authenticate(createAuthContext("Bearer " + generateJwt(ISSUER, goodGuy.getIdentityId(), true, true)));
		Assert.assertEquals(authenticate, goodGuy);
	}
