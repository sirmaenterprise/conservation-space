...
public final class RuntimeContext {
	...
	private static final Set<String> TRANSFERRABLE_KEYS = new HashSet<>();
	...
	public static boolean isSet(String key, String... other) {
		...
		if (!result && other != null && other.length > 0) {
			for (int i = 0; i < other.length; i++) {
				result |= isConfigurationSet(other[i]);
			}
		}
		return result;
	}
	...
	public static CurrentRuntimeConfiguration replaceConfiguration(CurrentRuntimeConfiguration runtimeConfiguration) {
		...
		Map<String, Deque<Serializable>> configurationData = getOrCreateConfigurationData(CONFIGURATION);
		configurationData.keySet().removeAll(TRANSFERRABLE_KEYS);
		configurationData = getOrCreateConfigurationData(INHERITABLE_CONFIGURATION);
		configurationData.keySet().removeAll(TRANSFERRABLE_KEYS);
	...
	private static Map<String, Deque<Serializable>> getTransferrableConfigurationData(
			ThreadLocal<Map<String, Deque<Serializable>>> store) {
		Map<String, Deque<Serializable>> map = store.get();
		...
		copy.keySet().retainAll(TRANSFERRABLE_KEYS);
		return copy;
	}
	...
	public static final class CurrentRuntimeConfiguration {
		...
		private CurrentRuntimeConfiguration() {
			...
			currentConfiguration = getTransferrableConfigurationData(CONFIGURATION);
			currentInheritableConfiguration = getTransferrableConfigurationData(INHERITABLE_CONFIGURATION);
		}
	...
	private static class ConfigImpl implements Config {
		 ...
		 *            the store
		 */
		public ConfigImpl(String key, ThreadLocal<Map<String, Deque<Serializable>>> store) {
			this.key = key;
			this.store = store;
		}
	...
	private static class OptionImpl implements Option {
		 ...
		 *            the store
		 */
		public OptionImpl(String key, ThreadLocal<Map<String, Deque<Serializable>>> store) {
			this.key = key;
			this.store = store;
		}
	...
	private static class GroupOption extends OptionImpl {
		 ...
		 *            the additional options
		 */
		public GroupOption(String key, ThreadLocal<Map<String, Deque<Serializable>>> store, Option... options) {
			super(key, store);
			if (options == null) {
