 ...
 *            the collection type
 */
public class BaseLazyCollection<E, C extends Collection<E>> implements Collection<E>, Serializable {

	private static final long serialVersionUID = -7433170766018002033L;
	private E initial;
	...
	protected C getStore() {
		if (expandedStore == null) {
			// the synchronization is here to be more optimal. to prevent double initialization but also to remove the
			// performance penalty of constant synchronization when accessing the store.
			synchronized (this) {
