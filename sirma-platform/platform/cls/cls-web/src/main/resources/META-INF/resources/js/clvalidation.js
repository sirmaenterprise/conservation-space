...
CLS.initCLCreateValidator = function(onSuccess, onError) {
	var rules = {
		cl_id: {
			...
			regex: "^[0-9]*$",
			remote: {
				url: "service/codelists?excludeValues=true&responseFields=value",
				type: "GET",
				cache: false,
	...
	var messages = {
		cl_id: {
			regex: "Must be an integer value. No special characters allowed.",
...
CLS.initCVCreateValidator = function(onSuccess, onError) {
	var rules = {
		cv_id_modal: {
			required: true,
			remote: {
				url: "service/codelists/3/codevalues?responseFields=value",
				type: "GET",
				cache: false,
				dataFilter: function(data) {
					var json = JSON.parse(data);
...
CLS.initValidator = function(formId, rules, messages, onSuccess, onError) {
    ...
    var validator = $("#" + formId).validate({
        ...
        errorPlacement: function (error, element) {
        	...
        	$(element).popover({ title : errorMsg}).popover("show");
