...
$(function() {
		...
		CLS.util.makeGetJsonRequest(EMF.config.baseURL + 'service/codelists/' +
		...
		$("#cv-search-btn").click(function() {
			CLS.util.displayAjaxLoader("code-values", "Loading...", false);
			CLS.searchCodeValues("service/codelists/" + codeListID +
		...
		$("#cv-clear-btn").click(function() {
			CLS.util.resetTextInputsWithin("cv-search-form");
		});
...
//		});
	...
	var errorMessage = function (response) {
		EMF.dialog.notify({
			title : 'Error',
			message : 'There is already a code value with the same ID.',
			okBtn       : 'OK',
	...
	CLS.saveCLEdit = function () {
		if(CLS.validateCLData()){
			...
			$.ajax ({
		        type: "POST",
		        url: "service/codelists/" + jsonData.value,
		        contentType : 'application/json',
		        async: false,
		        data: JSON.stringify(jsonData),
  				success: function () {
  					CLS.toggleCLDetails();
	        		CLS.util.makeGetJsonRequest('service/codelists/' +
	...
	CLS.saveCVEdit = function () {
		if(CLS.validateCLData()){
			...
			$.ajax ({
		        type: "POST",
		        url: "service/codelists/codevalue/" + jsonData.value,
		        contentType : 'application/json',
		        async: false,
		        data: JSON.stringify(jsonData),
				success: function () {
					...
					CLS.searchCodeValues("service/codelists/" + CLS.currentCodeList.results[0].value  +
...
CLS.createCV = function () {
	if(CLS.validateCLData()){
		...
		$.ajax ({
	        type: "PUT",
	        url: "service/codelists/"+ CLS.currentCodeList.results[0].value + "/codevalues/",
	        contentType : 'application/json',
	        async: false,
	        data: JSON.stringify(jsonData),
			success: function () {
...
var CLS = CLS || {};
	...
	CLS.saveAndPopulateFields = function(dataObj){
		  ...
		  //CLS.displayCodeValues(dataObj.results[0]);
			var updateURL="service/codelists/"+dataObj.results[0].masterValue+"/codevalues?responseFields=value";
			CLS.util.makeGetJsonRequest(updateURL, function(data) {
				CLS.populateDropDown(data, "cv_master", "");
...
//		 $("#cl_comment").val(CLS.currentCodeList.descriptions[1].description);
		 ...
		 var anchor = CLS.util.getURLAnchorValue(window.location.href);
		 CLS.searchCodeValues("service/codelists/" + codeList.value +
				 "/codevalues" + CLS.generateCvParamsList() + (anchor?"#"+anchor:""));
		 CLS.updateDropDownValues(
					"service/codelists?excludeValues=true&responseFields=value", "cl_master", codeList.masterValue);
	}
	...
	**/
	CLS.displayCodeValues = function (dataObj) {
		...
		var table = "<table id='results-table' class='table table-hover table-bordered table-striped'>" +
	      ...
	      "<th style='width: 60%'>Description (en)</th>" +
	      "<th style='width: 10%'>Valid from</th>" +
	      "<th style='width: 5%'>Valid to</th>" +
	      "<th style='width: 5%'></th>" +
	      "</tr>" +
	      "</thead>" +
	      "<tbody>";
		  ...
		  for(var i = 0; i < codeValuesObj.length; i++) {
			    var cvObj = codeValuesObj[i];
			     table+= "<tr data-toggle='modal' cl-id="+
			     cvObj.codeListId +" cv-id=" +cvObj.value +" cv-from="+ cvObj.validFrom +" cv-to=" +
			     cvObj.validTo + " style='cursor: pointer'>"
				 + "</td><td>" + cvObj.value
				 + "</td><td>" + CLS.util.getDescriptionByLanguage(cvObj.descriptions, CLS.LANGUAGE_CODE_EN)
				 + "</td><td>" + CLS.util.formatDate(cvObj.validFrom)
				 + "</td><td>" + CLS.util.formatDate(cvObj.validTo) 
				 + "</td><td><button type='button' id='cv-edit' data-toggle='modal' data-target='#cvDetailsModal' class='btn btn-sm btn-primary "
				 + "style='cursor: default' onclick='CLS.openCVDetails(" 
				 +	cvObj.codeListId + ",\"" + cvObj.value + "\",\"" + cvObj.validFrom + "\",\"" + cvObj.validTo + "\")'>"
				 + _emfLabels['codevalue.details.button.label'] +"</button>"
				 + "</td></tr>";
		  }
		  table+= "</tbody></table>";
	...
	/**
	 * Populates the code value details modal window with the CV data.
	 */
	CLS.openCVDetails = function (codeListId, value, validFrom, validTo){
		CLS.handleCVClick(codeListId, value, validFrom, validTo);
		CLS.toggleCVDetails(true);
	}

	/**
	 * Constructs the string of parameters that gets appended to the URL for the code values search.
	 */
