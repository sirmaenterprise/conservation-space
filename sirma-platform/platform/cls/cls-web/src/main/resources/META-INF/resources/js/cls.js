...
/**
 * Resets all input text and select2 fields that are children of the given parent element.
 */
CLS.util.resetTextInputsWithin = function(parentElementID) {
	var parent = $("#" + parentElementID);
	parent.find(":text").val('');
	parent.find(".select2").select2("val", "");
}
...
/**
 * Gets the description string by the given ISO language code (case-insensitive).
 */
CLS.util.getDescriptionByLanguage = function(descriptions, language) {
	var matchedDescription;
	$.each(descriptions, function(index, descriptionObj) {
		  if(descriptionObj.language.toUpperCase() === language.toUpperCase()) {
			  matchedDescription = descriptionObj.description;
			  return false;
		  }
	});
...
/**
 * Updates the site title to be Code list server.
 * This is a Workaround. Since at that point it requires more effort to fix the orders of the components
 * overriding the site title (which is not worth for the obsolete UI 1).
 */
CLS.updateSiteTitle = function() {
	$(".header-menu .site-title").html(_emfLabels['codelist.server.title']);
}
