...
public class CodeValueValidator implements Validator<CodeValue> {
	...
	public Map<String, String> validate(CodeValue code, boolean update) {
		...
		if (code.getCodeListId() == null) {
			...
			validateExistingCodeList(codeLists, code, errorsMap);
		}
		...
		if (update) {
			...
			valdiateDuplicateCodeValues(code, errorsMap);
		}
		return errorsMap;
	}

	/**
	 * Validates if the a code list with that ID exists and if code value with this ID belongs to master code list.
	 *
	 * @param codeLists
	 *            are the code lists
	 * @param code
	 ...
	 *            is the error map
	 */
	private void validateExistingCodeList(SearchResult codeLists, CodeValue code, Map<String, String> errorsMap) {
	...
	/**
	 * Validates if there is already a code value with the same ID.
	 *
	 * @param code
	 *            is the code value
	 * @param errorsMap
	 *            is the errors map
	 */
	private void valdiateDuplicateCodeValues(CodeValue code, Map<String, String> errorsMap) {
