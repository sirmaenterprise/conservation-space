...
public class StartupComponentProvider implements Extension {
	...
	private Map<StartupPhase, List<Component>> componentMapping = CollectionUtils.createLinkedHashMap(4);
	 ...
	 */
	public List<Component> getComponentsForPhase(StartupPhase phase) {
	 ...
	 */
	public List<Component> getDeploymentComponents() {
	 ...
	 */
	public List<Component> getBeforeApplicationStartedComponents() {
	 ...
	 */
	public List<Component> getAfterApplicationStartedComponents() {
	...
	protected <X> void beforeDeployment(@Observes BeforeBeanDiscovery beanDiscovery) {
		for (StartupPhase startupPhase : StartupPhase.values()) {
			componentMapping.put(startupPhase, new LinkedList<Component>());
		}
	}
	...
	private <X> void registerClassAnnotation(AnnotatedType<X> annotatedType, BeanManager beanManager) {
		...
		CollectionUtils.addValueToMap(componentMapping, annotation.phase(),
				new ClassComponentInstance(annotation, annotatedType.getJavaClass(), beanManager));
	}
	...
	private <X> void registerMethodAnnotation(AnnotatedMethod<X> annotatedType, BeanManager beanManager) {
		...
		CollectionUtils.addValueToMap(componentMapping, annotation.phase(), new MethodComponentInstance(annotation,
