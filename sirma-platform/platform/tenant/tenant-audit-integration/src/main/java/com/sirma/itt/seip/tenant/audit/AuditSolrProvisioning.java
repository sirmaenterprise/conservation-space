 ...
 */
@ApplicationScoped
public class AuditSolrProvisioning extends BaseRelationalDbProvisioning {
	 ...
	 *             on provision error
	 */
	public void provisionRecentActivitiesModel(TenantInfo tenantInfo, String tenantName) throws RollbackedException {
		try {
			securityContextManager.initializeTenantContext(tenantInfo.getTenantId());
			// init the correct context values before server creation
			transactionSupport.invokeInNewTx(() -> insertRecentActivitiesConfigurations(tenantInfo));
			provisionRecentActivitiesModelInternal(tenantInfo, tenantName);
		} catch (RollbackedException e) {
			throw e;
		} catch (Exception e) {
			throw new RollbackedException(e);
	 ...
	 *            name of the core
	 * @throws IOException
	 *             when unloading the core
	 * @throws RollbackedException
	 *             when producing the http client
	 */
	public void unloadCore(String coreName) throws IOException, RollbackedException {
	...
	private void provisionRecentActivitiesModelInternal(TenantInfo tenantInfo, String tenantName)
			throws RollbackedException {
		try (CloseableHttpClient produceHttpClient = produceHttpClient()) {
			String coreName = getRecentActivitiesCoreName(tenantName);
	...
	private void solrAdminRequest(String operation, String dbName, CloseableHttpClient produceHttpClient)
			throws IOException, RollbackedException {
		if ("CREATE".equals(operation)) {
			/** "http://localhost:8984/solr/admin/cores?action=CREATE&name=audit2&configSet=audit_configs_cs1"; */
			HttpGet request = new HttpGet(auditConfiguration.getSolrAdminAddress().get() + "/cores?action=CREATE&name="
					+ dbName + "&configSet=" + dbName);
			...
			HttpGet request = new HttpGet(auditConfiguration.getSolrAdminAddress().get()
					+ "/cores?action=UNLOAD&deleteInstanceDir=true&core=" + dbName);
	...
	private void insertRecentActivitiesConfigurations(TenantInfo tenantInfo) {
		String escapedTenantId = DbProvisioning.createUserNameFromTenantId(tenantInfo);
		Configuration recentActivitiesCoreName = new Configuration(
				auditConfiguration.getRecentActivitiesSolrCoreConfigurationName(),
				getRecentActivitiesCoreName(escapedTenantId), tenantInfo.getTenantId());
		configurationManagement.updateConfiguration(recentActivitiesCoreName);
	}
	 ...
	 *            the tenant info
	 */
	public void rollbackRecentActivitiesCoreCreation(TenantRelationalContext relationalContext, TenantInfo tenantInfo) {
		String coreName = getRecentActivitiesCoreName(relationalContext.getDatabaseName());
	...
	private void rollbackCoreCreation(String coreName, TenantInfo tenantInfo) {
		...
		if (!SecurityContext.isDefaultTenant(tenantInfo.getTenantId())) {
			...
			try {
				...
				LOGGER.warn("Could not rollback solr creation for tenant {} due to error", tenantInfo.getTenantId(), e);
			} finally {
				securityContextManager.endContextExecution();
			}
		}
	...
	private static String getRecentActivitiesCoreName(String tenantName) {
		return tenantName + RECENT_ACTIVITIES_SUFFIX;
	}
