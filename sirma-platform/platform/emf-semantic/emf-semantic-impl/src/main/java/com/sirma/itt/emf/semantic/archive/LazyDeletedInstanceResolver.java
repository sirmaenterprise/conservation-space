...
public class LazyDeletedInstanceResolver implements DependencyResolver {
	...
	public Integer countDependencies(Instance instance) {
		...
		query
				.append("${query(")
					.append(COUNT_ALL_CHILD_INSTANCES)
					.append(", objectUri=")
					.append(id)
					.append(", transactionId=")
					.append(transactionId)
					.append(").count}");
		return evaluator.evaluate(query.toString(), Integer.class);
	}

	@Override
	public int currentBatchSize() {
		return dependencyDeleteBatchSize.get().intValue();
	}
	...
	Collection<Instance> loadElements(SearchArguments<? extends Instance> arguments, int size) {
		...
		if (arguments.getResult() != null && !arguments.getResult().isEmpty()) {
			List<Instance> result = (List<Instance>) arguments.getResult();
			Options.ALLOW_LOADING_OF_DELETED_INSTANCES.enable();
			try {
				local = BatchEntityLoader.loadDistinctInstances(result, serviceRegistry, taskExecutor);
			} finally {
	...
	private class LazyQueryIterator implements Iterator<Instance> {
		 ...
		 */
		@SuppressWarnings("unchecked")
		public <I extends Instance> LazyQueryIterator(SearchArguments<I> arguments, Integer count, Integer batchSize) {
			expectedCount = count;
		...
		private boolean checkForMore() {
			...
			if (expectedCount.intValue() == returnedInstances) {
				return false;
			}
			return true;
		}
