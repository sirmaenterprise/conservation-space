...
public class SemanticQueryVisitor extends AbstractQueryVistor {
	...
	private List<String> listOfPendingPredicates;
	...
	@Override
	public void start() {
		listOfPendingPredicates = new ArrayList<>(7);
		...
		if (StringUtils.isNotNullOrEmpty(projection)) {
		...
		if (StringUtils.isNotNullOrEmpty(ignoreInstancesForTypeClause) && applyFilterForType) {
	...
	private void buildSimpleContextClause(String subject, Serializable value, StringBuilder filterClause) {
		String relationPredicate = VARIABLE + createVariableName("anyRelation");
