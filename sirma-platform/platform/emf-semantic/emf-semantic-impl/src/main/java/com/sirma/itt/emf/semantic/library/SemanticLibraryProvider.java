...
public class SemanticLibraryProvider implements LibraryProvider {
	...
	@Override
	public List<ClassInstance> getAllowedLibraries(String libraryId, String forAction) {
		List<ClassInstance> library = semanticDefinitionService.getLibrary(libraryId);
		return filterLibrariesForAction(library, forAction);
	}
	...
	@Override
	public ClassInstance getLibraryElement(String libraryId, String elementId) {
		List<ClassInstance> library = semanticDefinitionService.getLibrary(libraryId);
		if (CollectionUtils.isEmpty(library) || StringUtils.isNullOrEmpty(elementId)) {
			return null;
		}
		String fullUri = namespaceRegistryService.buildFullUri(elementId);
		return library.stream().filter(clazz -> clazz.getId().equals(fullUri)).findFirst().orElse(null);
	}
}
