 ...
 */
@ApplicationScoped
public class ObjectPropertyModificationObserver {
	...
	void onLinkAdded(@Observes LinkAddedEvent event) {
		...
		if (addChanges(addedLink)) {
			transactionSupport.invokeBeforeTransactionCompletion(() -> touchUsingBuffer());
		}
	}
	...
	private void touchUsingBuffer() {
		try {
			...
			transactionSupport.invokeInNewTx(() -> instanceService.touchInstance(changes));
