...
public class SparqlQueryFilterProvider {
	...
	private static final String IS_NOT_LAST_REVISION = "\n\toptional {\n\t%s " + EMF.PREFIX + ":"
			+ EMF.REVISION_TYPE.getLocalName() + " " + EMF.PREFIX + ":" + EMF.TYPE_LATEST_REVISION.getLocalName()
			+ ".\n\t%s " + EMF.PREFIX + ":" + EMF.IS_DELETED.getLocalName()
			+ " ?check%s.\n\t}\n\tFILTER(! BOUND( ?check%s)).\n";

	private Map<String, Function<String, String>> filters = new HashMap<>();

	/**
	 * Initialize the provider with the default filters
	 */
	@PostConstruct
	...
	void initialize() {
		...
		filters.put(NamedQueries.Filters.IS_NOT_LAST_REVISION, var -> {
			long index = CHECK_INDEX.getAndIncrement();
			return String.format(IS_NOT_LAST_REVISION, var, var, index, index);
		});
	}
