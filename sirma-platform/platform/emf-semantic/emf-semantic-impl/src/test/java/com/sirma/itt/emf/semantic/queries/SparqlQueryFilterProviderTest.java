...
public class SparqlQueryFilterProviderTest {
	...
	@Test
	public void testFilter_isNotLastRevision() throws Exception {
		String filter = filterProvider.getFilterBuilder(NamedQueries.Filters.IS_NOT_LAST_REVISION).apply("?instance");
		assertTrue(filter.contains("?instance"));
		assertTrue(filter.contains("emf:revisionType"));
		assertTrue(filter.contains("emf:latestRevision"));
		assertTrue(filter.contains("?check"));
	}
}
