...
public class SemanticURIConverterRestServiceTest extends EmfTest {
	...
	private void mockStringToURIConverter() {
		Mockito.when(typeConverter.convert(Matchers.eq(URI.class), Matchers.any(String.class))).thenAnswer(invocation -> {
			...
			return new URIImpl(fullURI);
		});
	}

	private void mockURIToStringConverter() {
		Mockito.when(typeConverter.convert(Matchers.eq(String.class), Matchers.any(URI.class))).thenAnswer(invocation -> {
			URI fullURI = invocation.getArgumentAt(1, URI.class);
