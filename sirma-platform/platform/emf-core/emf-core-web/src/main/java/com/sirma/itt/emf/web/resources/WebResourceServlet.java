...
/**
 * Provides access to static and dynamic resources like scripts, css and images by using a set of plugged
 * {@link WebResourceHandler} classes.
 *
 * @author Adrian Mitev
 */
...
public class WebResourceServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (resource != null) {
			...
			if (!StringUtils.isNullOrEmpty(ifNoneMatchHeader) && ifNoneMatchHeader.equals(resource.getHash())) {
	...
	private void addCacheHeaders(HttpServletResponse response) {
		// one month from now
		response.addHeader("Last-Modified", new Date(System.currentTimeMillis() + 2592000 * 1000).toString());
		// one month in the past
		response.addHeader("Expires", new Date(System.currentTimeMillis() - 2592000 * 1000).toString());
