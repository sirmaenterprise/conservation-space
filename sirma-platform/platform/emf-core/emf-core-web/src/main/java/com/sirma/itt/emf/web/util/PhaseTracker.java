...
public class PhaseTracker implements PhaseListener {
	...
	public void beforePhase(PhaseEvent event) {
		...
		event.getFacesContext().getExternalContext().getRequestMap().put("PhaseTracker", System.currentTimeMillis());
	}

	@Override
	public void afterPhase(PhaseEvent event) {
		Long time = (Long) event.getFacesContext().getExternalContext().getRequestMap().get("PhaseTracker");
		LOGGER.info("After phase: " + event.getPhaseId() + " " + (System.currentTimeMillis() - time) + " msec");
	}
