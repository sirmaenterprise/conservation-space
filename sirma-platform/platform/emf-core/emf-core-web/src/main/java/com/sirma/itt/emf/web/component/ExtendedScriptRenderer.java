...
/**
 * Overrides the default renderer for h:outputScript render urls that fetch scripts directly, not through the faces
 * servlet.
 *
 * @author Adrian Mitev
 */
public class ExtendedScriptRenderer extends ScriptRenderer {
	...
	public void encodeEnd(FacesContext context, UIComponent component) throws IOException {
		...
		if ("jsf.js".equals(name)) {
			...
			if (minified != null && Boolean.FALSE.equals(context.getExternalContext().getApplicationMap().get(
					ApplicationConfigurationProvider.APPLICATION_MODE_DEVELOPMENT))) {

				if (Boolean.parseBoolean(minified)) {
			...
			writer.writeURIAttribute("src", context.getExternalContext().getRequestContextPath() + "/"
					+ (library != null ? library + "/" : "") + name + "?time=" + startupTime, "src");
		}
