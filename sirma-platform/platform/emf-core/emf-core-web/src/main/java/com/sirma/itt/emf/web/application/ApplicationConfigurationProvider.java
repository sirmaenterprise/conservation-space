...
import com.sirma.itt.seip.content.ContentConfigurations;
...
public class ApplicationConfigurationProvider {
	...
	public static final String APPLICATION_MODE_DEVELOPMENT = SystemConfiguration.APPLICATION_MODE_DEVELOPMENT;
	/** Whether or not to install logging PhaseListener. */
	public static final String UI_JSF_PHASETRACKER = "ui.jsf.phasetracker";
	...
	@Inject
	@Configuration
	@ConfigurationPropertyDefinition(name = UI_JSF_PHASETRACKER, type = Boolean.class, defaultValue = "false", system = true, label = "If true a special phase tracking PhaseListener is installed that logs the time taken by each jsf phase")
	private ConfigurationProperty<Boolean> jsfPhaseTracker;
	...
	@Inject
	@Configuration
	@ConfigurationPropertyDefinition(name = "application.module.schedule.disable", type = Boolean.class, defaultValue = "false", label = "If PM schedule module should be inaccessible trough the application.")
	private ConfigurationProperty<Boolean> applicationModuleScheduleDisable;
	...
	static String buildLogo(ConverterContext context, ApplicationConfigurationProvider provider) {
		...
		if (isBase64Encoded(raw)) {
			...
			return getLogoAsInternalResource(context.getDefaultValue(), provider);
		}

		if (EqualsHelper.nullSafeEquals(context.getDefaultValue(), raw)) {
			return getLogoAsInternalResource(raw, provider);
		}
		...
		if (external.isFile()) {
			...
			return tryLoadingAsExternalFile(context, provider, raw, external);
		}
		return tryLoadingAsLink(provider, raw);
	}

	private static String tryLoadingAsExternalFile(ConverterContext context, ApplicationConfigurationProvider provider,
	...
	private static String tryLoadingAsLink(ApplicationConfigurationProvider provider, String raw) {
		try {
	...
	private static String getLogoAsInternalResource(String name, ApplicationConfigurationProvider provider) {
		...
		try {
			...
			return builder.toString().replace(RESOURCES_LOCATION, provider.getBaseURL());
		}
		return base64;
	}
	...
	private static boolean isValidBase64Encoded(String raw) {
		String base64 = raw.split(";base64,")[1];
	 ...
	 * @return the applicationModeDevelopement
	 */
	public Boolean getJsfPhaseTracker() {
		return jsfPhaseTracker.get();
	}

	/**
	 * Gets the jsf phase tracker configuration.
	 *
	 * @return the jsf phase tracker configuration
	 */
	public ConfigurationProperty<Boolean> getJsfPhaseTrackerConfiguration() {
		return jsfPhaseTracker;
	}
	...
	public Boolean getApplicationModuleScheduleDisable() {
		return applicationModuleScheduleDisable.get();
	}
	...
	public String getBaseURL() {
		return systemConfiguration.getSystemAccessUrl().get() + getContextPath() + "/";
	}
