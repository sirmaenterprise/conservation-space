...
var SessionTimer = {
	...
	config: {
		...
		// Handler functions that will be executed when session timeout occurs.
		// Every registered handler function should return true or false as result.
		// If any of registered handlers returns false, then the session timer will
		// be reset and the logout action won't be executed.
		// Register handler like this:
		// EMF.config.beforeLogoutHandlers.print = function() {
		//   console.log('print1 before logout');
		//   return true;
		// }
		beforeLogoutHandlers: null
	},
	...
	sessionTimeout: function(logout) {
		var handlers = SessionTimer.config.beforeLogoutHandlers,
			executeLogout = true,
			success = false;
		for ( var handler in handlers) {
			if (handlers.hasOwnProperty(handler) && $.isFunction(handlers[handler])) {
				success = handlers[handler].call(this);
				if (!success) {
					executeLogout = false;
				}
			}
		...
		if (executeLogout) {
			...
			window.location.href = SessionTimer.config.baseURL + 'ServiceLogout';
		} else {
	...
	init: function(opts) {
		...
		// set the handlers explicitly because of some reason they are not merged from options to local config
		SessionTimer.config.beforeLogoutHandlers = opts.beforeLogoutHandlers;
