...
(function($){
	$.fn.tipTip = function(options) {
		...
		return this.each(function() {
			...
			var org_title = '';
			var timer;
			// TODO: temporary removed 'focus' event because in modal panels when opened the first link is auto focused and this is annoing for the user
			container.on('mouseover', opts.tooltipActivator, function(evt) {
				var element = $(this);
				...
				setLoading($(evt.target), true, true);
				timer = setTimeout(function() {
					...
					if(tooltipContent.length === 0) {
						...
						if (tooltipContent.length > 0) {
							...
							setLoading($(evt.target), false, true);
							return;
						}
					}
				...
				setLoading(org_elem, false, false);
			...
			function setLoading(element, isLoading, isOnFocus) {
				var currentElement, cursorStyle
					defaultCursor = !isOnFocus ? 'default' : 'pointer';
				if(element) {
					current = element.jquery ? element[0] : element;
					cursorStyle = isLoading ? 'progress' : defaultCursor;
					current.style.cursor = cursorStyle;
					document.getElementsByTagName('html')[0].style.cursor = cursorStyle;
					if (isLoading) {
						tiptip_holder.addClass('tip-loading');
					} else {
						tiptip_holder.removeClass('tip-loading');
					}
				}
			};
			...
			var searchInElement = {
				...
				/**
				 * Helper that will holds methods for generating tool-tip from 
				 * different parts of the link.
				 * 
				 * The tool-tip can be generated by tag, attributes etc.
				 */
				searchUtils : {
					/**
					 * Generate tool-tip from link attribute <b>href</b>. This attribute 
					 * will holds data for specific instance, the instance will be loaded from 
					 * the server and based on it's headers will generate tool-tip.
					 */
					extractInstanceData : function(element){
						// mark parameters that have values, represent URL matcher
						var urlPropertyPattern = "=([^&]*)";
						var INSTANCE_ID = "instanceId";
						var TYPE = "type";
						var LINK_HREF = "href";
						var MESSAGES = {
							PROPERTY_NOT_FOUND  : 'Cannot extract tooltip parameters instanceId and type!',
							EMPTY_LINK_LOCATION : 'Link location is empty!'
						};
						...
						// extract location of the link
						var href = headerLink.attr(LINK_HREF);
						if(!$.trim(href).length){
							console.error(MESSAGES.EMPTY_LINK_LOCATION);
							return null;
						}
						// find instance identifier
						var instanceId = new RegExp(INSTANCE_ID + urlPropertyPattern).exec(href);
						// find instance type
						var type = new RegExp(TYPE + urlPropertyPattern).exec(href);
						
						if(!instanceId || !type){
							console.error(MESSAGES.PROPERTY_NOT_FOUND);
							return null;
						}
						instanceData.id	  = instanceId[1];
						instanceData.type = type[1];
						return instanceData;
					}
				}
			}
