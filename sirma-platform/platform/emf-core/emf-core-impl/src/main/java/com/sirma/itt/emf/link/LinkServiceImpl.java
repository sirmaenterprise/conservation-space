...
public class LinkServiceImpl extends AbstractLinkService {
	...
	private List<LinkReference> getLinksInternal(Object from, Object to, String linkId, Collection<String> linkIds,
		...
		if (fromRef != null) {
			query = links.isEmpty() ? EmfQueries.QUERY_LINK_BY_SRC_KEY : EmfQueries.QUERY_LINK_BY_SRC_AND_IDS_KEY;
		...
		if (toRef != null) {
			query = links.isEmpty() ? EmfQueries.QUERY_LINK_BY_TARGET_KEY : EmfQueries.QUERY_LINK_BY_TARGET_AND_IDS_KEY;
	...
	private void removeProperties(Entity<?> instance, PathElement path) {
		propertiesService.removeProperties(instance, path);
	}
	...
	public boolean linkSimple(InstanceReference from, InstanceReference to, String linkId) {
		Pair<Serializable, Serializable> pair = link(from, to, encodeSimpleLink(linkId),
				encodeSimpleLink(getReverseLinkType(linkId)), Collections.<String, Serializable> emptyMap());
