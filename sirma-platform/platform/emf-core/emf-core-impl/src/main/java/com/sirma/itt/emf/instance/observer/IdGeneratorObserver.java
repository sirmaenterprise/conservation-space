...
public class IdGeneratorObserver {
	...
	public void onInstanceCreate(@Observes BeforeInstancePersistEvent<?, ?> event) {
		...
		PropertyDefinition node = dictionaryService.getProperty(DefaultProperties.UNIQUE_IDENTIFIER, instance);
		if (node == null) {
	...
	private void generateId(BeforeInstancePersistEvent<?, ?> event, Instance instance, PropertyDefinition node) {
		try {
			if (StringUtils.isNotNullOrEmpty(node.getRnc()) && node.getRnc().contains("rootContext")) {
				Instance rootInstance = InstanceUtil.getRootInstance(instance);
				if (rootInstance != null) {
					Serializable serializable = instance.getProperties().get(DefaultProperties.UNIQUE_IDENTIFIER);
	 ...
	 *            the value
	 */
	private static void setEvaluatedValueIfValid(BeforeInstancePersistEvent<?, ?> event, Instance instance,
			Serializable value) {
	 ...
	 *            the event
	 */
	private static void updateAsHandled(BeforeInstancePersistEvent<?, ?> event) {
