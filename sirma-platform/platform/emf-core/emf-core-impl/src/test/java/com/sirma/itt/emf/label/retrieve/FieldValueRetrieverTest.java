...
public class FieldValueRetrieverTest {
	...
	public void testActionValuesRetrieverNoFilter() {
		...
		RetrieveResponse values = fieldValueRetrieverService.getValues(FieldId.ACTION_ID, null, 0, 10);
		Assert.assertEquals(values.getTotal(), new Long(40));
		Assert.assertEquals(values.getResults().size(), 10);
		Assert.assertEquals(values.getResults().get(0).getFirst(), "cv0");
		Assert.assertEquals(values.getResults().get(0).getSecond(), "Label 0 (Task)");
	}
