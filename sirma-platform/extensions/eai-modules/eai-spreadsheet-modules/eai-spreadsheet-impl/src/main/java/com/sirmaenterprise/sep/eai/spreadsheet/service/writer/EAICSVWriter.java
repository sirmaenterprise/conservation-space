...
public class EAICSVWriter implements SpreadsheetWriter {
	...
	public boolean isSupported(ContentInfo content) {
		return EAISpreadsheetConstants.CSV_MIMETYPE.equals(content.getMimeType()) || EAISpreadsheetConstants.TEXT_MIMETYPE.equals(content.getMimeType());
	}
	...
	static void setImportStatus(StringBuilder sheet, List<String> header, List<String> row, String statusValue) {
		if (header.get(header.size() - 1).equals(EAISpreadsheetConstants.IMPORT_STATUS)) {
			row.set(header.indexOf(EAISpreadsheetConstants.IMPORT_STATUS), statusValue);
			sheet.append(sanitizeRow(row)).append(System.lineSeparator());
		} else {
			header.add(EAISpreadsheetConstants.IMPORT_STATUS);
			row.add(statusValue);
			sheet.append(sanitizeRow(row)).append(System.lineSeparator());
		}
	}
	...
	private File generateFile(StringBuilder sheet) throws IOException {
		...
		FileUtils.writeStringToFile(file, sheet.toString());
		return file;
	}
