...
public class RulesRestService {
	...
	private static JSONObject convertRuleToJson(InstanceRule rule, Boolean isActive) {
		...
		JsonUtil.addToJson(object, "async", Boolean.valueOf(rule.isAsyncSupported()));
		...
		List<String> types = rule.getSupportedObjects().stream().map(c -> c.getSimpleName().toLowerCase()).collect(
				Collectors.toCollection(LinkedList::new));
		JsonUtil.addToJson(object, "types", new JSONArray(types));
