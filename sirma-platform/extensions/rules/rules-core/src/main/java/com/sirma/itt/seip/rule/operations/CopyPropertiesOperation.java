...
public class CopyPropertiesOperation extends BaseRuleOperation {
	...
	private OnDuplicateConfig onDublicateConfig = new OnDuplicateConfig();
	...
	public boolean configure(Context<String, Object> configuration) {
		...
		if (duplicateConf != null) {
			onDublicateConfig = OnDuplicateConfig.parse(duplicateConf);
		}
		if (onDublicateConfig != null && !propertyMapping.isEmpty()) {
			...
			for (PropertyMapping mapping : propertyMapping) {
				if (mapping.getOnDuplicate() == null) {
					mapping.setOnDuplicate(onDublicateConfig);
				}
			}
		}
		isDisabled = false;
