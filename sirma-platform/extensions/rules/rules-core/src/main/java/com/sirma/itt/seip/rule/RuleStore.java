...
public class RuleStore {
	...
	/**
	 * On definition loaded.
	 *
	 * @param event
	 *            the event
	 */
	void onDefinitionLoaded(@Observes AllDefinitionsLoaded event) {
		taskExecutor.executeAsync(this::prepareDefInstanceRules);
	}

	/**
	 * Populate operation cache.
	 *
	 * @param iterable
	 *            the iterable
	 * @param cache
	...
	private void prepareDefInstanceRules() {
		...
		try {
			...
			for (GenericDefinition definition : allDefinitions) {
				...
				List<GenericDefinition> subDefinitions = definition.getSubDefinitions();
				for (GenericDefinition subDefinition : subDefinitions) {
					buildAndConfigureRuleFromDefinition(subDefinition, defRules);
				}
	...
	private Context<String, Object> readConfiguration(DefinitionModel definition, DynamicInstanceRule instanceRule) {
		...
		Set<Class<?>> instanceTypes = new LinkedHashSet<>();
		...
		for (PropertyDefinition property : definition.getFields()) {
			...
			switch (property.getIdentifier()) {
				case OPERATIONS:
					operations.addAll(Arrays.asList(SPILT_PATTERN.split(property.getDefaultValue())));
					break;
				case INSTANCE_TYPES:
					// FIXME: this code should be updated because the types here are no longer valid
					for (String string : SPILT_PATTERN.split(property.getDefaultValue())) {
						CollectionUtils.addNonNullValue(instanceTypes, typeProvider.getInstanceClass(string));
					}
					break;
				case CONFIG:
				...
				case ASYNC_SUPPORT:
					...
					break;
				case ON_DEFINITIONS:
					definitions.addAll(Arrays.asList(SPILT_PATTERN.split(property.getDefaultValue())));
					break;
				default:
		...
		instanceRule.setIsAsyncSupported(asyncSupport.booleanValue());
	...
	private InstanceRule instantiateRuleFromDefinition(DefinitionModel definition) {
		for (PropertyDefinition field : definition.getFields()) {
			if (CONFIG.equalsIgnoreCase(field.getName()) && field.getControlDefinition() != null
					&& StringUtils.isNotNullOrEmpty(field.getControlDefinition().getIdentifier())) {
	...
	private static class ActivatingInstanceRule implements InstanceRule {
		...
		public static boolean isActive(InstanceRule rule) {
			if (rule instanceof ActivatingInstanceRule) {
				return ((ActivatingInstanceRule) rule).isActive();
			}
			return true;
		}
