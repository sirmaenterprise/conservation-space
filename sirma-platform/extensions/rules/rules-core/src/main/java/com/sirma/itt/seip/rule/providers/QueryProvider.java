...
public class QueryProvider implements DependencyResolver, Configurable {
	...
	public Iterator<Instance> resolveDependenciesLazily(Instance parent) {
		...
		return new LazyIterator(queryConfig.beforeQueryExecute(arguments), batchLoadSize, loadFullInstances,
				searchService, taskExecutor, serviceRegistry, instanceTypes);
	}
	...
	static class LazyIterator implements Iterator<Instance> {
		 ...
		 */
		@SuppressWarnings("unchecked")
		public <I extends Instance> LazyIterator(SearchArguments<I> arguments, int batchSize, Boolean loadFullInstances,
				SearchService searchService, TaskExecutor taskExecutor, ServiceRegistry serviceRegistry,
		...
		private Collection<Instance> loadElements() {
			...
			List<Instance> found = arguments.getResult();
			if (CollectionUtils.isNotEmpty(found)) {
				if (loadFull.booleanValue()) {
					local = BatchEntityLoader.loadDistinctInstances(found, serviceRegistry, taskExecutor);
				} else {
					local = found;
				}
			}
