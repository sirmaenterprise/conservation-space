...
public class EntityRecognitionConfigBuilder {
	...
	public BaseRuleConfig readConfig(Map<String, Object> config) {
		...
		if (objectTypes instanceof Collection) {
			...
			List<Class<?>> classes = new ArrayList<>(collection.size());
			for (Object type : collection) {
				Class<? extends Instance> instanceClass = typeProvider.getInstanceClass(type.toString());
				CollectionUtils.addNonNullValue(classes, instanceClass);
			}
