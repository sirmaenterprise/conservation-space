/*
 * @(#)DefaultHelpBroker.java 1.69 06/10/30 Copyright (c) 2006 Sun Microsystems, Inc. All Rights Reserved. DO NOT ALTER
 * OR REMOVE COPYRIGHT NOTICES OR THIS FILE HEADER. This code is free software; you can redistribute it and/or modify it
 * under the terms of the GNU General Public License version 2 only, as published by the Free Software Foundation. Sun
 * designates this particular file as subject to the "Classpath" exception as provided by Sun in the LICENSE file that
 * accompanied this code. This code is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without
 * even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License
 * version 2 for more details (a copy is included in the LICENSE file that accompanied this code). You should have
 * received a copy of the GNU General Public License version 2 along with this work; if not, write to the Free Software
 * Foundation, Inc., 51 Franklin St, Fifth Floor, Boston, MA 02110-1301 USA. Please contact Sun Microsystems, Inc., 4150
 * Network Circle, Santa Clara, CA 95054 USA or visit www.sun.com if you need additional information or have any
 * questions.
 */
...
public class FancyHelpBroker implements HelpBroker, KeyListener {
	private static final String ERROR_MISSING_ID = "Missing required argument: id!";
	...
	static {
		try {
			...
			LOGGER.error("Error during look and feel init!", e);
		}
	}
	...
	/**
	 * Set the presentation attributes from a HelpSet.Presentation. The HelpSet.Presentation must be in the current
	 * HelpSet.
	 *
	 * @param hsPres
	 ...
	 *
	 * @return This component's locale. If this component does not have a locale, the defaultLocale is returned.
	 * @see #setLocale
	 */
	@Override
	 ...
	 *
	 * @param l
	 *            The locale to become this component's locale. A null locale is the same as the defaultLocale.
	 * @see #getLocale
	 */
	@Override
	...
	/**
	 * Set the currentView to the navigator with the same name as the <tt>name</tt> parameter.
	 *
	 * @param name
	 *            The name of the navigator to set as the current view. If nav is null or not a valid Navigator in this
	 *            HelpBroker then an IllegalArgumentException is thrown.
	 */
	@Override
	...
	/**
	 * Initializes the presentation. This method allows the presentation to be initialized but not displayed. Typically
	 * this would be done in a separate thread to reduce the intialization time.
	 */
	@Override
	 ...
	 *            The name of a Presentation section from a HelpSet to use. For some Presentations this will also be the
	 *            name to apply to the Presentation.
	 * @see Presentation
	 */
	@Override
	...
	private Presentation getPresentation(String presentation, String presentationName) {
		...
		try {
			...
			throw new UserhelpException(presentation + " presentation is not available in the system!", cnfex);
		} catch (Exception ex) {
			throw new UserhelpException("Error invoking presentation due to :" + ex.getMessage(), ex);
		}
	...
	/**
	 * Shows this ID as content relative to the (top) HelpSet for the HelpBroker instance--HelpVisitListeners are
	 * notified.
	 *
	 * @param id
	 *            A string that identifies the topic to show for the loaded (top) HelpSet
	...
	/**
	 * Enables the Help key on a Component. This method works best when the component is the rootPane of a JFrame in
	 * Swing implementations, or a java.awt.Window (or subclass thereof) in AWT implementations. This method sets the
	 * default helpID and HelpSet for the Component and registers keyboard actions to trap the "Help" keypress. When the
	 * "Help" key is pressed, if the object with the current focus has a helpID, the helpID is displayed. otherwise the
	 * default helpID is displayed.
	 *
	 * @param comp
	 ...
	 * @param hs
	 *            the default HelpSet to be displayed. If hs is null the default HelpSet will be assumed.
	 ...
	 *            the default HelpID to be displayed
	 * @param hs
	 *            the default HelpSet to be displayed. If hs is null the default HelpSet from the HelpBroker is
	 *            assummed. null is not valid if presenation is not null.
	 * @param presentation
	 *            The Presentation class to display the content in. If presentation is null the HelpBroker is used to
	 *            display the content.
	 * @param presentationName
	 *            The name of a Presentation section to control the display of the content. Also for some Presenations
	 *            this will be used to "name" the Presentation.
	 * @see getHelpKeyActionListener
	...
	public void enableHelpKey(Component comp, String id, HelpSet hs, String presentation, String presentationName) {
		if (id == null) {
			throw new UserhelpException(ERROR_MISSING_ID);
		}
	...
	/**
	 * Invoked when a key is typed. This event occurs when a key press is followed by a key release. Not intended to be
	 ...
	 * @param hs
	 *            the HelpSet the id is in. If hs is null the default HelpSet will be assumed.
	 * @see CSH.setHelpID
	 * @see CSH.setHelpSet
	 */
	@Override
	...
	/**
	 * Enables help for a Component. This method sets a Component's helpID and HelpSet and adds an ActionListener. When
	 * an action is performed it displays the Component's helpID and HelpSet in the default viewer.
	 *
	 * @param comp
	 *            the Component to set the id and hs on. If the Component is not a javax.swing.AbstractButton or a
	...
	public void enableHelpOnButton(Component comp, String id, HelpSet hs) {
		if (!(comp instanceof AbstractButton) && !(comp instanceof Button)) {
			throw new UserhelpException(
					"Invalid Component of type: " + (comp != null ? comp.getClass().getSimpleName() : "null"));
		}
		if (id == null) {
			throw new UserhelpException(ERROR_MISSING_ID);
		}
	...
	/**
	 * Enables help for a MenuItem. This method sets a Component's helpID and HelpSet and adds an ActionListener. When
	 * an action is performed it displays the Component's helpID and HelpSet in the default viewer.
	 *
	 * @param comp
	 *            the MenuItem to set the id and hs on. If comp is null an IllegalAgrumentException is thrown.
	 * @param id
	 *            the String value of an Map.ID.
	 * @param hs
	 *            the HelpSet the id is in. If hs is null the default HelpSet will be assumed.
	...
	public void enableHelpOnButton(MenuItem comp, String id, HelpSet hs) {
		if (comp == null) {
			throw new UserhelpException("Invalid Component: null!");
		}
		if (id == null) {
			throw new UserhelpException(ERROR_MISSING_ID);
		}
	...
	public void enableHelpOnButton(Object obj, String id, HelpSet hs, String presentation, String presentationName) {
		if (!(obj instanceof AbstractButton) && !(obj instanceof Button) && !(obj instanceof MenuItem)) {
			throw new UserhelpException(
					"Invalid object of type: " + (obj != null ? obj.getClass().getSimpleName() : "null"));
		}
		if (id == null) {
			throw new UserhelpException(ERROR_MISSING_ID);
		}
		...
		attachListnersToHelpButton(obj, hs, presentation, presentationName);

	}

	private void attachListnersToHelpButton(Object obj, HelpSet hs, String presentation, String presentationName) {
		if (presentation == null) {
	...
	/**
	 * Set the activation window from given Component or MenuItem. It find Window component in the component tree from
	 * given Component or MenuItem end call
	 *
	 * <pre>
	 * setActivationWindow
	 * </pre>
	...
	/**
	 * Set the activation window. If the window is an instance of a Dialog and the is modal, modallyActivated help is
	 * set to true and ownerDialog is set to the window. In all other instances modallyActivated is set to false and
	 * ownerDialog is set to null.
	 *
	 * @param window
