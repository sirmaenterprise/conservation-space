...
public class TimedScheduleTriggerTest {
	...
	@Spy
	private ContextualConcurrentMap<String, SchedulerTask> archivedTasks = ContextualConcurrentMap.create();
	@Spy
	...
	public void testScheduleCoreTasks() throws Exception {
		...
		timedTrigger.checkAndScheduleCoreTasks();
	...
	@Test
	public void onNonTimedTaskAdded() throws Exception {

		SchedulerEntry entry = createEntry(4L);

		when(entry.getConfiguration().getType()).thenReturn(SchedulerEntryType.CRON);

		timedTrigger.onTaskAdded(new SchedulerEntryAddedEvent(entry));
	...
	public void onEntryChange() throws Exception {
		...
		try {
			timedTrigger.onTaskUpdated(new SchedulerEntryUpdatedEvent(entry, null));
			timedTrigger.onTaskUpdated(new SchedulerEntryUpdatedEvent(entry, null));
			Collection<SchedulerTask> archiveTasks = timedTrigger.getArchiveTasks();
			assertNotNull(archiveTasks);
			assertFalse(archiveTasks.isEmpty());
		} finally {
			timedTrigger.onShutdown();
		}
	}
	...
	public void maxElementsInGroup() throws Exception {
		...
		when(mainSchedulerTrigger.scheduleMainChecker(any(), anyLong(), any())).then(a -> mock(Future.class));
	...
	@Test
	public void onNonTimedTaskUpdate() throws Exception {
		SchedulerEntry entry = createEntry(11L);
		when(entry.getConfiguration().getType()).thenReturn(SchedulerEntryType.CRON);

		timedTrigger.onTaskUpdated(new SchedulerEntryUpdatedEvent(entry, null));
		verify(scheduledExecutor, never()).submit(any(), anyLong(), any());
	}
