...
public class CronScheduleTriggerTest {
	 ...
	 */
	@Test
	public void testOnCronTaskUpdateCronTask() {
		//setup test
		SchedulerEntryUpdatedEvent entryUpdated = Mockito.mock(SchedulerEntryUpdatedEvent.class);
		...
		when(entryUpdated.getNewEntry()).thenReturn(newEntry);
		when(newEntry.getConfiguration()).thenReturn(newEntryConfiguration);


		SchedulerEntry oldEntry = Mockito.mock(SchedulerEntry.class);
		when(entryUpdated.getOldEntry()).thenReturn(oldEntry);


		//execution of tested method
		cronTrigger.onCronTaskUpdate(entryUpdated);

		//verification
		verify(transactionSupport).invokeOnSuccessfulTransaction(any());
	}

	@Test
	public void testOnUpdateEvent_Completed_None_Running() {
		SchedulerEntry newEntry = Mockito.mock(SchedulerEntry.class);
		...
		SchedulerEntry oldEntry = Mockito.mock(SchedulerEntry.class);
		when(oldEntry.getIdentifier()).thenReturn("testId");
		SecurityContextManager securityContextManager = new SecurityContextManagerFake();
		SecureExecutor executor = new SecureExecutor(securityContextManager);
		SchedulerEntryUpdatedEvent entryUpdated = new SchedulerEntryUpdatedEvent(newEntry, oldEntry, executor);
	...
	@Test
	public void testOnUpdateEvent_Completed_Old_Cancel() {
		SchedulerEntry newEntry = Mockito.mock(SchedulerEntry.class);
		when(newEntry.getStatus()).thenReturn(SchedulerEntryStatus.COMPLETED);
		SchedulerConfiguration newEntryConfiguration = Mockito.mock(SchedulerConfiguration.class);
		when(newEntryConfiguration.getType()).thenReturn(SchedulerEntryType.CRON);
		...
		SecurityContextManager securityContextManager = new SecurityContextManagerFake();
		SecureExecutor executor = new SecureExecutor(securityContextManager);
		SchedulerEntryUpdatedEvent test = new SchedulerEntryUpdatedEvent(newEntry, oldEntry, executor);

		//execution of tested method
		cronTrigger.onCronTaskUpdate(test);
	...
	@Test
	public void testOnUpdateEvent_Running() {
		SchedulerEntry newEntry = Mockito.mock(SchedulerEntry.class);
		when(newEntry.getStatus()).thenReturn(SchedulerEntryStatus.RUNNING);
		SchedulerConfiguration newEntryConfiguration = Mockito.mock(SchedulerConfiguration.class);
		when(newEntryConfiguration.getType()).thenReturn(SchedulerEntryType.CRON);
		...
		//verification
		verify(transactionSupport).invokeOnSuccessfulTransaction(any());
		verify(runningTasks, never()).remove(anyString());
	}
	
	/**
	 * Test method onCronTaskUpdate scenario with not cron timer.
	 */
	@Test
	public void testOnCronTaskUpdateNotCronTask() {
		...
		when(entryUpdated.getNewEntry()).thenReturn(newEntry);
		when(newEntry.getConfiguration()).thenReturn(newEntryConfiguration);
		when(newEntryConfiguration.getType()).thenReturn(SchedulerEntryType.TIMED);
		
		//execution of tested method
		cronTrigger.onCronTaskUpdate(entryUpdated);
		
		//verification
		verify(transactionSupport, never()).invokeOnSuccessfulTransaction(any());
	}
	...
	public void testScheduleCoreTasks() throws Exception {
		...
		verify(dbDao, times(2)).getTasksForExecution(anySet(), anySet(), anyLong(), any(), any());
	...
	public void testScheduleTenantTasks_canceled() throws Exception {
		...
		verify(dbDao, times(2)).getTasksForExecution(anySet(), anySet(), anyLong(), any(), any());
		verify(schedulerStore, atLeast(6)).saveChanges(any());
	}

	@Test
	public void onNonCronTaskAdded() throws Exception {
		SchedulerEntry entry = createEntry(4L);

		cronTrigger.onCronTaskAdded(new SchedulerEntryAddedEvent(entry));

		when(entry.getConfiguration().getType()).thenReturn(SchedulerEntryType.TIMED);
	...
	public void onCronTaskAdded_RemoveCompleted() throws Exception {
		...
		mockRunTaskSuccessfully();
		SchedulerEntryAddedEvent schedulerEntryAddedEvent = new SchedulerEntryAddedEvent(entry);
		cronTrigger.onCronTaskAdded(schedulerEntryAddedEvent);
