...
public class SchedulerServiceImpl implements SchedulerService {
	...
	private SchedulerEntry notifyForNewEntry(SchedulerEntry schedulerEntry) {
		eventService.fire(new SchedulerEntryAddedEvent(schedulerEntry));
	 ...
	 */
	protected Integer convertToClassIndex(Class<?> clazz) {
	 ...
	 * @return the class
	 */
	protected Class<?> convertToClass(int classIndex) {
		Class<?> registeredClass = kryoHelper.getRegisteredClass(Integer.valueOf(classIndex));
	...
	protected Class<? extends SchedulerAction> convertToActionClass(Integer classIndex) {
		if (classIndex != null) {
			return (Class<? extends SchedulerAction>) convertToClass(classIndex.intValue());
		}
		return null;
	}
	...
	private static EventTrigger buildEventTrigger(EmfEvent event) {
		...
		if (instance instanceof Instance) {
			...
			targetId = ((InstanceReference) instance).getIdentifier();
		}
	...
	public SchedulerEntry save(SchedulerEntry entry) {
		...
		SchedulerEntry oldEntry = schedulerStore.saveChanges(entry);
		if (oldEntry != null) {
	...
	public SchedulerEntry activate(Long entryId) {
		...
		updateSchedulerEntryState(entity, SchedulerEntryStatus.RUNNING);
		return convertToScheduleEntry(entity, true, false);
	}
