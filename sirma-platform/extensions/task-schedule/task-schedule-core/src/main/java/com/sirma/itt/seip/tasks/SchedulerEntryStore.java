...
import com.sirma.itt.seip.instance.dao.BatchEntityLoader.BatchPrimaryKeyEntityLoaderCallback;
...
class SchedulerEntryStore {
	 ...
	 * @param entry
	 *            the entry to save
	 * @return the old copy of the known data before the saving the changes
	 */
	@SuppressWarnings("boxing")
	SchedulerEntry saveChanges(SchedulerEntry entry) {
		SchedulerEntry oldEntry = null;
		SchedulerEntity currentEntity = findById(entry.getId());
		if (currentEntity != null) {
			oldEntry = currentEntity.toSchedulerEntry(serializationHelper);
