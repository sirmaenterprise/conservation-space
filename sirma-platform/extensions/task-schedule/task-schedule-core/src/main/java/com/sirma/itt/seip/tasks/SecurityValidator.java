...
class SecurityValidator {
	...
	void onNewEntry(SchedulerConfiguration configuration) {
		if (!securityContext.isSystemTenant() && configuration.getRunAs() == RunAs.ALL_TENANTS) {
			LOGGER.error(CANNOT_ACCESS_OTHER_TENANTS_FROM_A_TENANT_CONTEXT + configuration.getIdentifier());
			throw new SecurityException(
					CANNOT_ACCESS_OTHER_TENANTS_FROM_A_TENANT_CONTEXT + configuration.getIdentifier());
		}
	}

	void checkOnSave(SchedulerEntry entry) {
		if (!securityContext.isSystemTenant() && entry.getConfiguration().getRunAs() == RunAs.ALL_TENANTS) {
			LOGGER.error(CANNOT_ACCESS_OTHER_TENANTS_FROM_A_TENANT_CONTEXT + entry.getIdentifier());
			throw new SecurityException(CANNOT_ACCESS_OTHER_TENANTS_FROM_A_TENANT_CONTEXT + entry.getIdentifier());
		}
