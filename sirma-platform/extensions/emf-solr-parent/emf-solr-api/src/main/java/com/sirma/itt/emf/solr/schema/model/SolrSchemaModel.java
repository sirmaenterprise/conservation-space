...
public class SolrSchemaModel {
	...
	public SolrSchemaModel diff(SolrSchemaModel fromModel) throws SolrSchemaException {
		Map<String, SchemaEntry> fromEntries = new LinkedHashMap<String, SchemaEntry>(fromModel.entries);
		...
		for (Entry<String, SchemaEntry> schemaEntry : entries.entrySet()) {
			...
			if (!fromEntries.containsKey(key)) {
				newModel.addEntry(value.clone(SolrSchemaMode.ADD));
			}
		}
		// add the replaced
		for (Entry<String, SchemaEntry> schemaEntry : entries.entrySet()) {
			...
			if (fromEntries.containsKey(key)) {
				...
				if (!sameEntry(value, oldEntry)) {
					newModel.addEntry(value.clone(SolrSchemaMode.REPLACE));
				}
			}
		}
		// now add the removed
		for (SchemaEntry schemaEntry : fromEntries.values()) {
			newModel.addEntry(schemaEntry.clone(SolrSchemaMode.DELETE));
		}
		return newModel;
	}
	...
	public String build() {
		...
		while (iterator.hasNext()) {
			...
			if (StringUtils.isNotNullOrEmpty(nextEntry)) {
				if (builder.length() > 1) {
					builder.append(",");
				}
