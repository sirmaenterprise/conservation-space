...
public class WorkflowUtil {
	 ...
	 * @throws AlfrescoRuntimeException the alfresco runtime exception
	 */
	public static void approve(final NodeRef ref, final NodeService nodeService,
			final CopyService copyService) throws AlfrescoRuntimeException {
		Node docNode = new Node(ref);

		if (docNode.hasAspect(ApplicationModel.ASPECT_SIMPLE_WORKFLOW) == false) {
			throw new AlfrescoRuntimeException(
					"Cannot approve a node that is not part of a workflow.");
		}
		...
		NodeRef approveFolder = (NodeRef) props
				.get(ApplicationModel.PROP_APPROVE_FOLDER.toString());
	...
	public static void reject(NodeRef ref, NodeService nodeService, CopyService copyService)
		...
		if (docNode.hasAspect(ApplicationModel.ASPECT_SIMPLE_WORKFLOW) == false) {
			throw new AlfrescoRuntimeException(
					"Cannot reject a node that is not part of a workflow.");
		}
