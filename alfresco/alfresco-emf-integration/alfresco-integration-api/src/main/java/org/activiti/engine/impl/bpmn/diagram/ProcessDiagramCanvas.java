...
/**
 * Represents a canvas on which BPMN 2.0 constructs can be drawn. Some of the icons used are
 * licenced under a Creative Commons Attribution 2.5 License, see
...
public class ProcessDiagramCanvas {
	...
	/** The signal catch image. */
	private static BufferedImage SIGNAL_CATCH_IMAGE;

	/** The signal throw image. */
	private static BufferedImage SIGNAL_THROW_IMAGE;

	/** The business rule task image. */
	private static BufferedImage BUSINESS_RULE_TASK_IMAGE;
	...
	/**
	 * Creates an empty canvas with given width and height. Allows to specify minimal boundaries on
	 * the left and upper side of the canvas. This is useful for diagrams that have white space
	 ...
	 *            the height
	 * @param minX
	 *            Hint that will be used when generating the image. Parts that fall below minX on
	 ...
	 * @param width the width
	 * @param height the height
	 * @param isTriggeredByEvent the is triggered by event
	 */
	 ...
	 *            the multi instance sequential
	 * @param multiInstanceParallel
	 *            the multi instance parallel
	 * @param collapsed
	 *            the collapsed
	 */
