...
public class ProcessDiagramGenerator {
  ...
  /** The Constant activityDrawInstructions. */
   ...
   *
   * @param processDefinition the process definition
   * @param highLightedActivities the high lighted activities
   * @return the process diagram canvas
   */
   ...
   * Draw activity.
   *
   * @param processDiagramCanvas the process diagram canvas
  ...
  protected static void drawActivity(ProcessDiagramCanvas processDiagramCanvas, ActivityImpl activity, List<String> highLightedActivities) {
    ...
    FontMetrics fontMetrics =processDiagramCanvas.g.getFontMetrics();
    ...
    for (PvmTransition sequenceFlow : activity.getOutgoingTransitions()) {
      ...
      for (int i = 2; i < waypoints.size(); i += 2) { // waypoints.size()
        ...
        if (i < waypoints.size() - 2) {
          ...
          String name = (String) sequenceFlow.getProperty("name");
          if (name!=null){
        	  int x = Math.min(waypoints.get(i - 2),waypoints.get(i));
        	  int y = Math.max(waypoints.get(i - 1),waypoints.get(i+1));
        	  int width = Math.abs(waypoints.get(i - 2)-waypoints.get(i));
        	  int height = Math.abs(waypoints.get(i - 1)-waypoints.get(i+1));
        	  List<String> rows = drawMultilineString(fontMetrics, name,  x,  y, width, height);
        	  int padding=2;
        	  if (rows != null) {
        		  int heightFont = fontMetrics.getHeight();
        		  if (height<heightFont){
        			  height = heightFont * rows.size() +padding;
        			  y = y-height-padding;
        		  }
        		  ...
        		  for (int j = 0; j < rows.size(); j++) {
        			  ...
        			  int textY = y + size + heightFont * (j + 1);
        			  processDiagramCanvas.g.drawString(row, textX, textY);
        		  }
        	  }
//        	processDiagramCanvas.g.drawString(textToWidth, waypoints.get(i - 2)+5, waypoints.get(i - 1)+10);
          }
        }
      }
    }
   ...
   * @return the list
   */
  private static List<String> drawMultilineString(FontMetrics fontMetrics,  String name, int x, int y, int width, int height) {
		// text'
		if (name != null) {
			...
			for (String data : split) {
				...
				if (stringWidth < maxWidth||tempString.length()==0) {
  ...
  /**
   * Inits the process diagram canvas.
   *
   * @param processDefinition the process definition
   * @return the process diagram canvas
   */
  ...
  /**
   * The Interface ActivityDrawInstruction.
   */
  protected interface ActivityDrawInstruction {
     ...
     * Draw.
     *
     * @param processDiagramCreator the process diagram creator
