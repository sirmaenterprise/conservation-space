...
<#escape x as jsonUtils.encodeJSONString(x)>
<#if propertiesMap??>
	<#assign nodeProperies=propertiesMap[row.nodeRef?string]>
<#else>
	<#assign nodeProperies=versionProperties>
</#if>
"type": "${row.typeShort}",
...
"nodeRef": "${row.nodeRef}",
"properties": <@propertiesJSON node=row properties=nodeProperies />
...
<#list properties?keys as key>
		"${key}":<#if properties[key]??><@singlePropertiesJSON properties[key]/><#else>null</#if><#if (key_has_next)>,</#if>
</#list>
<#if (properties?size>0)>,</#if>"nodeRef":"${node.nodeRef}"
}
</#macro>

<#macro singlePropertiesJSON val>
	 <#if val?is_boolean == true>${val?string}<#elseif val?is_date == true>"${xmldate(val)}"<#elseif val?is_number == true>${val?c}<#elseif val?is_sequence>[<#list val as element><@singlePropertiesJSON element/><#if (element_has_next)>,</#if></#list>]<#else>"${shortQName(val?js_string)}"</#if>
</#macro>
