   ...
   try
   {
      ...
      for each (field in formdata.fields)
      {
         switch (String(field.name).toLowerCase())
         {
            ...
            case "aspects":
               aspects = field.value != "-" ? field.value.replaceAll("\\s+","").split(",") : [];
               break;
            case "properties":
            	properties = field.value != "-" ?  jsonUtils.toObject(field.value) : {};
                break;
      ...
      if ((filename === null || content === null) || (destination === null && (siteId === null || containerId === null) && updateNodeRef === null && uploadDirectory==null))
      {
      ...
      else
      {
         ...
         {
            var destNodeLocal = search.selectNodes(uploadDirectory);
            if (destNodeLocal === null ||destNodeLocal.length!=1)
            {
               exitUpload(404, "Cannot upload file since upload directory '" + uploadDirectory + "' does not exist.");
               return;
            }
            destNode = destNodeLocal[0];
         }
         /**
         ...
         if (existingFile !== null)
         {
            ...
            if (existingFile.hasAspect("cm:versionable") && overwrite)
            {
               ...
               if (aspects.length > 0)
               {
                  ...
                  {
                	  existingFile.addAspect(aspects[i]);
                  }
               }
               //custom - add properties
               if (properties){
              	 for (var key in properties) {
              		existingFile.properties[key] = properties[key];
              	}
               }
               existingFile.save();
...
//         newFile.properties = properties;
         ...
         if (properties){
        	 for (var key in properties) {
        		  newFile.properties[key] = properties[key];
        	}
         }
