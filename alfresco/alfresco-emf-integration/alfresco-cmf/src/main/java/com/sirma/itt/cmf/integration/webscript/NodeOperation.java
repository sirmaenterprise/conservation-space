...
public class NodeOperation extends BaseFormScript {
		...
		try {
			...
			if (serverPath.contains("/cmf/node/update")) {
				...
				if (request.has(KEY_NODEID) && request.has(KEY_REFERENCE_ID)) {
					...
					String assocName = request.has(KEY_CHILD_ASSOC_NAME) ? request
							.getString(KEY_CHILD_ASSOC_NAME) : "child";
					ChildAssociationRef createdAssoc = nodeService.addChild(updateable, reference,
							CMFModel.ASSOC_CHILD_PARENT_REF,
							QName.createQName(CMFModel.CMF_MODEL_1_0_URI, assocName));
	...
	private NodeRef createFolderRequest(JSONObject request) throws Exception {
		...
		if (type == null) {
			LOGGER.warn("Falling back to default type for folder");
		...
		if (properties.get(ContentModel.PROP_NAME) == null) {
			...
			String caseName = properties.get(CMFModel.PROP_IDENTIFIER) != null ? properties.get(
					CMFModel.PROP_IDENTIFIER).toString() : "folder_" + GUID.generate();
		...
		getOwnableService().setOwner(createdFolder, AuthenticationUtil.getSystemUserName());
	...
	private Collection<QName> toAspectList(JSONArray jsonArray) throws Exception {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			if (resolvedToQName != null) {
				boolean contained = getDataDictionaryService().getAllAspects().contains(
						resolvedToQName);
				if (contained) {
