...
public class CodeValueServiceImplTest {
	...
	public void updateCodeValueDescriptions() throws CodeListException {
		...
		criteria.setCodeListId("227");
		criteria.setIds(Arrays.asList("TSTYPE09"));
		CodeValue retrievedCodeValue = (CodeValue) service.getCodeValues(criteria).getResults()
				.get(0);
	...
	/**
	 * Tests the retrieval of specific code values by creating a complex criteria with code list
	 * value "227", extra 2 that begins with "qertyu1*" and code values that begins with "tstype"
	 * and ends with "1".
	 */
	@Test
	...
	/**
	 * Tests the correct retrieving of code values without date range criteria criteria for code
	 * list "1".
	 */
	@Test
	...
	/**
	 * Test the correct retrieving of code values with validity start date criteria for code list
	 * "1".
	 */
	@Test
	...
	/**
	 * Test the correct retrieving of code values with validity start date and end date criteria for
	 * code list "1".
	 */
	@Test
	...
	/**
	 * Test the correct retrieving of code values with an offset. Test for correct total number of
	 * items and correct results size.
	 */
	@Test
	...
	/**
	 * Test the correct retrieving of code values with an offset which will load the last page. Test
	 * for correct total number of items and correct results size.
	 */
	@Test
	...
	/**
	 * Test the correct retrieving of code values with an offset and limit. Test for correct total
	...
	/**
	 * Gets the queries out of the sesion's statistics and checks for the presence of any joins in
	 * the first query.
	 * 
	 * @param st
	 *            the statistics
	...
	/**
	 * Imitates the low level behavior of {@link InstanceDao#loadInstancesByDbKey()}. Extracts
	 * {@link CodeList} from a list where all {@link CodeList} are kept after persisting into the
	 * test database.
	 * 
	 * @param ids
	...
	private List<CodeList> getCodeLists(List<Long> ids) {
		if (ids.size() != 0) {
			TypedQuery<CodeList> query = em.createQuery(
					"select cl from CodeList cl where cl.id in :ids", CodeList.class);
			query.setParameter("ids", ids);

			return query.getResultList();
		}
		List<CodeList> emptyList = new ArrayList<CodeList>();
		return emptyList;
	}

	/**
	 * Imitates the low level behavior of {@link InstanceDao#loadInstancesByDbKey()}. Extracts
	 * {@link CodeValue} from a list where all {@link CodeValue} are kept after persisting into the
	 * test database.
	 * 
	 * @param ids
	...
	private List<CodeValue> getCodeValues(List<Long> ids) {
		if (ids.size() != 0) {
			TypedQuery<CodeValue> query = em.createQuery(
					"select cv from CodeValue cv where cv.id in :ids", CodeValue.class);
			query.setParameter("ids", ids);
			return query.getResultList();
		}
		List<CodeValue> emptyList = new ArrayList<CodeValue>();
		return emptyList;
	}

	/**
	 * Overrides the {@link InstanceDao#loadInstancesByDbKey()} mock's method to return list of
	 * {@link CodeList} corresponding to the provided list of IDs.
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private void setupMock() {
		EasyMock.expect(
				codeListInstanceDao.loadInstancesByDbKey(EasyMock.anyObject(List.class),
						EasyMock.anyBoolean())).andAnswer(new IAnswer<List>() {
			@Override
		...
		EasyMock.expect(codeValueInstanceDao.loadInstancesByDbKey(EasyMock.anyObject(List.class)))
				.andAnswer(new IAnswer<List>() {
					@Override
					public List answer() throws Throwable {
						List<Long> ids = (List<Long>) EasyMock.getCurrentArguments()[0];
		...
		EasyMock.expect(codeValueInstanceDao.saveEntity(EasyMock.anyObject(Entity.class)))
				.andAnswer(new IAnswer<Entity>() {
					@Override
					public Entity answer() throws Throwable {
						...
						return (Entity) EasyMock.getCurrentArguments()[0];
					}
				}).anyTimes();
	...
	/**
	 * Fills the database with valid test code lists and values. They are retrieved as input stream
	 * and then validated and parsed. Finally they are persisted in the database in one transaction.
	 * 
	 * @throws Exception
	...
	private void loadDataBase() throws Exception {
		...
		final List<CodeList> testCodeLists = sheetParser.parseXLS(codeListSheet);
