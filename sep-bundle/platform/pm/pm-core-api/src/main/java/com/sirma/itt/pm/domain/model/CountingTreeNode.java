...
/**
 * Tree node that counts and keeps track for the number of his child nodes and the level where it's
 * placed.
 * 
 * @param <E>
 *            the data type
 * @author BBonev
 */
public class CountingTreeNode<E extends Serializable> implements Entity<Long>, Node, Serializable,
		PathElement {
	...
	/** The children. */
	private List<CountingTreeNode<E>> children = new ArrayList<CountingTreeNode<E>>();

	/**
	 * The children snapshot returned from the getter method for children property. This is used to
	 * force the user to use the custom add and remove methods instead of the collection methods
	 */
	...
	public Node getChild(String name) {
		return PathHelper.find(children, name);
	}
	...
	/**
	 * Returns a snapshot of the children. The list cannot be modified!!!. To add or remove elements
	 * call the methods {@link #addChild(CountingTreeNode)} and
	 * {@link #removeChild(CountingTreeNode)}.
	 * 
	 * @return the children
	 */
	 ...
	 * @return the integer
	 */
	public Integer addChild(CountingTreeNode<E> countingTreeNode) {
	 ...
	 * @return the integer
	 */
	public Integer removeChild(CountingTreeNode<E> countingTreeNode) {
	 ...
	 *            the diff
	 */
	protected void updateChildrenCount(boolean isAdd, Integer diff) {
		if (isAdd) {
			childCount += diff;
