...
/**
 * UserConverter used for the preview field in picklist component to convert between userId and
 * displayName.
 * 
 * @author svelikov
 */
...
public class MultiItemPreviewConverter implements Converter {
	...
	/**
	 * Find out the hidden field in picklist wrapper and return its value instead of that passed for
	...
	private String getDisplayValueForUser(Object value) {
		...
		if (StringUtils.isNotNullOrEmpty(passedValue)) {
			Resource resource = resourceService.getResource(passedValue,
					getTypeByResourceId(passedValue));
			if (resource != null) {
	...
	/**
	 * Gets the resource type by given identifier.
	 * 
	 * @param resourceId
	 *            resource identifier
	 * @return resource type
	 */
	private ResourceType getTypeByResourceId(String resourceId) {
		if (resourceId.startsWith(ResourceType.GROUP.toString())) {
