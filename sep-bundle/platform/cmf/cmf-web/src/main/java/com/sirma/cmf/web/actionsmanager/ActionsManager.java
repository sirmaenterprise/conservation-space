...
public class ActionsManager extends com.sirma.cmf.web.Action implements Serializable {
	...
	public String executeAllowedAction(Action action, Instance instance) {
		if ((instance != null) && (action != null)) {
			...
			Instance freshInstance = fetchInstance(instance.getId(), instance.getClass()
			...
			// Update instance in context if already there.
			// On dashboards for example we may not have the instance for which this action is
			// executed in document context.
			// For deleted instances, we don't want to re-apply them in the context again.
	...
	private void initContextForImmediateAction(Instance instance, Instance updatedInstance) {
		if ((instance != null) && (updatedInstance != null)) {
			if (updatedInstance instanceof RootInstanceContext) {
				// add root instance for the root instance header
				getDocumentContext().updateRootInstance(updatedInstance);
				getDocumentContext().updateContextInstance(updatedInstance);
				getDocumentContext().updateInstance(updatedInstance);
			} else {
