		...
		if (!plugin.options.preview) {
			...
			if(plugin.options.header) {
				...
				if (plugin.options.mode === 'objects') {
					panel.add(plugin.getPropertySelector(plugin.filterRelationsHandler, plugin.options.labels.filterByType, 'relationTypesFilter'));
					panel.add({
		    	           xtype          : 'textfield',
...
//		    	           labelWidth	  : 0,
				...
				if (plugin.options.mode === 'objects') {
					btns.push(plugin.getPropertySelector(plugin.filterRelationsHandler, plugin.options.labels.filterByType, 'relationTypesFilter'));
					btns.push({
	    	            xtype          : 'textfield',
	    	            inputId		   : 'objectTypeValueFilter',
	    	            flex		   : 1,
...
//	    	            labelWidth	   : 0,
	...
	plugin.getRelationsStore = function() {
		...
		var url = opts.contextPath + opts.service.loadData +
				'?displayEmpty=' + opts.displayEmpty +
				'&id=' + opts.instanceId +
		...
		if (plugin.options.mode === 'literals') {
			...
			if(plugin.options.instanceId == EMF.documentContext.currentInstanceId) {
...
//		    	}
            ...
            if (record) {
	            ...
	            if (plugin.options.mode === 'objects') {
	                ...
	                if(!plugin.options.btnIds.editorId) {
						...
						plugin.options.btnIds.editor = EMF.extJsComponents.getFieldEditor(plugin.options, record.raw, { id: plugin.options.btnIds.editorId, margin: '13 10 10 10' }, plugin.fieldValueChangeHandler);
	...
	return this.each(function() {
		...
		plugin.propertyEditor = new Ext.form.Panel({
			...
			items	: [
			    plugin.getPropertySelector(plugin.changeRelationTypeHandler, plugin.options.labels.propertySelector, "relations-type-selector-" + EMF.util.generateUUID())
			],
		    defaults: {
		    	labelAlign	: 'top'
		    }
