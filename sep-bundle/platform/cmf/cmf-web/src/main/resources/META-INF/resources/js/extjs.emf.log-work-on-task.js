$.fn.extjsLogWorkOnTask = function(opts) {
	        ...
	        if (form.isValid()) {
	        	if (active) {
	        		...
	        		if(workLogUserId != undefined && currentUserId != workLogUserId){
	        			var descriptionTemplate = "Work logged by user : description";
			...
			columns.push(
					{
						...
						items       : [{
							...
							handler: function(grid, rowIndex, colIndex) {
								var rec = grid.getStore().getAt(rowIndex);
								if (rec.data['delete']) {
									var message = 'Selected record will be permanently deleted.<br>Do you want to continue?';
									EMF.dialog.open({
										title       : '',
										message     : message,
										okBtn       : 'Ok',
	...
	/**
	 * Opens a popup dialog to create/edit and save logged work record.
	 */
