...
;(function ( $, window, document, undefined ) {
  ...
  Picklist.prototype.bindHandlers = function(opts) {
    ...
    filterField.keypress(function(event) { return event.keyCode != 13; });
    ...
    function filterList() {
    	...
    	/* Checks is there are selected items and removed them from the filtered list. 
    	 * This is necessary, because when the filter is used to find users the passed 
    	 * list must contain all users, but when the given user is already selected, he 
    	 * must not present in the filtered list.<b> This prevents from selecting already 
    	 * selected user.</b>
    	 */
    	var selectedItems = getSelectedItems();
    ...
    /**
     * Two level data syncronization. 
     *  1. from container data to hidden input
     *  2. from hidden input to container
     */
    ...
    /**
     * Method that will merge selected elements from the picker to 
     * safe place and prevent loosing data on form refresh.
     * 
     * @param keyID array with user keys
     * @param valueID array with user labels
     */
    ...
    /**
     * Method that will be used for retrieving and moving UI elements from left picker box
     * to the right.
     */
