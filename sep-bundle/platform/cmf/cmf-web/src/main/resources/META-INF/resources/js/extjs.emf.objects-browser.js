...
EMF.CMF.objectsExplorer = {
	...
	config : {
		...
		rootItemsLimit		: 30,		// Number of items to be loaded in the roots selector dropdown.
		rootType			: 'caseinstance',	// Default root type if not specified.
		minSignsForSearch	: 2,		// The minimum signs required to be entered before the roots selector to trigger remote loading.
		...
		autoExpandAll		: false,	// TODO: not implemented yet - should expand the whole tree.
										...
										// This is used in combination with 'allowSelection'. If allowSelection=false, then no filtering is applied.	
		filters				: null,//"(instanceType=='sectioninstance'&&purpose==null)||(instanceType=='documentinstance'&&purpose=='iDoc')||(instanceType=='objectinstance')",
		...
		panelWidth			: '100%',	// The width of the panel that wraps the tree. Can be number or percent.
		rootSelectorWidth	: 'auto',	// The width of the roots combo box.
		panelHeight			: null,		// If not set, then height is not fixed.
		tooltipsEnabled		: true,		// If tooltips should be enabled.
		rootNodeText		: null,		// Default root node text.
		rootInstanceHeader	: null,
		rootNodeTooltip		: 'Root',	// Default root node tooltip.
		rootExpanded		: true,		// If the root node should be expanded by default.
		rootSelectorId		: 'objectBrowserRootSelector',	// The id of the combo box for root selection.
		renderSelectorLabel	: true,		// If there are should be a label in from of the root selector combo box.
		rootSelectorLabel	: 'Browse Projects: ',	// Default label rendered in front of the root selector combo box. 
		rootSelectorWidth	: 400,		// Default width of the root selector dropdown menu.
		autoExpandInterval	: 150,		// Time interval for checking before next search for given node to be executed.
		autoExpandIntervalCount: 20,	// Number of times to be repeated searching for given node in the DOM when expanding the tree.
		rootVisible			: true,		// If the root node should be visible or not.
	...
	init : function(opts) {
		...
		// When the browser is loaded with no context, then we should allow the plugin to be initialized
//		if (!config.node) {
...
//		}
	...
	loadRoots: function(config) {
		...
		if (config.renderSelectorLabel) {
			$('<label for="' + config.rootSelectorId + '" class="ob-root-selector-label">' + config.rootSelectorLabel + '</label>').appendTo(container);
		}
		var rootSelector = $('<input type="hidden" id="' + config.rootSelectorId + '" data-select-name="rootSelector" class="ob-root-selector" />').appendTo(container);
...
//							var data = theStore.proxy.reader.jsonData;
		...
		function createPanel() {
			...
			listeners.cellclick = function( _this, td, cellIndex, record, tr, rowIndex, e, eOpts ) {
				...
				listeners.checkchange = function(node, checked, eOpts) {
					...
					if (container.length > 0) {
						...
						Ext.Array.each(records, function(rec) {
							...
							var data = {
									// TODO: check where is used and remove this property - dbId should be used instead
									nodeId	: rec.raw.dbId,
									dbId	: rec.raw.dbId,
									path	: rec.data.id,
