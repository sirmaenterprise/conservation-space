 ...
 */
@ApplicationScoped
@RoleEvaluatorType(ObjectTypesObject.SAVED_FILTER)
@Extension(target = RoleEvaluator.TARGET_NAME, order = 95)
public class SavedFilterRoleEvaluator extends BaseRoleEvaluator<SavedFilter> {

	/** The Constant SUPPORTED. */
	private static final List<Class<?>> SUPPORTED = Arrays
			.asList(new Class<?>[] { SavedFilter.class });
	 ...
	 */
	@Override
	protected String getContainer(SavedFilter target) {
	 ...
	 * {@inheritDoc}
	 */
	protected Class<SavedFilter> allowedClass() {
	 ...
	 */
	@Override
	protected Pair<Role, RoleEvaluator<SavedFilter>> evaluateInternal(SavedFilter target,
	 ...
	 */
	@Override
	protected Boolean filterInternal(SavedFilter target, Resource resource, Role role,
			Set<Action> actions) {
