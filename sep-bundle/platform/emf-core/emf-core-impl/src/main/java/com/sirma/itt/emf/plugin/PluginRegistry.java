...
public class PluginRegistry {
	...
	/**
	 * Fetches all {@link Plugin} classes that are annotated with @Extension, performs a merging
	 * logic, stores them and.
	 */
	@PostConstruct
	public void init() {
		beans = new HashMap<>();
		cachedPlugins = new HashMap<>();
		...
		for (Bean<?> bean : pluginBeans) {
			Extension metadata = bean.getBeanClass().getAnnotation(Extension.class);
			...
			if (sameOrderPlugin != null) {
				...
				if (metadata.priority() == sameOrderMetadata.priority()) {
					throw new IllegalStateException(
							"There are two extensions with the same order and priority - '"
									+ bean.getBeanClass().getName() + "' and '"
									+ sameOrderPlugin.getBeanClass().getName() + "'");
		...
		beanManager.fireEvent(new AfterPluginRegistryInitializedEvent(beans),
	...
	/**
	 * Provides a list of all plugins for a specific extension point. If the entries in this list
	 * are changed, the available list plugins for the particular extension point also gets changed.
	 *
	 * @param target
	 ...
	 *
	 * @param target target for which to get the extensions
	...
	public Iterable producePlugins(InjectionPoint injectionPoint) {
		ExtensionPoint extensionPoint = injectionPoint.getAnnotated().getAnnotation(
		...
		List<Plugin> plugins = instantiatePlugins(extensionPoint.value(),
				extensionPoint.singleton(), extensionPoint.reverseOrder());
		...
		return filterPluginsByQualifiers(plugins, qualifiers);
	}
	 ...
	 */
	private Map<Class<? extends Annotation>, Annotation> getInjectionPointQualifiers(
		...
		Map<Class<? extends Annotation>, Annotation> qualifiers = CollectionUtils
				.createHashMap(size);
	 ...
	 * @return the iterable
	 */
	private Iterable<?> filterPluginsByQualifiers(List<Plugin> plugins,
		...
		for (Plugin plugin : plugins) {
			...
			if (matchPluginToQualifiers(plugin, qualifiers)) {
				filtered.add(plugin);
			}
		}
		return filtered;
	...
	/**
	 * Checks if the plugin matches the list of qualifiers found on the injection point. The current
	 * implementation does not check the qualifiers binding methods, but only the presence of an
	 * annotation.
	 *
	 * @param plugin
	 *            the plugin to match
	 ...
	 */
	private boolean matchPluginToQualifiers(Plugin plugin,
			Map<Class<? extends Annotation>, Annotation> qualifiers) {
		...
		for (Annotation annotation : annotations) {
			...
			// filtering
			if (!(annotation instanceof Extension) && !(annotation instanceof Documentation)) {
	 ...
	 */
	private boolean matchQualifiers(Annotation annotation, Annotation filter) {
	 ...
	 * @param method
	 *            the method
	 * @return the annotation method value or <code>null</code> if nothing was set or some error
	 *         occur
	 */
	private Object getAnnotationMethodValue(Annotation annotation, Method method) {
		try {
			return method.invoke(annotation);
		} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
			// we are not so interested in these exceptions here but we still going to log them
			LOGGER.trace(
					"Could not get the value of method {} from qualifier {} with instance {} due to {}",
	 ...
	 *            target for which to get the extensions
	 * @param singleton
	 *            if true and if all the plugins are in @Dependent, they will be cached. If even one
	 *            plugin is in non-dependent scope, the plugins won't be cached.
	 * @param inverse
	 *            if true, the plugins will be served in reverse order.
	 * @return list with the instantiated plugins.
	 */
	private List<Plugin> instantiatePlugins(String target, boolean singleton, boolean inverse) {
		List<Plugin> result = cachedPlugins.get(target);
		if (result == null) {
			...
			for (Bean<?> bean : extensionPointBeans) {
				...
				Plugin pluginInstance = (Plugin) beanManager.getReference(bean, Plugin.class,
						creationalContext);
				result.add(pluginInstance);
			}

			if (isEligableForCache(singleton, extensionPointBeans)) {
				// TODO: should prevent the build plugins list from modifications
				cachedPlugins.put(target, result);
			}
		}
	...
	/**
	 * Examine all plugins for the particular extension point and see if they're is a bean that is
	 * not in @Dependent scope. If there issuch bean, the extension point will not be cached.
	 *
	 * @param singleton
	 ...
	 */
	private boolean isEligableForCache(boolean singleton, List<Bean<?>> extensionPointBeans) {
	 ...
	 */
	private Extension getExtensionMetadata(Bean<?> bean) {
		return bean.getBeanClass().getAnnotation(Extension.class);
	}
	 ...
	 * @author Adrian Mitev
	 */
	private class PluginComparator implements Comparator<Bean<?>> {
		...
		public int compare(Bean<?> plugin1, Bean<?> plugin2) {
			return Double.compare(getExtensionMetadata(plugin1).order(),
					getExtensionMetadata(plugin2).order());
		}
	}
