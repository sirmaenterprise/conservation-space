 ...
 * <ul>
 * <li>{sequenceId} - reference a value from a sequence - gets the current value and does not modify
 * the sequence with the given id.
 * <li>{+sequenceId} - reference a value from a sequence - increments the sequence with the given
 * name and returns the result.
 * <li>{sequenceId+} - reference a value from a sequence - returns the current value and increments
 * the sequence
 * <li>[field_name] - gets the value of a property from the current object instance. If a property
 * is referenced but not found then the evaluator will throw
 * {@link com.sirma.itt.emf.exceptions.EmfConfigurationException}.
 * <li>(date_pattern) - gets the value after evaluating the date pattern. If the pattern is not
 * valid a {@link com.sirma.itt.emf.exceptions.EmfConfigurationException} will be thrown.The pattern
 * is based on {@link java.text.SimpleDateFormat}
 * </ul>
 * 
...
public class GenerateIdEvaluator extends BaseEvaluator {
	...
	/**
	 * The expression pattern. The pattern does not validate the internal structure due to non
	 * determined structure. The structure is validated on evaluation
	 */
	private static final Pattern GENERAL_EXPRESSION = Pattern.compile(EXPRESSION_START
			+ "\\{seq\\((.+?)\\)(?:.min\\((?<min>[\\d]+?)\\))?(?:.init\\((?<init>[\\d]+?)\\))?"
			+ FROM_PATTERN + "}");
	...
	protected Serializable evaluateInternal(Matcher matcher, ExpressionContext context,
		...
		if ((startIndex >= 0) && (endIndex > -1)) {
			...
			if (serializable != null) {
				internalExression = internalExression.substring(0, startIndex)
						+ serializable.toString()
						+ internalExression.substring(endIndex + 1, internalExression.length());
			}
		}
		if (StringUtils.isNotNullOrEmpty(initialValue)) {
		...
		if (StringUtils.isNotNullOrEmpty(minimalLenght)) {
