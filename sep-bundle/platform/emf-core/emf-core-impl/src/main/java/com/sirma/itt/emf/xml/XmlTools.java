...
/**
 * Class containing some utility methods for working with XMLs. Such as removing the empty tags from
 * a XML and etc.
 * 
 * @author B.Bonev
 */
public class XmlTools {
	...
	private static final MessageFormat SCHEMA_LOCATION_FORMATTER = new MessageFormat(
			NO_NAMESPACE_SCHEMA_LOCALTION);
	private static final Pattern XML_DECLARATION_PATTERN = Pattern.compile(
			"(<\\?xml version=\"1\\.0\" encoding=\"[\\w-]+\"\\?>)?\\r?\\n?"
					+ "<(\\w+)\\s*?(xmlns:xsi=\"http://www\\.w3\\.org/2001/XMLSchema-instance\")?"
					+ "\\s*?(xsi:noNamespaceSchemaLocation=\"(.+?)\")?\\s*?>(.*?)</\\2>",
			Pattern.DOTALL);

	/**
	 * Captures in group with number 3 the XSD content (without the schema tag)
	 */
	private static final Pattern XSD_CONTENT_PATTERN = Pattern.compile(
			"((?:<\\?(?:xml version=\"[\\d\\.]+\")?\\s*(?:encoding=\"[\\w-]+\\s*\")?\\?>)?\\r?\\n?"
	...
	/**
	 * Merges the given XSDs into single XSD with header from the first entry. The method will not
	 * include <xs:include /> entries. Also will check the default prefix for the
	 * http://www.w3.org/2001/XMLSchema namespace in rest of the files and if finds a discrepancies
	 * then will change them to the first schema. <br>
	...
	/**
	 * Adds default XML instance namespace and schema location for no namespace location to the
	 * given XML.
	 * 
	 * @param xml
	...
	public static String addSchemaDeclaration(String xml, String schemaName) {
		...
		if ((declaredSchemaName != null) && !schemaName.equalsIgnoreCase(declaredSchemaName)) {
			LOGGER.warn("Found schema declaration " + declaredSchemaName
					+ " that does not match expected schema " + schemaName);
		}
		...
		xmlBuilder.append('>').append(matcher.group(6)).append("</").append(xmlRootElement)
				.append('>');
	...
	public static String getXMLRootTagName(String xml) {
		...
		String string = xml.substring(start + 1, end);
		return string;
	}
