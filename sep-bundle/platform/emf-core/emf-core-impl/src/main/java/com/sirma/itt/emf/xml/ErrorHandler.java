...
import static com.sirma.itt.emf.xml.XmlErrorLevel.ERROR;
import static com.sirma.itt.emf.xml.XmlErrorLevel.FATAL;
import static com.sirma.itt.emf.xml.XmlErrorLevel.WARNING;
 ...
 * default behavior is to collect the errors, if you need the other case you need to call the method
...
public class ErrorHandler extends DefaultHandler {
	...
	public void error(SAXParseException e) throws SAXException {
		...
		if (!("Document is invalid: no grammar found.".equals(string) || (string
				.startsWith("Document root element") && string
				.endsWith("must match DOCTYPE root \"null\".")))) {
			if (validating) {
				addError(e, ERROR);
			} else {
				throw e;
	...
	/**
	 * Returns <code>true</code> if after parsing there are errors and <code>false</code> if no
	 * errors detected.
	 * 
	...
	private void addError(SAXParseException exception, XmlErrorLevel level) {
		errorsList.add(new XmlError(exception.getMessage(), level, exception.getLineNumber(),
