...
public class UsernameFieldValueRetriever extends PairFieldValueRetriever {
	...
	public String getLabel(String... value) {
		if (!ArrayUtils.isEmpty(value) && value[0] != null) {
			User resource = resourceService.getResource(value[0], ResourceType.USER);
	...
	public RetrieveResponse getValues(String filter, Integer offset, Integer limit) {
		...
		for (int i = 0; i < allUsers.size(); i++) {
			...
			if (StringUtils.isNullOrEmpty(filter)
					|| ((String) anUser.getDisplayName()).toLowerCase().startsWith(
							filter.toLowerCase())) {
				validateAndAddPair(results, (String) anUser.getIdentifier(),
						anUser.getDisplayName(), filter, offset, limit, total);
				total++;
			}
		}
