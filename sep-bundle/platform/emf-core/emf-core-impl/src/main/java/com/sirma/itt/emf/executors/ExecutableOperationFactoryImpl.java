...
/**
 * Default implementation of a {@link ExecutableOperationFactory}. The implementation injects all
 * operations and maps them by operation id. If one operation is defined more then once the one with
...
public class ExecutableOperationFactoryImpl implements ExecutableOperationFactory {
	...
	public void initializeMappings() {
		...
		for (ExecutableOperation operation : operationsExtensionPoint) {
			...
			if (current != null) {
				LOGGER.warn("Overriding executor for operation {} from {} to {}",
						operation.getOperation(), current, operation);
			}
