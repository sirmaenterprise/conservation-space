...
public class CacheFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (!httpRequest.getRequestURI().contains(WebResourceServlet.SERVLET_PATH)
			...
			httpResponse.addHeader("Expires", new Date(System.currentTimeMillis()
					- (2592000 * 1000)).toString());
			// one month in the past
			httpResponse.addHeader("Last-Modified", new Date(System.currentTimeMillis()
					+ (2592000 * 1000)).toString());
