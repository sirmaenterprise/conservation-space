...
/**
 * Map implementation that has a fixed size. If added element that will exceed the specified size an
 * element will be removed from it. The removed element will be the one that is longer in the map.
 * This is useful for fixed size caches.
 * 
 * @author BBonev
 * @param <K>
 *            the key type
...
public class FixedSizeMap<K, V> extends LinkedHashMap<K, V> {
	...
	public void putAll(Map<? extends K, ? extends V> m) {
		...
		if (m.size() > maxSize) {
			...
			throw new EmfRuntimeException("Insufficient storage capacity. The new data " + m.size()
