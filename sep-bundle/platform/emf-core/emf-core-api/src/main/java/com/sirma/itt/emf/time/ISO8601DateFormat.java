 ...
 * A negative sign indicates a year BCE. The absence of a sign or the presence of a positive sign
 * indicates a year CE (for example, -0055 would indicate the year 55 BCE, while +1969 and 1969
 * indicate the year 1969 CE).</li>
	...
	public static Date parse(String isoDate) {
		...
		try {
			...
			if (isoDate.charAt(offset) != '-') {
				throw new IndexOutOfBoundsException("Expected - character but found "
			...
			if (isoDate.charAt(offset) != 'T') {
				throw new IndexOutOfBoundsException("Expected T character but found "
						+ isoDate.charAt(offset));
			}
