 ...
 * <p>
 * The contents of ALFRESCO_TEMP_FILE_DIR [%java.io.tmpdir%/Alfresco] are managed by this class.
 * Temporary files and directories are cleaned by TempFileCleanerJob so that after a delay [default
 * 1 hour] the contents of the alfresco temp dir, both files and directories are removed.
 * <p>
...
public interface TempFileProvider {
	...
	/**
	 * Get the Alfresco temp dir, by defaut %java.io.tempdir%/Alfresco. Will create the temp dir on
	...
	/**
	 * creates a longer living temp dir. Files within the longer living temp dir will not be garbage
	 * collected as soon as "normal" temporary files. By default long life temp files will live for
	 * for 24 hours rather than 1 hour.
	 * <p>
	 * Code using the longer life temporary files should be careful to clean up since abuse of this
	 * feature may result in out of memory/disk space errors.
	 * 
	 * @param key
	 *            can be blank in which case the system will generate a folder to be used by all
	 *            processes or can be used to create a unique temporary folder name for a particular
	 *            process. At the end of the process the client can simply delete the entire
	 *            temporary folder.
	 ...
	 * @param suffix
	 *            the suffix
	 * @return Returns a temp <code>File</code> that will be located in the <b>Alfresco</b>
	 *         subdirectory of the default temp directory
	 * @see #CMF_TEMP_FILE_DIR
	...
	/**
	 * Delete file by first try to remove it, and on fail, tries to remove all child first
	 * recursively.
	 * 
	 * @param tempFile
	 *            the temp file
	...
	/**
	 * Gets the extension from given name. 'application.pdf' would return .pdf, 'my.application.pdf'
	 * would return the same. If no extension is available '.tmp' is returned. If name is null or
