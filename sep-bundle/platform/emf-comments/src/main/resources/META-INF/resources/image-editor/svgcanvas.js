...
var curConfig = {
	...
	selectNew: true,
	dimensions: [svgSpace.canWidth, svgSpace.canHeight]	//TOOL ANNOTATION
};
...
var idprefix = rsprefix;	//TOOL ANNOTATION
...
var all_properties = {
	shape: {
		fill: "#" + curConfig.initFill.color,					//TOOL ANNOTATION
		fill_paint: null,
...
var addSvgElementFromJson = this.addSvgElementFromJson = function(data) {
	...
	if(data.curStyles) {
    	    	var colorRGB = $('#palette li a').last().attr("data-rgb");		//TOOL ANNOTATION
    	    	if($('#palette li.active a').size()>0){					//TOOL ANNOTATION
    	    	    colorRGB = $('#palette li.active a').last().attr("data-rgb");	//TOOL ANNOTATION
    	    	}									//TOOL ANNOTATION
		svgedit.utilities.assignAttributes(shape, {
			"fill": "none",							//TOOL ANNOTATION
			"stroke": colorRGB,						//TOOL ANNOTATION
			"stroke-width": 1,						//TOOL ANNOTATION
...
(function(c) {
	...
	var idprefix = rsprefix;													//TOOL ANNOTATION
	var obj_num = 1;														//TOOL ANNOTATION
	getId = c.getId = function() {													//TOOL ANNOTATION
		if (events["getid"]) return call("getid", obj_num);									//TOOL ANNOTATION
		return idprefix + obj_num;												//TOOL ANNOTATION
	};																//TOOL ANNOTATION
	getNextId = c.getNextId = function() {												//TOOL ANNOTATION
		var id = getId();													//TOOL ANNOTATION
		while (getElem(id) || getElem(id+rssufix) || !svgSpace.isShapeUnique(id) || !svgSpace.isShapeUnique(id+rssufix)) {	//TOOL ANNOTATION
			obj_num++;													//TOOL ANNOTATION
			id = getId();													//TOOL ANNOTATION
		}															//TOOL ANNOTATION
		return id;														//TOOL ANNOTATION
	};																//TOOL ANNOTATION
...
var clearSelection = this.clearSelection = function(noCall) {
	svgSpace.selectedItem = null; // TOOL ANNOTATION
...
var addToSelection = this.addToSelection = function(elemsToAdd, showGrips) {
	var notSubViewRect = elemsToAdd[0].id != sub_view_rect ,
		notSubView = elemsToAdd[0].id != sub_view,
		notUniqueShape = svgSpace.isShapeUnique(elemsToAdd[0].id) == false;
		
    if(notSubViewRect && notSubView && notUniqueShape && !svgSpace.editItem){
    	return;
    }	//TOOL ANNOTATION
	...
	while (i--) {
		...
		if (!elem || !getBBox(elem) || elem.tagName == 'g') continue;	//TOOL ANNOTATION
...
//			if (j == 0) selectedBBoxes[0] = svgedit.utilities.getBBox(elem);
	...
	if ((showGrips || selectedElements.length == 1) && svgSpace.editItem) {
...
(function() {
	...
	var dblclick = function(event) { //Annotation tool
	...
	var mouseDown = function(evt)
	{
		...
		root_sctm.e = root_sctm.e - $(window).scrollLeft();			//TOOL ANNOTATION
		root_sctm.f = root_sctm.f - $(window).scrollTop();			//TOOL ANNOTATION
...
// 		}
		...
		if (current_mode == "select") {																//TOOL ANNOTATION
		    $('[stroke-width="5"]').attr('stroke-width','');													//TOOL ANNOTATION
		    svgSpace.callDeselection(mouse_target,evt);														//TOOL ANNOTATION
		    $('.' + rowSelected).removeClass(rowSelected);													//TOOL ANNOTATION
		}																			//TOOL ANNOTATION
		
		start_transform = mouse_target.getAttribute("transform");
		var tlist = getTransformList(mouse_target);
		if ((mouse_target.id == 'svgroot' || mouse_target.id.toString().indexOf('backimage') == 0								//TOOL ANNOTATION
			|| (mouse_target.id.toString().indexOf('frontimage') == 0 && mouse_target.id.toString().length == 10 && svgSpace.fullX == svgSpace.canWidth))	//TOOL ANNOTATION
			&& current_mode == "select") {															//TOOL ANNOTATION
		    if (mouse_target.id.toString().indexOf('backimage') == 0) {												//TOOL ANNOTATION
			canvas.clearSelection();															//TOOL ANNOTATION
		    }																			//TOOL ANNOTATION
		    mouseDownX = mouse_x;																//TOOL ANNOTATION
		    mouseDownY = mouse_y;																//TOOL ANNOTATION
		    svgSpace.setEditBtnDisabledState(true);
		    return;																		//TOOL ANNOTATION
		}																			//TOOL ANNOTATION
		if (current_mode == "select") {																//TOOL ANNOTATION
		    if (mouse_target.id.indexOf(rsprefix) === 0) {													//TOOL ANNOTATION
		    	mouse_target.setAttribute('stroke-width','5');													//TOOL ANNOTATION
				svgSpace.callClick(mouse_target, evt);														//TOOL ANNOTATION
				svgSpace.setEditBtnDisabledState(false);
				$(document).trigger('iat:dom:annotation-selected', mouse_target.id);
				svgSpace.selectedItem = mouse_target.id;
		    } else {
		    	svgSpace.selectedItem = null;
		    }
		}																//TOOL ANNOTATION
		
		var inEdit = mouse_target.id === svgSpace.editItem,
			rsMouseTarget = mouse_target.id.indexOf(rsprefix) === 0,
			canEdit = (rsMouseTarget && (inEdit || svgSpace.createdItem)),
			canCreate = !$(mouse_target).is('#frontimagerect, #frontimage');
		...
		switch (current_mode) {
			case "fill_shape":
				if (canEdit) {
					...
					if (svgSpace.fillColor) {
						colorHexString = svgSpace.fillColor.toHexString();
						colorAlpha = svgSpace.fillColor.alpha;
					} else {
						colorHexString = svgCanvas.getColor('fill');
					}
					
					mouse_target.removeAttribute('opacity');
					mouse_target.setAttribute('fill', colorHexString);
					mouse_target.setAttribute('fill-opacity', '' + colorAlpha);
				}
				clearSelection();																//TOOL ANNOTATION
			    $('.tool_button_current').removeClass('tool_button_current').addClass('tool_button');							//TOOL ANNOTATION
			    $('#tool_select').addClass('tool_button_current').removeClass('tool_button');								//TOOL ANNOTATION
			    $('#styleoverrides').text('#svgcanvas svg *{cursor:move;pointer-events:all} #svgcanvas svg{cursor:default}');				//TOOL ANNOTATION
				break;
			case "stroke_color":
				if (canEdit) {
					var colorHexString,
					...
					if (svgSpace.strokeColor) {
						colorHexString = svgSpace.strokeColor.toHexString();
						colorAlpha = svgSpace.strokeColor.alpha;
					} else {
						colorHexString = svgCanvas.getColor('stroke');
					}
					
					mouse_target.removeAttribute('opacity');
					mouse_target.setAttribute('stroke', colorHexString);
					mouse_target.setAttribute('stroke-opacity', '' + colorAlpha);
				}
				clearSelection();																//TOOL ANNOTATION
			    $('.tool_button_current').removeClass('tool_button_current').addClass('tool_button');							//TOOL ANNOTATION
			    $('#tool_select').addClass('tool_button_current').removeClass('tool_button');								//TOOL ANNOTATION
			    $('#styleoverrides').text('#svgcanvas svg *{cursor:move;pointer-events:all} #svgcanvas svg{cursor:default}');				//TOOL ANNOTATION
				break;
			case "free_text":																	//TOOL ANNOTATION
				if (svgSpace.createdItem || !canCreate) {
					event.preventDefault();
					return;
				}
				
				var existingEditor = $('.iat-free-text-editor');
				if (existingEditor.length) {
					existingEditor.focus();
				} else {
					var imageElementId = getNextId() + rssufix;
					showFreeTextEditor(imageElementId, mouse_x, mouse_y);
					clearSelection();																//TOOL ANNOTATION
					$('.tool_button_current').removeClass('tool_button_current').addClass('tool_button');							//TOOL ANNOTATION
					$('#tool_select').addClass('tool_button_current').removeClass('tool_button');								//TOOL ANNOTATION
					$('#styleoverrides').text('#svgcanvas svg *{cursor:move;pointer-events:all} #svgcanvas svg{cursor:default}');				//TOOL ANNOTATION
			    ...
			    break;	
			case "star":																	//TOOL ANNOTATION
				if (svgSpace.createdItem || !canCreate) {
					event.preventDefault();
					return;
				}
			    var d_final = "M" + mouse_x + "," + mouse_y;												//TOOL ANNOTATION
			    d_final = d_final + "L" + mouse_x + "," + mouse_y;												//TOOL ANNOTATION
			    d_final = d_final + "C" + mouse_x + "," + (mouse_y-10);											//TOOL ANNOTATION
			    d_final = d_final + " " + (mouse_x-8) + "," + (mouse_y-20);											//TOOL ANNOTATION
			    d_final = d_final + " " + mouse_x + "," + (mouse_y-20);											//TOOL ANNOTATION
			    d_final = d_final + "C" + (mouse_x+8) + "," + (mouse_y-20);											//TOOL ANNOTATION
			    d_final = d_final + " " + mouse_x + "," + (mouse_y-10);											//TOOL ANNOTATION
			    d_final = d_final + " " + mouse_x + "," + mouse_y;												//TOOL ANNOTATION
			    d_final = d_final + "L" + mouse_x + "," + mouse_y + "Z";											//TOOL ANNOTATION
			    
			    var shp = addSvgElementFromJson({														//TOOL ANNOTATION
			    	"element": "path",															//TOOL ANNOTATION
			    	"attr": {																//TOOL ANNOTATION
				    	"id": getNextId() + rssufix,													//TOOL ANNOTATION
				    	"d": d_final,															//TOOL ANNOTATION
				    	"fill": "#ff8888",														//TOOL ANNOTATION
				    	"stroke": cur_shape.stroke													//TOOL ANNOTATION
					}																//TOOL ANNOTATION
			    });																		//TOOL ANNOTATION
			    
			    clearSelection();																//TOOL ANNOTATION
			    svgSpace.callCustomFuction(shp);														//TOOL ANNOTATION
			    $('.tool_button_current').removeClass('tool_button_current').addClass('tool_button');							//TOOL ANNOTATION
			    $('#tool_select').addClass('tool_button_current').removeClass('tool_button');								//TOOL ANNOTATION
			    $('#styleoverrides').text('#svgcanvas svg *{cursor:move;pointer-events:all} #svgcanvas svg{cursor:default}');				//TOOL ANNOTATION
			    break;																	//TOOL ANNOTATION
			case "select":
				isCursorUp = false;															//TOOL ANNOTATION
				started = true;
				current_resize_mode = "none";
				if(right_click) started = false;
				
				if (mouse_target != svgroot) {
					var targetId = mouse_target.id;
					
					if (targetId && targetId.indexOf(rsprefix) !== -1) {
						svgSpace.selectedItem = targetId;
					}
					
					
				    if(svgSpace.createdItem == mouse_target.id || mouse_target.id.toString().indexOf(sub_view_rect)==0){				//TOOL ANNOTATION
...
// 					console.log('s',[start_x, start_y]);
			...
			case "image":
				...
				newImage.setAttributeNS(xlinkns, "xlink:href", last_good_img_url);	//TOOL ANNOTATION
...
// 					newText.textContent = "text";
	...
	var mouseMove = function(evt)
	{
		...
		{
			case "select":
			    	$('#'+info_panel).css('visibility','hidden');					//TOOL ANNOTATION
				...
				if (selectedElements[0] !== null) {
					...
					if (dx != 0 || dy != 0) {
						var len = selectedElements.length;
						var isSubViewSelected = false;					//TOOL ANNOTATION
						for (var j = 0; j < len; j++) {					//TOOL ANNOTATION
						    if (selectedElements[j] == null) continue;			//TOOL ANNOTATION
						    if (selectedElements[j].id == sub_view) {			//TOOL ANNOTATION
							isSubViewSelected = true;				//TOOL ANNOTATION
							break;							//TOOL ANNOTATION
						    }								//TOOL ANNOTATION
						}								//TOOL ANNOTATION
						if (isSubViewSelected == true) {				//TOOL ANNOTATION
						    var rectangle = document.getElementById(sub_view_rect);	//TOOL ANNOTATION
						    addToSelection([rectangle]);				//TOOL ANNOTATION
						}								//TOOL ANNOTATION
						for (var i = 0; i < len; ++i) {
...
//							}
							...
							var deltaX = dx;																	//TOOL ANNOTATION
							var deltaY = dy;																	//TOOL ANNOTATION
							if (selected.id == sub_view_rect && isSubViewSelected == false) {											//TOOL ANNOTATION
							    var wrapper = document.getElementById(sub_view);													//TOOL ANNOTATION
							    var real = document.getElementById(sub_view_rect);													//TOOL ANNOTATION
							    deltaX = calculateSubViewDelta(wrapper.getAttribute('x'),wrapper.getAttribute('width'),real.getAttribute('x'),real.getAttribute('width'),dx);	//TOOL ANNOTATION
							    deltaY = calculateSubViewDelta(wrapper.getAttribute('y'),wrapper.getAttribute('height'),real.getAttribute('y'),real.getAttribute('height'),dy);	//TOOL ANNOTATION
							    var resizeStep = 1;																	//TOOL ANNOTATION
							    if (tileSizeX[0] > svgSpace.canWidth / 3 || tileSizeY[0] > svgSpace.canHeight / 3) {								//TOOL ANNOTATION
								resizeStep = 2;																	//TOOL ANNOTATION
							    }																			//TOOL ANNOTATION
							    var frontSizeX = tileSizeX[0] / resizeStep;														//TOOL ANNOTATION
							    var frontSizeY = tileSizeY[0] / resizeStep;														//TOOL ANNOTATION
							    subViewStepX = (deltaX * max_sizeX[0]) / (wrapper.getAttribute('width') * tileSizeX[0] * maxZoom[0] / zoomFactor);					//TOOL ANNOTATION
							    subViewStepY = (deltaY * max_sizeY[0]) / (wrapper.getAttribute('height') * tileSizeY[0] * maxZoom[0] / zoomFactor);					//TOOL ANNOTATION
							}																			//TOOL ANNOTATION
							xform.setTranslate(deltaX,deltaY);															//TOOL ANNOTATION
							if(tlist.numberOfItems) {
			...
			case "multiselect":
				...
				break;
			case "resize":
			    	if(selected.getAttribute("id").indexOf(rssufix)>0){				//TOOL ANNOTATION
			    	    break;									//TOOL ANNOTATION
			    	}										//TOOL ANNOTATION
...
// 					}
	...
	{
		if (mouseDownX != 0 && mouseDownY !=0) {				//TOOL ANNOTATION
	            var pt2 = transformPoint( evt.pageX, evt.pageY, root_sctm );	//TOOL ANNOTATION
	            var mouse_x2 = pt2.x * current_zoom;				//TOOL ANNOTATION
	            var mouse_y2 = pt2.y * current_zoom;				//TOOL ANNOTATION
	            var oldX = currentX;						//TOOL ANNOTATION
	            var oldY = currentY;						//TOOL ANNOTATION
	            mouseDownX = mouseDownX - mouse_x2;					//TOOL ANNOTATION
	            mouseDownY = mouseDownY - mouse_y2;					//TOOL ANNOTATION
	            if (mouseDownX != 0 && mouseDownY !=0) {				//TOOL ANNOTATION
		            if (mouseDownX>=svgSpace.minimumDragX) {			//TOOL ANNOTATION
		        	currentX = currentX + getVisibleTilesX(0) / 2;		//TOOL ANNOTATION
		        	currentX = normalizeX(currentX,zoomFactor);		//TOOL ANNOTATION
		            } else  if (mouseDownX<=-svgSpace.minimumDragX) {		//TOOL ANNOTATION
		        	currentX = currentX - getVisibleTilesX(0) / 2;		//TOOL ANNOTATION
		        	currentX = normalizeX(currentX,zoomFactor);		//TOOL ANNOTATION
		            }								//TOOL ANNOTATION
		            if (mouseDownY>=svgSpace.minimumDragY) {			//TOOL ANNOTATION
		        	currentY = currentY + getVisibleTilesY(0) / 2;		//TOOL ANNOTATION
		        	currentY = normalizeY(currentY,zoomFactor);		//TOOL ANNOTATION
		            } else  if (mouseDownY<=-svgSpace.minimumDragY) {		//TOOL ANNOTATION
		        	currentY = currentY - getVisibleTilesY(0) / 2;		//TOOL ANNOTATION
		        	currentY = normalizeY(currentY,zoomFactor);		//TOOL ANNOTATION
		            }								//TOOL ANNOTATION
	            zoom(oldX - currentX, oldY - currentY, 1);				//TOOL ANNOTATION
		    mouseDownX = 0;							//TOOL ANNOTATION
		    mouseDownY = 0;							//TOOL ANNOTATION
		    return;								//TOOL ANNOTATION
	            }									//TOOL ANNOTATION
		}									//TOOL ANNOTATION

		if(evt.button === 1) return;						//TOOL ANNOTATION
		...
		switch (current_mode)
		{
			...
			case "multiselect":
				...
				current_mode = "select";
			case "select":
			    	isCursorUp = true;													//TOOL ANNOTATION
				if (subViewStepX != 0 || subViewStepY != 0) {										//TOOL ANNOTATION
				    currentX = currentX + subViewStepX;											//TOOL ANNOTATION
				    currentY = currentY + subViewStepY;											//TOOL ANNOTATION
				    zoom(-subViewStepX, -subViewStepY, 1);										//TOOL ANNOTATION
				    subViewStepX = 0;													//TOOL ANNOTATION
				    subViewStepY = 0;													//TOOL ANNOTATION
				}															//TOOL ANNOTATION
				if (selectedElements[0] != null) {
					...
					if (selectedElements[1] == null) {
						...
						var selected = selectedElements[0];
						if (selected.tagName != "g" && selected.tagName != "image" && selected.tagName != "foreignObject") {	//TOOL ANNOTATION
							cur_properties.fill = selected.getAttribute("fill");						//TOOL ANNOTATION
							cur_properties.fill_opacity = selected.getAttribute("fill-opacity");				//TOOL ANNOTATION
							cur_properties.stroke_opacity = selected.getAttribute("stroke-opacity");			//TOOL ANNOTATION
							cur_properties.stroke_width = selected.getAttribute("stroke-width");				//TOOL ANNOTATION
							cur_properties.stroke_dasharray = selected.getAttribute("stroke-dasharray");			//TOOL ANNOTATION
							cur_properties.stroke_linejoin = selected.getAttribute("stroke-linejoin");			//TOOL ANNOTATION
							cur_properties.stroke_linecap = selected.getAttribute("stroke-linecap");			//TOOL ANNOTATION
						}													//TOOL ANNOTATION
...
// 							call("selected", [selected]);
			...
			case "fhpath":
				...
				svgSpace.callCustomFuction(element);	//TOOL ANNOTATION
				break;
			case "line":
			...
			case "path":
				...
				element = res.element
				keep = res.keep;
				if (res.element != null) {			//TOOL ANNOTATION
				    svgSpace.callCustomFuction(element);	//TOOL ANNOTATION
				}						//TOOL ANNOTATION
				break;
			case "pathedit":
	...
	function bringToFront(t) {																//TOOL ANNOTATION
	    var oldParent = t.parentNode;															//TOOL ANNOTATION
	    var oldNextSibling = t.nextSibling;															//TOOL ANNOTATION
	    t = t.parentNode.appendChild(t);															//TOOL ANNOTATION
	    if (oldNextSibling != t.nextSibling) {														//TOOL ANNOTATION
		call("changed", [ t ]);																//TOOL ANNOTATION
	    }																			//TOOL ANNOTATION
	}																			//TOOL ANNOTATION

	var mouseOver = function(evt) {																//TOOL ANNOTATION
	    if (current_mode == 'select' && evt.button == 0) {													//TOOL ANNOTATION
		var mouse_target = getMouseTarget(evt);														//TOOL ANNOTATION
		if (mouse_target.id != 'svgroot' && mouse_target.id.toString().indexOf('backimage') < 0) {							//TOOL ANNOTATION
		    if (isCursorUp == true) {															//TOOL ANNOTATION
		        var shouldSelect = true;														//TOOL ANNOTATION
		        if ('selectorParentGroup' == mouse_target.id || 'selectorGroup0' == mouse_target.id || svgSpace.editItem != mouse_target.id) {	//TOOL ANNOTATION
			    shouldSelect = false;														//TOOL ANNOTATION
		        }																	//TOOL ANNOTATION
			if (mouse_target.id.toString().indexOf(sub_view) >= 0) {										//TOOL ANNOTATION
			    shouldSelect = false;														//TOOL ANNOTATION
			}																	//TOOL ANNOTATION
		        if (shouldSelect == true) {														//TOOL ANNOTATION
		            bringToFront(mouse_target);														//TOOL ANNOTATION
		            svgSpace.callSelection(mouse_target,evt);												//TOOL ANNOTATION
    		            canvas.clearSelection();														//TOOL ANNOTATION
    		            selectOnly([mouse_target]);														//TOOL ANNOTATION
		            bringToFront(document.getElementById(sub_view));											//TOOL ANNOTATION
		            bringToFront(document.getElementById(sub_view_rect));										//TOOL ANNOTATION
		        } else if(mouse_target.id.toString().indexOf(sub_view) < 0) {										//TOOL ANNOTATION
		            svgSpace.callSelection(mouse_target,evt);												//TOOL ANNOTATION
		        }																	//TOOL ANNOTATION
		    }																		//TOOL ANNOTATION
		}																		//TOOL ANNOTATION
	...
	var mouseOut = function(evt) {																//TOOL ANNOTATION
	    if (current_mode == 'select' && evt.button == 0) {													//TOOL ANNOTATION
		var mouse_target = getMouseTarget(evt);														//TOOL ANNOTATION
		if (mouse_target.id != 'svgroot' && mouse_target.id.toString().indexOf('backimage') < 0) {							//TOOL ANNOTATION
		    mouse_target.setAttribute('stroke-dasharray','');												//TOOL ANNOTATION
		    svgSpace.callDeselection(mouse_target,evt);													//TOOL ANNOTATION
		}																		//TOOL ANNOTATION
	    }																			//TOOL ANNOTATION
	};																			//TOOL ANNOTATION
	...
	$(container)
		...
		.mouseover(mouseOver)
		.mouseout(mouseOut)
		.dblclick(dblclick);	//TOOL ANNOTATION
//	$(window).mouseup(mouseUp);
	...
	$(container).bind("mousewheel DOMMouseScroll", function(e){
		...
		if(e.originalEvent.wheelDelta) {				//TOOL ANNOTATION
			if (e.originalEvent.wheelDelta >= 120) {		//TOOL ANNOTATION
				zoomIn();					//TOOL ANNOTATION
			} else if (e.originalEvent.wheelDelta <= -120) {	//TOOL ANNOTATION
				zoomOut();					//TOOL ANNOTATION
			}							//TOOL ANNOTATION
		} else if(e.originalEvent.detail) {				//TOOL ANNOTATION
			if (e.originalEvent.detail > 0) {			//TOOL ANNOTATION
				zoomOut();					//TOOL ANNOTATION
			} else if (e.originalEvent.detail < 0) {		//TOOL ANNOTATION
				zoomIn();					//TOOL ANNOTATION
			}							//TOOL ANNOTATION
		}								//TOOL ANNOTATION
		//if(!bbox.factor) return;					//TOOL ANNOTATION
		//call("zoomed", bbox);						//TOOL ANNOTATION
	});
...
// 					keep = true;
		...
		mouseMove: function(mouse_x, mouse_y) {
			hasMoved = true;
			if(current_mode === "path") {	if(true)return;					//TOOL ANNOTATION
...
this.setMode = function(name) {
	if(selectedElements && selectedElements[0] && sub_view_rect==selectedElements[0].id){	//TOOL ANNOTATION
	    $("circle[id^='selectorGrip_']").attr("visibility","hidden");			//TOOL ANNOTATION
	}else{											//TOOL ANNOTATION
	    $("circle[id^='selectorGrip_']").attr("visibility","visible");			//TOOL ANNOTATION
	}											//TOOL ANNOTATION
	$("#selectorGrip_rotate").attr("visibility","hidden");					//TOOL ANNOTATION
	$("#selectorGrip_rotateconnector").attr("visibility","hidden");				//TOOL ANNOTATION
	pathActions.clear(true);
	textActions.clear();
...
/**
 * Sets the stroke type used for the shapes. After setting the type it also applies it,
 * if there is an active element.
 * Allowed values are solid, dashed and dotted.
 */
this.setStrokeType = function(type) { //TOOL ANNOTATION	
	this.iatStrokeType = { 
		type: 'solid',
		attrName: 'stroke',
		attrValue: '#333333',
		oppositeAttrName: 'stroke-dasharray'
	};
	...
	if (svgSpace.createdItem) {
		var element = document.getElementById(svgSpace.createdItem);
		if (element) {
			this.applyCurrentStrokeType(element);
		}
	}
}

/**
 * When editting an annotation the stroke type could be changed.
 * When adding an annotation a stroke type should be applied to the element.
 */
this.applyCurrentStrokeType = function(element) { //TOOL ANNOTATION
	if (this.iatStrokeType.oppositeAttrName) {
		element.removeAttribute(this.iatStrokeType.oppositeAttrName)
	}
	element.setAttribute(this.iatStrokeType.attrName, this.iatStrokeType.attrValue);
}
...
this.setColor = function(type, val, preventUndo) {
	if(type == "stroke"){								//TOOL ANNOTATION
		var colorRGB = $('#palette li a').last().attr("data-rgb");		//TOOL ANNOTATION
		if($('#palette li.active a').size()>0){					//TOOL ANNOTATION
			colorRGB = $('#palette li.active a').last().attr("data-rgb");	//TOOL ANNOTATION
		}									//TOOL ANNOTATION
		val = colorRGB;								//TOOL ANNOTATION
		$('#'+curr_color).attr("stroke",val);					//TOOL ANNOTATION
	}										//TOOL ANNOTATION
	cur_shape[type] = val;
