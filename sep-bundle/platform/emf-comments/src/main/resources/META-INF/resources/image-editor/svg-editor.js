...
(function() {
  ...
  document.addEventListener("touchcancel", touchHandler, true);
	...
	if(!window.svgEditor) window.svgEditor = function($) {
		...
		var defaultPrefs = {
			...
			curConfig = {
				...
				initFill: {
					color: 'transparent',  // solid red			//TOOL ANNOTATION
					opacity: 0						//TOOL ANNOTATION
				},
				initStroke: {
					width: 1,						//TOOL ANNOTATION
					color: '000000',  // solid black
					opacity: 1
				},
				initOpacity: 1,
				imgPath: svgSpace.toolPath + '/images/',			//TOOL ANNOTATION
				langPath: svgSpace.toolPath + '/locale/',			//TOOL ANNOTATION
				extPath: svgSpace.toolPath + '/extensions/',			//TOOL ANNOTATION
				jGraduatePath: svgSpace.toolPath + '/jgraduate/images/',	//TOOL ANNOTATION
				extensions: [],							//TOOL ANNOTATION
				initTool: 'select',
				wireframe: false,
				colorPickerCSS: null,
				gridSnapping: false,
	...
	// 			var lang = ('lang' in curPrefs) ? curPrefs.lang : null;
				if(Editor.putLocale){						//TOOL ANNOTATION
			...
			$.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
				...
				fallback:{
					...
					'arrow_down':'dropdown.gif'			//TOOL ANNOTATION
				},
				placement: {
					'#tool_star' : 'star',				//TOOL ANNOTATION
					'#tool_my_rect' : 'rect',			//TOOL ANNOTATION
					'#tool_my_ellipse' : 'ellipse',			//TOOL ANNOTATION
					'#tool_zoom_in' : 'zoom_in',			//TOOL ANNOTATION
					'#tool_zoom_out' : 'zoom_out',			//TOOL ANNOTATION
					'#tool_zoom_default' : 'zoom_default',		//TOOL ANNOTATION
					'#tool_left' : 'left',				//TOOL ANNOTATION
					'#tool_right' : 'right',			//TOOL ANNOTATION
					'#tool_up' : 'up',				//TOOL ANNOTATION
					'#tool_down' : 'down',				//TOOL ANNOTATION
...
//					'#logo':'logo',
					...
					//'#tool_unlink_use':'unlink_use',					//TOOL ANNOTATION
					...
					'#layer_up':'go_up',
					'#layer_down':'go_down',
					//'#layer_moreopts':'context_menu',					//TOOL ANNOTATION
					'#layerlist td.layervis':'eye',

					'#tool_source_save,#tool_docprops_save':'ok',				//TOOL ANNOTATION
					'#tool_source_cancel,#tool_docprops_cancel':'cancel',			//TOOL ANNOTATION

					'#rwidthLabel, #iwidthLabel':'width',
					'#rheightLabel, #iheightLabel':'height',
					'#cornerRadiusLabel span':'c_radius',
					'#angleLabel':'angle',
					//'#linkLabel,#tool_make_link,#tool_make_link_multi':'globe_link',	//TOOL ANNOTATION
					'#zoomLabel':'zoom',
					'#tool_fill label': 'fill',
					'#tool_stroke .icon_label': 'fill',					//TOOL ANNOTATION
					'#group_opacityLabel': 'opacity',
					'#blurLabel': 'blur',
					'#font_sizeLabel': 'fontsize',

					'.flyout_arrow_horiz':'arrow_right',
					'.dropdown button, #main_button .dropdown':'arrow_down'			//TOOL ANNOTATION
				},
				resize: {
					'#logo .svg_icon': 28,
					...
					//'.palette_item:first .svg_icon' : 15,					//TOOL ANNOTATION
					//'#fill_bg .svg_icon, #stroke_bg .svg_icon': 16,			//TOOL ANNOTATION
				...
				callback: function(icons) {
					...
					if(tleft.offset()==null){						//TOOL ANNOTATION
					    return;								//TOOL ANNOTATION
					}									//TOOL ANNOTATION
					var min_height = 23;tleft.offset().top + tleft.outerHeight();		//TOOL ANNOTATION
					var size = $.pref('iconsize');						//TOOL ANNOTATION
					if(size && size != 'm') {						//TOOL ANNOTATION
						svgEditor.setIconSize(size);					//TOOL ANNOTATION	
					} else if($(window).height() < min_height) {				//TOOL ANNOTATION
						svgEditor.setIconSize('s');					//TOOL ANNOTATION
					}									//TOOL ANNOTATION
			...
			Editor.show_save_warning = false;
			var	isMac = (navigator.platform.indexOf("Mac") >= 0),				//TOOL ANNOTATION
				isWebkit = (navigator.userAgent.indexOf("AppleWebKit") >= 0),
				modKey = (isMac ? "meta+" : "ctrl+"), // ⌘
				path = svgCanvas.pathActions,
				undoMgr = svgCanvas.undoMgr,
				Utils = svgedit.utilities,
				default_img_url = svgSpace.toolPath + "/logo.png",				//TOOL ANNOTATION
				workarea = $("#workarea"),
...
// 						|| elem && elem.tagName == "path" && !multiselected) { // This was added in r1430, but not sure why
				...
				//	paintBox.fill.update();						//TOOL ANNOTATION
				//	paintBox.stroke.update();					//TOOL ANNOTATION
				}
...
// 				$('#zoom').val(Math.round(zoomlevel*100));
			...
			//	paintBox.fill.prep();					//TOOL ANNOTATION
			//	paintBox.stroke.prep();					//TOOL ANNOTATION
			}
			...
			var setFlyoutTitles = function() {
				$('.tools_flyout').each(function() {
					...
					if (shower.length > 0) {			//TOOL ANNOTATION
					    shower[0].title = tooltips.join(' / ');	//TOOL ANNOTATION
					}						//TOOL ANNOTATION
				});
			}
			...
			var updateToolbar = function() {
				if (selectedElement != null) {
					...
					case 'a':
						...
						//paintBox.fill.update(true);					//TOOL ANNOTATION
						//paintBox.stroke.update(true);					//TOOL ANNOTATION
...
// 					}
			...
			$('.palette_item').mousedown(function(evt){
				...
				var picker = (!evt.shiftKey ? "stroke" : "fill");	//TOOL ANNOTATION
				var id = (!evt.shiftKey ? '#stroke_' : '#fill_');	//TOOL ANNOTATION
				var color = $(this).attr('data-rgb');
				var spectrumColorObject = {
				...
				if(paintBox[picker]){					//TOOL ANNOTATION
					paintBox[picker].setPaint(paint);		//TOOL ANNOTATION
				}							//TOOL ANNOTATION
				if (!isStroke) {					//TOOL ANNOTATION
					svgSpace.strokeColor = null;
					svgCanvas.setColor('stroke', color);
					if (color != 'none' && svgCanvas.getStrokeOpacity() != 1) {
						svgCanvas.setPaintOpacity('stroke', 1.0);
					}
					
					svgSpace.setFillColor(spectrumColorObject);
				} else {
					svgSpace.fillColor = null;
					svgCanvas.setColor('fill', color);
					if (color != 'none' && svgCanvas.getFillOpacity() != 1) {
						svgCanvas.setPaintOpacity('fill', 1.0);
					}
					
					svgSpace.setStrokeColor(spectrumColorObject);
				}
				updateToolButtonState();
			...
			var toolButtonClick = function(button, noHiding) {
				...
				if( buttonElement.parent().hasClass('tools_flyout') ) {
					return true;
				}
			...
			function setStrokeOpt(opt, changeElem) {
				//var id = opt.id;							//TOOL ANNOTATION
				//var bits = id.split('_');						//TOOL ANNOTATION
				//var pre = bits[0];							//TOOL ANNOTATION
				//var val = bits[1];							//TOOL ANNOTATION
				//if(changeElem) {							//TOOL ANNOTATION
				//	svgCanvas.setStrokeAttr('stroke-' + pre, val);			//TOOL ANNOTATION
				//}									//TOOL ANNOTATION
				//operaRepaint();							//TOOL ANNOTATION
				//setIcon('#cur_' + pre , id, 20);					//TOOL ANNOTATION
				//$(opt).addClass('current').siblings().removeClass('current');		//TOOL ANNOTATION
			}
			...
			*/
			...
			var clickFHPath = function() {
				if (toolButtonClick('#tool_fhpath')) {
					...
					$('#workarea').css('cursor', 'url("' + EMF.applicationPath + '/image-editor/view/images/cursors/pen.cur"), default');
				}
			};
			...
			var clickLine = function() {
				if (toolButtonClick('#tool_line')) {
					...
					$('#workarea').css('cursor', 'url("' + EMF.applicationPath + '/image-editor/view/images/cursors/line.cur"), default');
				}
			};
			...
			var clickZoomIn = function () {														//TOOL ANNOTATION
			        if (toolButtonClick('#tool_zoom_in')) {												//TOOL ANNOTATION
			            if(canZoomIn()){														//TOOL ANNOTATION
			            	zoomIn();															//TOOL ANNOTATION
			            	svgSpace.zoomIn();														//TOOL ANNOTATION
			            }																//TOOL ANNOTATION
			        }																//TOOL ANNOTATION
			    };																	//TOOL ANNOTATION
			var clickZoomOut = function () {													//TOOL ANNOTATION
			        if (toolButtonClick('#tool_zoom_out')) {											//TOOL ANNOTATION
			            if(canZoomOut()){														//TOOL ANNOTATION
			            zoomOut();															//TOOL ANNOTATION
			            svgSpace.zoomOut();														//TOOL ANNOTATION
			            }																//TOOL ANNOTATION
			        }																//TOOL ANNOTATION
			    };																	//TOOL ANNOTATION
			var clickZoomDefault = function () {													//TOOL ANNOTATION
			        if (toolButtonClick('#tool_zoom_default')) {											//TOOL ANNOTATION
			            var oldZoom = zoomFactor;													//TOOL ANNOTATION
			            zoomFactor = minZoom[0];													//TOOL ANNOTATION
			            var oldX = currentX;													//TOOL ANNOTATION
			            var oldY = currentY;													//TOOL ANNOTATION
			            currentX = 0;														//TOOL ANNOTATION
			            currentY = 0;														//TOOL ANNOTATION
			            zoom(oldX * (zoomFactor / oldZoom) - currentX, oldY * (zoomFactor / oldZoom) - currentY, zoomFactor / oldZoom);		//TOOL ANNOTATION
			        }																//TOOL ANNOTATION
			    };																	//TOOL ANNOTATION
			var clickLeft = function () {														//TOOL ANNOTATION
			        if (toolButtonClick('#tool_left')) {												//TOOL ANNOTATION
			            var oldX = currentX;													//TOOL ANNOTATION
			            currentX = currentX - getVisibleTilesX(0) / 2;										//TOOL ANNOTATION
			            currentX = normalizeX(currentX,zoomFactor);											//TOOL ANNOTATION
			            zoom(oldX - currentX, 0, 1);												//TOOL ANNOTATION
			        }																//TOOL ANNOTATION
			    };																	//TOOL ANNOTATION
			var clickRight = function () {														//TOOL ANNOTATION
			        if (toolButtonClick('#tool_right')) {												//TOOL ANNOTATION
			            var oldX = currentX;													//TOOL ANNOTATION
			            currentX = currentX + getVisibleTilesX(0) / 2;										//TOOL ANNOTATION
			            currentX = normalizeX(currentX,zoomFactor);											//TOOL ANNOTATION
			            zoom(oldX - currentX, 0, 1);												//TOOL ANNOTATION
			...
			var clickUp = function () {														//TOOL ANNOTATION
			        if (toolButtonClick('#tool_up')) {												//TOOL ANNOTATION
			            var oldY = currentY;													//TOOL ANNOTATION
			            currentY = currentY - getVisibleTilesY(0) / 2;										//TOOL ANNOTATION
			            currentY = normalizeY(currentY,zoomFactor);											//TOOL ANNOTATION
			            zoom(0, oldY - currentY, 1);												//TOOL ANNOTATION
			        }																//TOOL ANNOTATION
			    };																	//TOOL ANNOTATION
			var clickDown = function () {														//TOOL ANNOTATION
			        if (toolButtonClick('#tool_down')) {												//TOOL ANNOTATION
			            var oldY = currentY;													//TOOL ANNOTATION
			            currentY = currentY + getVisibleTilesY(0) / 2;										//TOOL ANNOTATION
			            currentY = normalizeY(currentY,zoomFactor);											//TOOL ANNOTATION
			            zoom(0, oldY - currentY, 1);												//TOOL ANNOTATION
			...
			var clickPath = function(){
				if (toolButtonClick('#tool_path')) {
					...
					$('#workarea').css('cursor', 'url("' + EMF.applicationPath + '/image-editor/view/images/cursors/path.cur"), default');
				}
			};
			
			var clickFreeText = function() {				//TOOL ANNOTATION
				toolButtonClick('#tool_free_text');			//TOOL ANNOTATION
				svgCanvas.setMode('free_text');			//TOOL ANNOTATION
				$('#workarea').css('cursor', 'text');
			};							//TOOL ANNOTATION
			
			var clickFillShape = function() {				//TOOL ANNOTATION
				toolButtonClick('#tool_fill_shape');			//TOOL ANNOTATION
				svgCanvas.setMode('fill_shape');			//TOOL ANNOTATION
				$('#workarea').css('cursor', 'url("' + EMF.applicationPath + '/image-editor/view/images/cursors/fill.cur"), default');
			};							//TOOL ANNOTATION
			
			var changeStrokeType = function(event) {				//TOOL ANNOTATION
				svgCanvas.setStrokeType($(event.target).val());
			};
			
			var clickBorderColor = function() {				//TOOL ANNOTATION
				toolButtonClick('#tool_border_color');			//TOOL ANNOTATION
				svgCanvas.setMode('stroke_color');			//TOOL ANNOTATION
				$('#workarea').css('cursor', 'url("' + EMF.applicationPath + '/image-editor/view/images/cursors/stroke_color.cur"), default');
			};							//TOOL ANNOTATION
			
			var clickStar = function() {				//TOOL ANNOTATION
				toolButtonClick('#tool_star');			//TOOL ANNOTATION
				svgCanvas.setMode('star');			//TOOL ANNOTATION
				$('#workarea').css('cursor', 'url("' + EMF.applicationPath + '/image-editor/view/images/cursors/point.cur"), default');
			};							//TOOL ANNOTATION
			var clickMyRect = function(){				//TOOL ANNOTATION
				toolButtonClick('#tool_my_rect');		//TOOL ANNOTATION
				svgCanvas.setMode('rect');			//TOOL ANNOTATION
				$('#workarea').css('cursor', 'url("' + EMF.applicationPath + '/image-editor/view/images/cursors/square.cur"), default');
			};							//TOOL ANNOTATION
			var clickMyEllipse = function(){			//TOOL ANNOTATION
				toolButtonClick('#tool_my_ellipse');		//TOOL ANNOTATION
				svgCanvas.setMode('ellipse');			//TOOL ANNOTATION
				$('#workarea').css('cursor', 'url("' + EMF.applicationPath + '/image-editor/view/images/cursors/circle.cur"), default');
			};							//TOOL ANNOTATION
...
// 				if(color == curPrefs.bkgd_color && url == curPrefs.bkgd_url) return;
			...
			var setIcon = Editor.setIcon = function(elem, icon_id, forcedSize) {
				//var icon = (typeof icon_id === 'string') ? $.getSvgIcon(icon_id, true) : icon_id.clone();	//TOOL ANNOTATION
				//if(!icon) {											//TOOL ANNOTATION
				//	console.log('NOTE: Icon image missing: ' + icon_id);					//TOOL ANNOTATION
				//	return;											//TOOL ANNOTATION
				//}												//TOOL ANNOTATION
				//$(elem).empty().append(icon);									//TOOL ANNOTATION
			}
...
// 					}
			...
			(function() {
				...
				$('#tool_undo, #tool_redo, #tool_zoom_in, #tool_zoom_out, #tool_zoom_default, #tool_up, #tool_down, #tool_left, #tool_right, #tool_select').mousedown(function(){	//TOOL ANNOTATION
					if (!$(this).hasClass('disabled')) $(this).addClass(cur_class);
				}).bind('mousedown mouseout',function(){$(this).removeClass(cur_class).addClass('tool_button');});									//TOOL ANNOTATION
			}());
...
// 				var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
				...
				$("#color_picker")
					...
					.css(curConfig.colorPickerCSS || {'left': pos.left, 'bottom': 60})												//TOOL ANNOTATION
					.jGraduate(
					{
						paint: paint,
			...
			var updateToolButtonState = function() {
			    var curr_border_color = svgCanvas.getColor('stroke');	//TOOL ANNOTATION
			    	$('div.palette_item').each(function(i) {		//TOOL ANNOTATION
			    	    var val_color = $(this).attr("data-rgb");		//TOOL ANNOTATION
			    	    if(val_color == curr_border_color) {		//TOOL ANNOTATION
			    		$(this).css('border-top-color','black');	//TOOL ANNOTATION
			    		$(this).css('border-right-color','black');	//TOOL ANNOTATION
			    		$(this).css('border-bottom-color','black');	//TOOL ANNOTATION
			    		$(this).css('border-left-color','black');	//TOOL ANNOTATION
			    	    } else {						//TOOL ANNOTATION
			    		$(this).css('border-top-color','#FFFFFF');	//TOOL ANNOTATION
			    		$(this).css('border-right-color','#808080');	//TOOL ANNOTATION
			    		$(this).css('border-bottom-color','#808080');	//TOOL ANNOTATION
			    		$(this).css('border-left-color','#FFFFFF');	//TOOL ANNOTATION
			    	    }							//TOOL ANNOTATION
			    	});							//TOOL ANNOTATION
				...
				var buttonsNeedingFillAndStroke = [ '#tools_rect .tool_button', '#tools_ellipse .tool_button', '#tool_text', '#tool_path', '#tool_star', '#tool_my_rect', '#tool_my_ellipse' ];		//TOOL ANNOTATION
				if (bNoStroke) {
			...
			var PaintBox = function(container, type) {
				...
				if($(container).size()==0){					//TOOL ANNOTATION
					return;							//TOOL ANNOTATION
				}								//TOOL ANNOTATION
			...
			//paintBox.fill = new PaintBox('#fill_color', 'fill');						//TOOL ANNOTATION
			//paintBox.stroke = new PaintBox('#stroke_color', 'stroke');					//TOOL ANNOTATION

			$('#stroke_width').val(curConfig.initStroke.width);
			$('#group_opacity').val(curConfig.initOpacity * 100);

			//// Use this SVG elem to test vectorEffect support						//TOOL ANNOTATION
			//var test_el = paintBox.fill.rect.cloneNode(false);						//TOOL ANNOTATION
			//test_el.setAttribute('style', 'vector-effect:non-scaling-stroke');				//TOOL ANNOTATION
			//var supportsNonSS = (test_el.style.vectorEffect === 'non-scaling-stroke');			//TOOL ANNOTATION
			//test_el.removeAttribute('style');								//TOOL ANNOTATION
			//var svgdocbox = paintBox.fill.rect.ownerDocument;						//TOOL ANNOTATION
			//// Use this to test support for blur element. Seems to work to test support in Webkit		//TOOL ANNOTATION
			//var blur_test = svgdocbox.createElementNS('http://www.w3.org/2000/svg', 'feGaussianBlur');	//TOOL ANNOTATION
			//if(typeof blur_test.stdDeviationX === "undefined") {						//TOOL ANNOTATION
			//	$('#tool_blur').hide();									//TOOL ANNOTATION
			//}												//TOOL ANNOTATION
			//$(blur_test).remove();									//TOOL ANNOTATION
			...
			setTimeout(function() {
				svgCanvas.embedImage(svgSpace.toolPath + '/images/logo.png', function(datauri) {	//TOOL ANNOTATION
					if(!datauri) {
						// Disable option
		...
		// 	}
			...
			var centerCanvas = function() {
				// this centers the canvas vertically in the workarea (horizontal handled in CSS)
			    	workarea.css('line-height', (workarea.height() - 120) + 'px');				//TOOL ANNOTATION
			};
		...
		// 	}
			...
			var Actions = function() {
				...
				var tool_buttons = [
					{sel:'#tool_left', fn: clickLeft, evt: 'click', key: ['Left', true], icon: 'circle'},			//TOOL ANNOTATION
					{sel:'#tool_right', fn: clickRight, evt: 'click', key: ['Right', true], icon: 'circle'},		//TOOL ANNOTATION
					{sel:'#tool_up', fn: clickUp, evt: 'click', key: ['Up', true], icon: 'circle'},				//TOOL ANNOTATION
					{sel:'#tool_down', fn: clickDown, evt: 'click', key: ['Down', true], icon: 'circle'},			//TOOL ANNOTATION
					{sel:'#tool_zoom_default', fn: clickZoomDefault, evt: 'click', key: ['0', true], icon: 'circle'},	//TOOL ANNOTATION
					{sel:'#tool_zoom_in', fn: clickZoomIn, evt: 'click', key: ['I', true], icon: 'circle'},			//TOOL ANNOTATION
					{sel:'#tool_zoom_out', fn: clickZoomOut, evt: 'click', key: ['O', true], icon: 'circle'},		//TOOL ANNOTATION
					{sel:'#tool_star', fn: clickStar, evt: 'click', key: ['P', true], icon: 'circle'},			//TOOL ANNOTATION
					{sel:'#tool_free_text', fn: clickFreeText, evt: 'click', icon: 'circle'},			//TOOL ANNOTATION
					{sel:'#tool_fill_shape', fn: clickFillShape, evt: 'click', icon: 'circle'},			//TOOL ANNOTATION
					{sel:'#tool_border_color', fn: clickBorderColor, evt: 'click', icon: 'circle'},			//TOOL ANNOTATION
					{sel:'#tool_stroke_type', fn: changeStrokeType, evt: 'change', icon: 'circle'},			//TOOL ANNOTATION
					{sel:'#tool_my_rect', fn: clickMyRect, evt: 'click', key: ['R', true], icon: 'circle'},			//TOOL ANNOTATION
					{sel:'#tool_my_ellipse', fn: clickMyEllipse, evt: 'click', key: ['E', true], icon: 'circle'},		//TOOL ANNOTATION
		...
		// 			{sel:'#tools_ellipse_show', fn: clickEllipse, evt: 'click'},
					...
					setAll: function() {
						...
						$.each(tool_buttons, function(i, opts)  {
							...
							if(opts.key) {
								...
								if (opts.sel && !opts.hidekey && this.btn) {						//TOOL ANNOTATION
			...
			var updateCanvas = Editor.updateCanvas = function(center, new_ctr) {
				...
				w = svgSpace.canWidth;							//TOOL ANNOTATION
				h = svgSpace.canHeight;							//TOOL ANNOTATION
				workarea.css('overflow','hidden');					//TOOL ANNOTATION

				var old_can_y = cnvs.height()/2;
				var old_can_x = cnvs.width()/2;
				cnvs.width(svgSpace.canWidth).height(svgSpace.canHeight);		//TOOL ANNOTATION
				var new_can_y = h/2;
				var new_can_x = w/2;
