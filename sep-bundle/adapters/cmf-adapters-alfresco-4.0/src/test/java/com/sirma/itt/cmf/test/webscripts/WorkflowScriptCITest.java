	...
	public void testStartParallelWorkflow() {
		...
		try {
			String httpRequest = "{\"workflowId\":\"activiti$WFTYPEPT01\",\"nextStateProperties\":{\"emf:status\":\"APPROVED\",\"emf:containerId\":\""
					...
					+ "\",\"emf:status\":\"COMPLETED\",\"cm:owner\":\""
					+ userName
					+ "\",\"bpm:workflowDueDate\":"
					+ nowDateAsLong
					+ ",\"emf:type\":\"TSTYPE97\",\"cm:description\":\"WFTYPEPT01\",\"bpm:assignee\":\""
					+ userName
					+ "\",\"cmfwf:modified\":"
					+ nowDateAsLong
					+ ",\"cmfwf:modifiedBy\":\""
			...
			String requestResult = httpClient.request(ServiceURIRegistry.CMF_WORKFLOW_START,
					createMethod);
			assertNotNull(requestResult, "Workflow should be started");
			JSONArray tasks = new JSONObject(requestResult).getJSONObject("tasks").getJSONArray(
					"data");
			assertTrue(tasks.length() == 1, "Only one task after start");

			String taskId = tasks.getJSONObject(0).getString("id");

			String transition = "{\"taskId\":\""
					+ taskId
					+ "\",\"transitionId\":\"RT0097\",\"nextStateProperties\":{\"emf:status\":\"APPROVED\",\"emf:containerId\":\""
					+ containerId
					+ "\"},\"properties\":{\"cmfwf:startedBy\":\""
					+ userName
					+ "\",\"cmfwf:taskOutcome\":\"RT0097\",\"bpm:description\":\"WFTYPEPT01\",\"bpm:assigneeA\":\"banchev\",\"emf:identifier\":\""
					+ UUID.randomUUID().toString()
					+ "\",\"emf:status\":\"COMPLETED\",\"bpm:comment\":\"go to parallel\",\"cm:owner\":\""
					+ userName
					+ "\",\"emf:type\":\"PT0001\",\"cmfwf:modified\":"
					+ nowDateAsLong
					+ ",\"cmfwf:modifiedBy\":\""
					+ userName
					+ "\",\"bpm:assigneesB\":\"automatron,automatron1,Consumer\",\"cmfwf:priority\":\"1\"}}";
			createMethod = httpClient.createMethod(new PostMethod(), transition, true);
			requestResult = httpClient.request(ServiceURIRegistry.CMF_WORKFLOW_TRANSITION,
					createMethod);
			tasks = new JSONObject(requestResult).getJSONArray("data");
			assertTrue(tasks.length() == 4,
					"3 multi instance tasks and 1 single task after transition");
			String managerTaskId = null;
			for (int i = 0; i < 4; i++) {
				JSONObject task = tasks.getJSONObject(i);
				checkOwner(task.getJSONObject(AlfrescoCommunicationConstants.KEY_PROPERTIES)
						.getString("cm_owner"), "automatron", "automatron1", "Consumer", "banchev");
			...
			transition = "{\"taskId\":\""
					...
					+ userName
					+ "\",\"emf:status\":\"COMPLETED\",\"emf:identifier\":\""
					+ UUID.randomUUID().toString()
					+ "\",\"cm:owner\":\"banchev\",\"cmfwf:priority\":\"1\",\"emf:type\":\"PT0002\"}}";
			createMethod = httpClient.createMethod(new PostMethod(), transition, true);
			requestResult = httpClient.request(ServiceURIRegistry.CMF_WORKFLOW_TRANSITION,
					createMethod);
			tasks = new JSONObject(requestResult).getJSONArray("data");
			...
			for (int i = 0; i < 3; i++) {
				JSONObject task = tasks.getJSONObject(i);
				checkOwner(task.getJSONObject(AlfrescoCommunicationConstants.KEY_PROPERTIES)
						.getString("cm_owner"), "automatron", "automatron1", "Consumer");
			}
		} catch (Exception e) {
			fail(e);
		}
