...
public abstract class CSRequestProviderAdapter implements EAIRequestProviderAdapter {
	...
	private RawQuery processCondition(Condition searchTree, String systemId, RawQuery rawQuery)
			throws EAIModelException {
		List<SearchTreeNode> rules = searchTree.getRules();
		if (rules == null || rules.isEmpty()) {
			return rawQuery;
		}
		Iterator<SearchTreeNode> iterator = rules.iterator();
		while (iterator.hasNext()) {
			SearchTreeNode searchTreeEntry = iterator.next();
	...
	@SuppressWarnings("unchecked")
	protected void appendQueryEntry(SearchTreeNode sourceQueryTree, String systemId, RawQuery rawQuery)
			throws EAIModelException {
