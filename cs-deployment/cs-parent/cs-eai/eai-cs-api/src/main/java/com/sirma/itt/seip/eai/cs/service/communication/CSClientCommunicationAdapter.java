...
public abstract class CSClientCommunicationAdapter extends HttpClientCommunicationServiceAdapter {
	...
	protected void processQueryParam(List<NameValuePair> params, QueryEntry queryEntry) {
		...
		if (EqualsHelper.nullSafeEquals(rawQueryEntry.getOperator(), "between", true)) {
			...
			String dateTo = null;
			JsonObject fromToValue = readJsonObject(values.iterator().next().toString());
			// check the index of parameter
			if (fromToValue.containsKey("0") && StringUtils.isNotBlank(fromToValue.getString("0"))) {
				dateFrom = getISODate(fromToValue.getString("0"));
			}
			if (fromToValue.containsKey("1") && StringUtils.isNotBlank(fromToValue.getString("1"))) {
				dateTo = getISODate(fromToValue.getString("1"));
			}
