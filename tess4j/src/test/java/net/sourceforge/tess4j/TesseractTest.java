...
public class TesseractTest {
	...
	public void testDoOCR_File_With_Configs() throws Exception {
		...
		instance.setConfigs(null); // since Tesseract instance is a singleton,
									// clear configs so the effects do not carry
	...
	public void testDoOCR_File_Rectangle() throws Exception {
		...
		Rectangle rect = new Rectangle(0, 0, 1024, 800); // define an equal or
															// smaller region of
															// interest on the
															// image
		String expResult = "The (quick) [brown] {fox} jumps!\nOver the $43,456.78 <lazy> #90 dog";
	...
	public void testCreateDocuments() throws Exception {
		...
		assertTrue(new File(outputbase1 + ".txt").exists());
		assertTrue(new File(outputbase1 + ".hocr").exists());
		assertTrue(new File(outputbase1 + ".pdf").exists());
		assertTrue(new File(outputbase2 + ".txt").exists());
		assertTrue(new File(outputbase2 + ".hocr").exists());
		assertTrue(new File(outputbase2 + ".pdf").exists());
	}
