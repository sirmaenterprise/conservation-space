...
public class SearchQueryBuilder {
	...
	public String build(Condition tree) {
		...
		SearchContext searchContext = new SearchContext(tree);

		q.append(SPARQLQueryHelper.CURLY_BRACKET_OPEN);
		buildRules(q, tree.getRules(), tree.getCondition(), searchContext);
		q.append(SPACE).append(PERMISSIONS_BLOCK_CONSTANT).append(OBJECT).append(CURLY_BRACKET_CLOSE);
		return q.toString();
	}

	private void buildRules(StringBuilder q, List<SearchNode> rules, Junction junction, SearchContext searchContext) {
		...
		while (iterator.hasNext()) {
			...
			if (NodeType.CONDITION == node.getNodeType()) {
				...
				buildRules(q, c.getRules(), c.getCondition(), searchContext);
			} else {
				buildRule(q, (Rule) node, searchContext);
			}
	...
	private void buildRule(StringBuilder builder, Rule rule, SearchContext searchContext) {
		...
		operation.buildOperation(builder, rule, searchContext);
	}
