...
public class ArchiveEntityPersistCallback extends BaseRelationalPersistCallback {
	...
	public ArchiveEntityPersistCallback(DbDao dbDao, @ArchivedDataAccess PropertiesService propertiesService,
			EntityLookupCacheContext cacheContext, ObjectMapper mapper, ArchivedPropertiesDao archivedPropertiesDao) {
		super(dbDao, propertiesService, cacheContext);
		this.archivedPropertiesDao = archivedPropertiesDao;
	...
	public <I extends PropertyModel, E extends List<I>> void onBatchConvertedInstances(E collection) {
		List<Instance> instances = collection.stream().map(Instance.class::cast).collect(toList());
		archivedPropertiesDao.load(instances);
	}
}