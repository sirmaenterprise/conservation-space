...
public class FreeTextSolrProcessor implements FreeTextSearchProcessor {
	...
	@Override
	public String buildFreeTextSearchQuery(String searchTerms, List<String> types) {
		...
		String typesFilter = types.stream().map(toTypeFilter()).collect(Collectors.joining(" OR "));
		...
		return new SolrDismaxQueryBuilder().setUserQuery(query)
				...
				.setFilterQuery(typesFilter)
				.build();
	}

	private Function<String, String> toTypeFilter() {
		return type -> {
			// Note that the quotes should be escaped as well in order to be valid query
			if (type.contains(":")) {
				return "rdfType: \\\"" + escapeTerms(type) + "\\\"";
			}
			return "emfType: \\\"" + escapeTerms(type) + "\\\"";
		};
	}
	...
	private final class SolrDismaxQueryBuilder {
		...
		private SolrDismaxQueryBuilder setFilterQuery(String filterQuery) {
			if (filterQuery.isEmpty()) {
				return this;
			}
			requestBuilder.add(CommonParams.FQ, filterQuery);
			return this;
		}
