...
public class FreeTextSolrProcessorTest {
	...
	@Test
	public void shouldConstructSolrEDisMaxRankingRequestWithTypeFilter() {
		String processedKeywords = freeTextSolrProcessor.buildFreeTextSearchQuery("one two three",
				Arrays.asList(EMF.CASE.toString(), "userDefinition"));
		JsonObject solrRequest = readRequest(processedKeywords);
		...
		assertEquals("0.5", solrRequest.getString(DisMaxParams.TIE));
		assertEquals(SolrQueryConstants.EDISMAX_DEF_TYPE, solrRequest.getString(SolrQueryConstants.DEF_TYPE));
		assertEquals("rdfType: \\\"" + EMF.CASE + "\\\" OR emfType: \\\"userDefinition\\\"" , solrRequest.getString(CommonParams.FQ));
	}

	@Test
	public void shouldEscapeEmbeddedQuotationMarksForExactMatching() {
		String processedKeywords = freeTextSolrProcessor.buildFreeTextSearchQuery("\"exact match\"", Collections.emptyList());
		JsonObject solrRequest = readRequest(processedKeywords);
	...
	public void shouldEscapeConfiguredSymbols() {
		String processedKeywords = freeTextSolrProcessor.buildFreeTextSearchQuery("symbols [for] escape", Collections.emptyList());
		JsonObject solrRequest = readRequest(processedKeywords);
	...
	public void shouldDisallowNullSearchTerms() {
		freeTextSolrProcessor.buildFreeTextSearchQuery(null, Collections.emptyList());
	}

	@Test(expected = IllegalArgumentException.class)
	public void shouldDisallowEmptySearchTerms() {
		freeTextSolrProcessor.buildFreeTextSearchQuery("", Collections.emptyList());
	}
	...
	public void shouldAppendWildcardsIfConfigured() {
		...
		// Single term
		String processedKeywords = freeTextSolrProcessor.buildFreeTextSearchQuery("foo", Collections.emptyList());
		JsonObject solrRequest = readRequest(processedKeywords);
		assertEquals("*foo*", solrRequest.getString(SolrQueryConstants.USER_QUERY));
		// Multiple terms
		processedKeywords = freeTextSolrProcessor.buildFreeTextSearchQuery("foo bar", Collections.emptyList());
	...
	public void shouldRecognizeSingleQuotationMark() {
		...
		processedKeywords = freeTextSolrProcessor.buildFreeTextSearchQuery("foo\"", Collections.emptyList());
		solrRequest = readRequest(processedKeywords);
		assertEquals("*foo\\\"*", solrRequest.getString(SolrQueryConstants.USER_QUERY));
	}
