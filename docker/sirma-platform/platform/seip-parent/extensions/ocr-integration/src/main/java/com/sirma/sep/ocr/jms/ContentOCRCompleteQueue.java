...
/**
 * Content OCR complete queue listener responsible for consuming any OCR-ed content messages from the content complete
 * topic and persisting them using the content service with an 'ocr' purpose. A {@link ContentAssignedEvent} will be
 * fired after the content has been uploaded so the tika content extractor could extract the ocred text and put in the
 * instance in the semantic db.
 *
 * @author nvelkov
 */
public class ContentOCRCompleteQueue {
	 ...
	 *
	 * @param message the message containing the ocr-ed content
	 * @throws JMSException if the JMS provider fails to get some property value from the incoming message due to some
	 *         internal error.
	 */
	...
	private void updateContent(String contentId, Serializable instanceId, File ocredFile, String mimetype) {
		Content ocredContent = buildContent(ocredFile, mimetype);
		// Update the prepared empty ocred content entry with the actual ocred content.
		instanceContentService.updateContent(contentId, new EmfInstance(instanceId.toString()), ocredContent);

		textExtractor.extract(mimetype, ocredContent.getContent()).ifPresent(
				ocrText -> contentPersister.saveOcrContent(instanceId, ocrText));
	}

	private static Content buildContent(File ocredFile, String mimetype) {
		return Content
				.createEmpty()
