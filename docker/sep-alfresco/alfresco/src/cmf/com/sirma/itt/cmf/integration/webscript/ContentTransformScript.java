...
public class ContentTransformScript extends BaseAlfrescoScript {
	...
	private NodeRef addTempPrintNode(NodeRef nodeToCopy) {
		if ("versionStore".equals(nodeToCopy.getStoreRef().getProtocol())) {
			...
			if (versionNode == null) {
				versionNode = createNode(nodeSpace, version, ContentModel.TYPE_CONTENT);
	...
	private NodeRef getVersionedPreviewSpace() {
		...
		versionedPreviewSpace = caseService.getChildContainerByName(systemNode,
				"versionedNodesPreviewSpace");
		if (versionedPreviewSpace == null) {
			Map<QName, Serializable> props = new HashMap<QName, Serializable>(1);
			props.put(ContentModel.PROP_NAME, "versionedNodesPreviewSpace");
			versionedPreviewSpace = nodeService.createNode(
					systemNode,
					ContentModel.ASSOC_CHILDREN,
					QName.createQName(CMFModel.CMF_WORKFLOW_MODEL_1_0_URI,
							"versionedNodesPreviewSpace"), ContentModel.TYPE_FOLDER, props)
							.getChildRef();
		}
	...
	protected Map<String, Object> executeInternal(WebScriptRequest req) {
		...
		try {
			...
			if (!APP_PDF_MIME_TYPE.equals(sourceMimetype)) {
				...
				if (newNodeRef == null) {
					...
					ContentTransformer transformer = getServiceProxy().getTransformer(
							sourceMimetype, APP_PDF_MIME_TYPE, options);
					// if we don't have a transformer, throw an error
					if (transformer == null) {
						if (sourceMimetype.startsWith("image/")) {
							...
							throw new WebScriptException(204, "Unable to locate transformer");
						}
					} else {
						// establish a content reader (from source)
			...
			} else if (sourceMimetype.equals(APP_PDF_MIME_TYPE)) {
