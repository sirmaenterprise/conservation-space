...
import {MiradorConfig} from './mirador-integration/mirador-config';
...
@Inject(NgScope, NgElement, NgCompile, ImageService, ObjectSelectorHelper, Eventbus, LocalStorageService, PromiseAdapter, NgTimeout,
  NgInterval, CommentsRestService, CommentsFilterService, LocationAdapter, ResizeDetectorAdapter, MiradorConfig)
export class ImageWidget {
  ...
  constructor($scope, element, $compile, imageService, objectSelectorHelper, eventbus, localStorageService, promiseAdapter, $timeout,
              $interval, commentRestService, commentsFilterService, locationAdapter, resizeDetectorAdapter, miradorConfig) {
    ...
    this.miradorConfig = miradorConfig;
...
export class MiradorViewer {
  ...
  setupConfig(config, withRefactor) {
    ...
    if (this.context.isPreviewMode() && this.widgetConfig.lockWidget) {
      this.mergeInCurrentConfig(this.widget.miradorConfig.zen());
    } else {
      this.mergeInCurrentConfig(this.widget.miradorConfig.get());
    }
