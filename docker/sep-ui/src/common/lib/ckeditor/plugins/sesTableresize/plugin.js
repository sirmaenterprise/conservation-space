...
( function() {
  ...
  function getParentWidth( el ) {
    ...
    var styleAttr = 'border-' + side + '-width';
		var computed;
    // getComputedStyle in IE11 returns 0px for td element while currentStyle returns medium which should be equivalent of 1px
    if( element.$.currentStyle ) {
      computed = element.$.currentStyle[ styleAttr ];
      if ( computed in borderMap ) {
        computed = borderMap[ computed ];
      } else {
        computed = element.getComputedStyle( styleAttr );
      }
    } else {
      computed = element.getComputedStyle( styleAttr );
    }
	...
	function getPillarAtPosition( pillars, positionX ) {
		for ( var i = 0, len = pillars.length; i < len; i++ ) {
			...
			if ( positionX >= parseInt(pillar.x, 10) && positionX <= ( parseInt(pillar.x, 10) + pillar.width ) )
				return pillar;
		}
	...
	function columnResizer( editor ) {
		...
		function resizeColumn() {
          ...
          //Set all changed width in percent
          // CMF-21570 Tables do not resize after changing the size of the CKEditor
          // CMF-21775 Tables in layouts with two or more columns break after resizing them
          // CMF-21776 Layout column separator does not affect tables
          //original code was
          ...
          rightCell && rightCell.setStyle( 'width', ( Math.max( rightOldWidth - sizeShift, 1 ) ) / getWidth(table) * 100 + '%');
            ...
            table.setStyle('width', ( ( tableWidth + sizeShift * ( rtl ? -1 : 1 ) ) / getParentWidth(table)) * 100 + '%');
          }
	...
	CKEDITOR.plugins.add( 'sesTableresize', {
		requires: 'tabletools',
