...
function addWidget(editor, name, definition) {
  ...
  var blockEvents = function () {
    ...
    element.onclick = function (event) {
      // workaround for bootstrap components that listen for events bound to document
      $(document).trigger('click');

      //breaks the links in the data table widget if not stopped
      event.stopPropagation();
    };
    ...
    element.onfocusin = blockDefaults;
  };
  ...
  var configToolbar = function (configuration) {
    ...
    this.wrapper.$.onmouseenter = function (event) {
      //we don't need toolbars in preview mode
      if (editor.readOnly) {
        return;
      }
      var toolbar = document.getElementById('configToolbar-panel');
   ...
   *  Each property in this object will be added to the function manager for further execution in the proper base function.
   */
  ...
  function removeWidgetWrapperListeners(wrapper) {
