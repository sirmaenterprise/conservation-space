...
describe('Object type select', () => {
  ...
  it('should have any object select type at first position inside the dropdown list', () => {
    expect(selectMenu.getOptionByName('Any Object').getInnerHtml()).to.eventually.equal('<span data-value="anyObject"><span class="type-undefined level-undefined">Any Object</span></span>');
  });

  it('should visualize results in options and option groups', () => {
    expect(selectMenu.getOptionByName('Document').getInnerHtml()).to.eventually.equal('<span data-value="http://ittruse.ittbg.com/ontology/enterpriseManagementFramework#Document"><span class="type-class level-1">Document</span></span>');
    expect(selectMenu.getOptionByName('Common document').getInnerHtml()).to.eventually.equal('<span data-value="OT210027"><span class="type-definition level-2">Common document</span></span>');
  });
  ...
  it('should be filterable', () => {
    ...
    expect(element.all(by.css('.select2-results li')).count()).to.eventually.eq(3);
  });
