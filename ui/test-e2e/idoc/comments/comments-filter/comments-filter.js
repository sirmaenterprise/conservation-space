...
class CommentsFilter {
  ...
  openFilterPanel() {
    var button = this.element.$('.btn-filter-comment');
...
class CommentsFiltersPanel extends Dialog {
  ...
  keywordField() {
    browser.wait(EC.visibilityOf(this.getKeywordField()), DEFAULT_TIMEOUT);
    return new Field(this.getKeywordField());
  }

  getKeywordField() {
    return this.element.$('.keyword-field');
  }

  clearButton() {
    var btn = this.getClearButton();
    browser.wait(EC.elementToBeClickable(btn), DEFAULT_TIMEOUT);
    return btn;
  }
  ...
  getClearButton() {
    var clear = this.element.$('.btn-clear-fields');
    // I guess the modal is repositioned and the button changes it's position
    var cond = EC.and(EC.visibilityOf(clear), EC.elementToBeClickable(clear), elementToStopMoving(clear));
  ...
  filter() {
    browser.wait(EC.elementToBeClickable(this.getFilterButton()), DEFAULT_TIMEOUT);
    this.getFilterButton().click();
    this.waitUntilClosed();
  }

  getFilterButton() {
    return $('.seip-btn-filter');
  }
