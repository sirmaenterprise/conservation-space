  ...
  describe('configurations key word filtration', ()=> {
    ...
    it('should filter single configurations when key word is entered', ()=> {
      ...
      return tenantConfig.getFilterButton().click().then(() => {
        tenantConfig.expandAllRegions();
        expect(tenantConfig.isFieldDisplayed(FILESIZE_INPUT)).to.eventually.be.true;
        expect(tenantConfig.isFieldDisplayed(BOOLEAN_ELEMENT)).to.eventually.be.false;
        expect(tenantConfig.isFieldDisplayed(FLOAT_INPUT)).to.eventually.be.false;
        expect(tenantConfig.isFieldDisplayed(FILENAME_INPUT)).to.eventually.be.false;
        expect(tenantConfig.isFieldDisplayed(STRING_INPUT)).to.eventually.be.false;
        expect(tenantConfig.isFieldDisplayed(URI_INPUT)).to.eventually.be.false;
      });
    });
