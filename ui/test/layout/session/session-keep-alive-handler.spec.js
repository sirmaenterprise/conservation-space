...
describe('SessionKeepAliveHandler', () => {
  var setupSessionTimeoutHandlerSpy;
  var setupKeepAliveHandlerSpy;
  ...
  it('should unsubscribe from events when component is destroyed', () => {
    ...
    config.configs = {};
    config.configs[Configuration.SESSION_TIMEOUT] = 1;
    config.configs[Configuration.SESSION_TIMEOUT_REDIRECT] = true;
  ...
  describe('ngOnInit', () => {
    var config = getConfigurationInstance();
    config.configs = {};
    config[Configuration.SESSION_TIMEOUT] = -1;
    config[Configuration.SESSION_TIMEOUT_REDIRECT] = false;

    it('should not setup handler if redirect on timeout config is false', () => {
      config.configs[Configuration.SESSION_TIMEOUT] = 1;
      config.configs[Configuration.SESSION_TIMEOUT_REDIRECT] = false;
