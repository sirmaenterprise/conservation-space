...
import {Entry} from 'layout/breadcrumb/breadcrumb';

describe('BreadcrumbEntryManager', function () {

  function mockEntry(objectId, persisted) {
    return new Entry(objectId, 'header', 'instanceType', persisted, 'stateUrl');
  }
  ...
  describe('removeEntry()', () => {
    it('should remove all entries with same objectId', () => {
      ...
      entryManager.entries = [
        new Entry(1, 'header1', 'documentinstance', true, 'url'),
        new Entry(2, 'header2', 'documentinstance', true, 'url'),
        new Entry(3, 'header3', 'documentinstance', true, 'url'),
        new Entry(2, 'header4', 'documentinstance', true, 'url')
      ];
      entryManager.removeEntry(2);
      expect(entryManager.entries.length).to.equal(2);
      expect(entryManager.entries[0].objectId).to.equal(1);
      expect(entryManager.entries[1].objectId).to.equal(3);
    });

    it('should remove an entry by provided objectId', () => {
      let entryManager = new BreadcrumbEntryManager();
      entryManager.entries = [
        new Entry(1, 'header', 'documentinstance', true, 'url'),
        new Entry(2, 'header', 'documentinstance', true, 'url'),
        new Entry(3, 'header', 'documentinstance', true, 'url'),
        new Entry(4, 'header', 'documentinstance', true, 'url')
      ];
      entryManager.removeEntry(2);
      expect(entryManager.entries.length).to.equal(3);
      expect(entryManager.entries[0].objectId).to.equal(1);
      expect(entryManager.entries[1].objectId).to.equal(3);
      expect(entryManager.entries[2].objectId).to.equal(4);
    });
  ...
  describe('compactEntries', () => {
    it('should remove duplicated successive entries from the breadcrumb', () => {
      ...
      expect(entryManager.entries.length).to.equal(3);
      expect(entryManager.entries[0].objectId).to.equal(1);
      expect(entryManager.entries[1].objectId).to.equal(2);
      expect(entryManager.entries[2].objectId).to.equal(1);
    });
  });

  describe('add()', () => {
    it('should not add new entry', function () {
      let entry = mockEntry(1, true);
      let entryManager = new BreadcrumbEntryManager();
      entryManager.entries.push(entry);
      let testEntry = mockEntry(1, false);

      expect(entryManager.add(testEntry)).to.be.false;
      expect(entryManager.getEntries().length).to.be.one;
      expect(entryManager.getLastEntry()).to.deep.equal(entry);
    });
    ...
    it('should clear the last not persisted entry', () => {
      ...
      expect(entryManager.entries[0].objectId).to.equal(2);
    });
