...
describe('ExternalSearch', () => {
  ...
  function getComponentInstance() {
    ...
    return new ExternalSearch(scope, timeout, externalSearchService, eaiService, translateService, promiseAdapter, dialogService, configuration, actionsService, eventbus, notificationService);
  }
  ...
  describe('buildDefaultSearchCriteria(type, properties)', () => {
    it('should build correct search criteria', () => {
      ...
      expect(innerSectionCriteria.rules.length).to.equal(2);
  ...
  describe('updateSearchToolbar(searchConfig)', () => {
    ...
    it('should update the search toolbar if the search configuration has an order config', () => {
      ...
      expect(externalSearch.$timeout.calledOnce).to.be.true;
