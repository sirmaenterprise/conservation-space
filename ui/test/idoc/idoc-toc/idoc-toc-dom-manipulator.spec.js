...
describe('Tests for the idoc table of contents : dom-operations', ()=> {
  ...
  it('Test if deleteSection(id) deletes main section properly', ()=> {
    let result = '';
    let config = {source: '#deleteSectionTest'};
    let virtualTree = {
      createSection: function (sectionID, parentID) {
      },
    ...
    let domManipulator = new ToCDomManipulator(config, virtualTree);
    domManipulator.deleteSection('a');
    let heading = $('#delete-a', config.source).nextUntil('h1');
    expect(heading.length).to.be.zero;
  });
  ...
  it('Test collapseExpandSection(section) to collapse section', ()=> {
    let config = {
    ...
    expect($(collapsedContainerID).children('#collapse-B').length).to.be.above.zero;
  });

  it('Test collapseExpandSection(section) to expand section', ()=> {
    let config = {
    ...
    $(collapsedContainerID).append('<div data-ref-id="expand-A"><h2 id="expand-B" class="idoc-visual-collapsed-hidden" style="display: none;"></h2></div>');
  ...
  it('Test if deleteCollapsedSection(parentID) deletes the collapsed section', ()=> {
    ...
    $(collapsedContainerID).append('<div data-ref-id="deleteCollapsed-A"><h2 id="deleteCollapsed-B" class="idoc-visual-collapsed-hidden" style="display: none;"></h2></div>');
    domManipulator.deleteCollapsedSection('deleteCollapsed-A');

    expect($(collapsedContainerID).children().length).to.be.not.above(0);
  });

  it('Test if deleteSection(id) deletes main section with collapsed sub ones', ()=> {
    let result = '';
    let config = {
      source: '#deleteSectionWithCollapsedTest',
      eventbus: eventbusMock
    };
    let virtualTree = {
      deleteSubTree: function (id) {
    ...
    let domManipulator = new ToCDomManipulator(config, virtualTree);
    domManipulator.createCollapseContainer();
    let collapsedContainerID = '#collapse-container-deleteSectionWithCollapsedTest';
    $(collapsedContainerID).append('<div data-ref-id="deleteSectionWithCollapsed-b"><h3 id="deleteSectionWithCollapsed-c" class="idoc-visual-collapsed-hidden" style="display: none;"></h3></div>');
    domManipulator.deleteSection('deleteSectionWithCollapsed-a');

    expect($(config.source).children().length).to.not.be.above(0);
    expect($(collapsedContainerID).children().length).to.not.be.above(0);
  });
