...
describe('IdocPage', function () {
  ...
  it('loadSystemTabs should populate systemTabs array', () => {
    let idocPage = IdocMocks.instantiateIdocPage();
  ...
  it('should generates proper string as result when getIdocContent() is called', () => {
    ...
    let idocPage = IdocMocks.instantiateIdocPage();
    expect(idocPage.getIdocContent()).to.equal('<div data-tabs-counter="3"><section data-id="id_0" data-title="Title 0" data-default="true" data-show-navigation="true" data-show-comments="true" data-revision="exportable" data-locked="false">Content 0</section>' +
      '<section data-id="id_1" data-title="Title 1" data-default="false" data-show-navigation="true" data-show-comments="false" data-revision="exportable" data-locked="false">Content 1</section>' +
      '<section data-id="id_2" data-title="Title 2" data-default="false" data-show-navigation="false" data-show-comments="false" data-revision="exportable" data-locked="true">Content 2</section>' +
      '<section data-id="id_3" data-title="Title 3" data-default="false" data-show-navigation="false" data-show-comments="true" data-revision="cloneable" data-locked="false">Content 3</section></div>');
  });
  ...
  describe('appendContent()', () => {
    ...
    it('should disable save while appending content', () => {
      currentObject = new InstanceObject(null, IdocMocks.generateModels(), IdocMocks.generateIntialContent());
      let idocPage = IdocMocks.instantiateIdocPage(IDOC_ID, 'edit');
      let disableSaveButtonSpy = sinon.spy(idocPage, 'disableSaveButton');
      idocPage.currentObject = currentObject;
      let content = '<div><section data-id="id_0" data-title="First title" data-default="false" data-show-navigation="true" data-show-comments="true">First content</section></div>';
      idocPage.appendContent(content);
      expect(disableSaveButtonSpy.callCount).to.equals(2);
      expect(disableSaveButtonSpy.getCall(0).args[0]).to.be.true;
      expect(disableSaveButtonSpy.getCall(1).args[0]).to.be.false;
    });
  });
  ...
  it('should subscribe for AfterEditActionExecutedEvent and PermissionsChangedEvent', (done) => {
    let eventbus = new Eventbus();
    let spySubscribe = sinon.spy(eventbus, 'subscribe');
    let idocPage = new IdocPage(undefined, IdocMocks.mockScope(), eventbus, IdocMocks.mockStateParamsAdapter(),
      ...
      IdocMocks.mockConfiguration(), IdocMocks.mockPluginsService(), {}, {}, {}, stub(ModelingIdocContextBuilder));
    ...
    expect(spySubscribe.getCall(2).args[0]).to.equal(PermissionsChangedEvent);

    var spyCheckPermissionsForEditAction = sinon.spy(idocPage, 'checkPermissionsForEditAction');
    idocPage.loadData().then(() => {
      eventbus.publish(new PermissionsChangedEvent());
  ...
  describe('checkPermissionsForEditAction()', () => {

    it('should allow edit of idoc if has edit action', (done) => {
      currentObject = new InstanceObject(IDOC_ID, IdocMocks.generateModels(), IdocMocks.generateIntialContent());
      ...
      idocPage.loadData().then(() => {
        idocPage.checkPermissionsForEditAction();

        actionsService.getActions(IDOC_ID).then(() => {
          expect(idocPage.editAllowed).to.be.true;
          done();
        }).catch(done);
      }).catch(done);
    });

    it('should not allow edit of idoc if no edit action', (done) => {
      currentObject = new InstanceObject(IdocMocks.NO_EDIT_ACTION_IDOC_ID, null, null);

      let actionsService = IdocMocks.mockActionsService();
      let idocPage = IdocMocks.instantiateIdocPage(IdocMocks.NO_EDIT_ACTION_IDOC_ID);

      idocPage.loadData().then(() => {
        idocPage.checkPermissionsForEditAction();

        actionsService.getActions(IdocMocks.NO_EDIT_ACTION_IDOC_ID).then(() => {
          expect(idocPage.editAllowed).to.be.false;
          done();
        }).catch(done);
      }).catch(done);
    });

    it('should skip check if creating new idoc', (done) => {
      currentObject = new InstanceObject('currentObjectTempId', null, null);

      let actionsService = IdocMocks.mockActionsService();
      let idocPage = IdocMocks.instantiateIdocPage('currentObjectTempId');
    ...
    it('should redirect to preview mode if edit not allowed', (done) => {
      currentObject = new InstanceObject(IdocMocks.NO_EDIT_ACTION_IDOC_ID, null, null);

      let actionsService = IdocMocks.mockActionsService();
      let idocPage = IdocMocks.instantiateIdocPage(IdocMocks.NO_EDIT_ACTION_IDOC_ID);

      sinon.stub(idocPage.context, 'isEditMode', () => {
        return true;
      });
    ...
    it('should lock document and allow edit if user have permissions', (done) => {
      currentObject = new InstanceObject(IDOC_ID, IdocMocks.generateModels(), IdocMocks.generateIntialContent());
      ...
      sinon.stub(idocPage.context, 'reloadObjectDetails', () => {
        return PromiseStub.resolve();
      });
      var spyLock = sinon.spy(idocPage.actionsService, 'lock');
      ...
      idocPage.loadData().then(() => {
        ...
        actionsService.getActions(IDOC_ID).then(() => {
          expect(idocPage.editAllowed).to.be.true;
          expect(spyLock.called).to.be.true;
          done();
        }).catch(done);
      }).catch(done);
    });
