...
describe('AggregatedTableConfig', function () {
  ...
  describe('setSelectedObjects', () => {
    ...
    it('should populate groupBy with codelist value', () => {
      AggregatedTableConfig.prototype.config = {
        groupBy: {name: 'type'}
      };
      sinon.stub(AggregatedTableConfig, 'convertToSelectorModel', () => {
        return [{id: 'type', text: 'Type', codelist: [2]}];
      });
      let aggregatedTableConfig = getConfigInstance();
      let expected = {name: 'type', label: 'Type', codelist: [2]};
      expect(aggregatedTableConfig.config.groupBy).to.eql(expected);
    });
  });
