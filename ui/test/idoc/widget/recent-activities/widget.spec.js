...
describe('RecentActivities', () => {
  ...
  beforeEach(() => {
    ...
    RecentActivities.prototype.control = {getId: () => 1};
  ...
  function getWidgetInstance() {
    var locationAdapterMock = {
      url: sinon.spy()
    };
    var eventbus = {
      publish: sinon.spy()
    };
    var instanceRestService = mockInstanceRestService({data: []});
    var objectSelectorHelper = mockObjectSelectorHelper({results: []});
  ...
  describe('load(config)', () => {
    ...
    it('should resolve the provided array of instance identifiers', () => {
      ...
      var loadSpy = widget.instanceRestService.loadAuditDataForInstances;
    ...
    it('should fire an event to notify that the widget is ready', () => {
      widget.objectSelectorHelper = mockObjectSelectorHelper({results: [{id: '123'}]});
      widget.load({pageSize: 5});
      expect(widget.eventbus.publish.calledOnce).to.be.true;
    });

    it('should fire an event to notify that the widget is ready even if there is an error', () => {
      widget.objectSelectorHelper = mockObjectSelectorHelper('Error!', true);
      widget.load({pageSize: 5});
      expect(widget.eventbus.publish.calledOnce).to.be.true;
    });
  ...
  describe('fireWidgetReadyEvent()', () => {
    ...
    it('should publish a WidgetReadyEvent', () => {
      ...
      var event = widget.eventbus.publish.getCall(0).args[0];
