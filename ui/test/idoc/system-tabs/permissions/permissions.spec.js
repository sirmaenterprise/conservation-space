import {Permissions, OTHER_USERS_ID, OTHER_USERS_LABEL, NO_PERMISSION, NO_PERMISSIONS_LABEL} from 'idoc/system-tabs/permissions/permissions';
...
describe('Permissions', () => {
  ...
  it('should reverse the permissions', ()=> {
    ...
    permissions.systemOthersIndex = 0;
    permissions.sort();
    expect(permissions.authorities[0].name).to.equal('name2');
    expect(permissions.authorities[2].name).to.equal('system');
    expect(permissions.systemOthersIndex).to.equal(2);
  });
  ...
  it('should set proper special permissions for other users', ()=> {
    var authority = {
      active: NO_PERMISSION,
      newSpecial: NO_PERMISSION,
      id: OTHER_USERS_ID
    };
    var active = permissions.calculateActive(authority);
    expect(active).to.equal(NO_PERMISSIONS_LABEL);
  });
  ...
  it('should set correct label for system other users', ()=> {
    let permission = [{
      active: NO_PERMISSION,
      special: NO_PERMISSION,
      id: OTHER_USERS_ID
    }];
    permissions.getRoleLabelByValue = sinon.spy();
    permissions.extractPermissions(permission);
    expect(permissions.authorities[0].label).equal(OTHER_USERS_LABEL);
  });

  it('should not hide authorities that had inherited parent permissions after disabled inheritance', ()=> {
    let objectPermissions = [{
      id: OTHER_USERS_ID,
      calculated: NO_PERMISSION,
      inherited: NO_PERMISSION,
      isManager: false,
