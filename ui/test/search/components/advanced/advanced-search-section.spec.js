...
describe('AdvancedSearchSection', () => {
  ...
  beforeEach(() => {
    ...
    AdvancedSearchSection.prototype.loaders = {
      properties: sinon.spy(() => {
        return PromiseStub.resolve([{'id': 'title'}])
      })
    };

    advancedSearchSection = new AdvancedSearchSection(mock$scope(), getTranslateServiceMock());
  });
  ...
  describe('initialize()', () => {
    it('should finally delegate to afterInit() to notify it is ready', () => {
      ...
      advancedSearchSection.initialize();
  ...
  describe('registerModelWatchers()', () => {
    ...
    it('should change the object type select state', () => {
      advancedSearchSection.objectTypeSelectConfig = {
        disabled: false
      };
      advancedSearchSection.config.disabled = true;
      advancedSearchSection.$scope.$digest();
      expect(advancedSearchSection.objectTypeSelectConfig.disabled).to.be.true;
    });
  ...
  describe('getObjectTypeValues()', () => {
    ...
    it('should construct correct object type value for non-semantic type with parent', () => {
      var types = getObjectTypes();
      var expected = 'emf:MainType1_CLSUBTYPE1';
      expect(advancedSearchSection.getObjectTypeValues(types)[2]).to.equal(expected);
    });
