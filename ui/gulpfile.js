...
gulp.task('bundle', function (done) {
  gulp.src(paths.workDir + '/config.js')
    ...
    .pipe(gulp.dest('./'))
    .pipe(shell('jspm install npm:clean-css@~3.1.0'))
...
gulp.task('static-files', function () {
  var staticPaths = [paths.src + '**/*.{html,css,png,jpg,json,gif,eot,svg,ttf,woff,properties}'];
...
gulp.task('browser-sync-app', function () {
  // prevents Error: DEPTH_ZERO_SELF_SIGNED_CERT
  ...
  proxy.on('proxyReq', function (proxyReq, req, res, options) {
    // required becase the nodejs proxy sends wrong 'host' http header to the server
