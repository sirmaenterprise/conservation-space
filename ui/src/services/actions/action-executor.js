...
import {ActionExecutedEvent} from './events';
...
export class ActionExecutor {
  ...
  invoke(actionHandler, action, context) {
    ...
    if (!promise) {
      promise = this.promiseAdapter.resolve();
    }
    promise.then((actionResponse) => this.eventbus.publish(new ActionExecutedEvent(action, context, actionResponse)));
  }
