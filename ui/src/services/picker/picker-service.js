...
export const PICKER_DIALOG_CLASS = 'picker';
 ...
 * Use {@link PickerService#getSelectedItems} and {@link PickerService#setSelectedItems} instead of directly accessing
 * them through the picker configuration.
 *
 * @author Mihail Radkov
 */
@Injectable()
@Inject(ExtensionsDialogService)
export class PickerService {

  constructor(extensionsDialogService) {
  ...
  assignDefaultSelectionHandler(searchConfig) {
    ...
    var singleSelection = searchConfig.results.config.selection === SINGLE_SELECTION;
  ...
  assignDefaultCreateConfigurations(extensions) {
    var searchConfig = extensions[SEARCH_EXTENSION];
    var useRootContext = searchConfig.useRootContext;
    var predefinedTypes = searchConfig.predefinedTypes;
    var selectedItems = searchConfig.results.config.selectedItems;
    var selectionHandler = searchConfig.results.config.selectionHandler;
  ...
  assignDefaultBasketConfigurations(extensions, tabs) {
    var searchConfig = extensions[SEARCH_EXTENSION];
    var selectableItems = searchConfig.results.config.selection !== NO_SELECTION;
    var selectedItems = searchConfig.results.config.selectedItems;
    var singleSelection = searchConfig.results.config.selection === SINGLE_SELECTION;
    var selectionHandler = searchConfig.results.config.selectionHandler;
  ...
  /**
   * Assigns default configuration for the recentyl used objects extension point to the provided extensions
   * configuration map.
   */
  assignDefaultRecentObjectsConfigurations(extensions) {
    ...
    var recentConfigDefaults = _.clone(extensions[BASKET_EXTENSION]);
  ...
  handleSelection(selectedItems, singleSelection, instance) {
    if (singleSelection) {
      ...
      var index = _.findIndex(selectedItems, (current) => current.id === instance.id);
      if (index === -1) {
  ...
  setSelectedItems(pickerConfig, selectedItems) {
    var currentSelection = this.getSelectedItems(pickerConfig);
