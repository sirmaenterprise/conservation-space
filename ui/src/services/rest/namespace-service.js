...
export class NamespaceService {
  ...
  convertToFullURI(uris) {
    var urisToConvert = _.filter(uris, this.isUri);
  ...
  toFullURI(uris) {
    ...
    return this.restClient.post(url, uris, this.config).then((result) => {
      var newCache = _.clone(this.cache);
  ...
  areAllShortUrisInCache(uris) {
    var shortUriNotInCache = _.find(uris, (uri) => {
