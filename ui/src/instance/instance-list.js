...
export class InstanceList extends Configurable {
  ...
  ngOnInit() {
    ...
    if ((!this.instances || !this.instances.length) && this.identifiers && this.identifiers.length) {
      this.instanceService.loadBatch(this.identifiers).then(this.loadInstances.bind(this));
      return;
    }
  ...
  loadHeaders(instances) {
    ...
    this.instanceService.loadBatch(instances.map(instance => instance.id)).then((response) => this.loadInstances(response));
  }
