import {Inject, Injectable, getComponentName} from 'app/app';
 ...
 */
@Injectable()
@Inject('$compile', '$document', '$rootScope', Eventbus, TranslateService)
export class DialogService {
  ...
  makeDraggable(dialogElement) {
    ...
    modalDialogElement.draggable({
      ...
      stop: () => DialogService.dragStop(modalDialogElement),
      scroll: false
    });
  }

  //If the element is dragged under the viewport, it returns the dialog into it.
  static dragStop(modalDialogElement) {
    let modalContentElement = modalDialogElement.find('.modal-content');
    let modalTop = modalContentElement.offset().top;
    let modalLeft = modalContentElement.offset().left;
