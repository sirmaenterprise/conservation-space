...
@Inject(NgScope, DialogService, TranslateService, InstanceRestService, PromiseAdapter, PickerService)
export class AdvancedSearchRelationCriteria extends Configurable {

  constructor($scope, dialogService, translateService, instanceRestService, promiseAdapter, pickerService) {
    super({});
    this.$scope = $scope;
  ...
  createSelectConfig() {
    this.selectConfig = {
      predefinedItems: [{
        id: ANY_OBJECT,
        properties: {
          title: this.translateService.translateInstant('search.advanced.value.anyObject')
        }
      }]
    };

    if (this.context) {
      this.selectConfig.predefinedItems.push({
        id: CURRENT_OBJECT,
        properties: {
  ...
  createDialogConfig() {
    return {
      largeModal: true,
      header: 'picker.dialog.header',
      helpTarget: 'picker',
      buttons: [
        this.dialogService.createButton(DialogService.OK, 'dialog.button.save', true),
        this.dialogService.createButton(DialogService.CLOSE, 'dialog.button.cancel')
      ],
