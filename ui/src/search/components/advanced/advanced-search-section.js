 ...
 * and an id. If no rules are defined, default ones will be added.
 *
...
export class AdvancedSearchSection extends SearchCriteriaComponent {
  ...
  registerModelWatchers() {
    ...
    // Enable/disable watcher for object type select
    this.$scope.$watch(() => {
      return this.config.disabled;
    }, () => {
      if (this.objectTypeSelectConfig) {
        this.objectTypeSelectConfig.disabled = this.isLockedOrDisabled(AdvancedSearchComponents.TYPE);
      }
    });
  }
  ...
  assignDefaultCriteria() {
    if (!this.criteria.rules || this.criteria.rules.length < 1) {
      var predefinedType = this.getPredefinedType();
      var defaultTypeCriteria = SearchCriteriaUtils.getDefaultObjectTypeRule(predefinedType);
  ...
  createObjectTypeConfig() {
    this.objectTypeSelectConfig = {
      ...
      disabled: this.isLockedOrDisabled(AdvancedSearchComponents.TYPE),
      multiple: true,
  ...
  /**
   * Constructs a string based on the current object type and its parent if present.
   *
   */
  getObjectTypeValues(objectTypes) {
    ...
    objectTypes.forEach(type => {
      var value = type.id;
      if (type.parent && !this.isObjectTypeSemantic(type)) {
        // If the subtype has a non-semantic ID
        value = `${type.parent}_${value}`;
      }
      values.push(value);
    });
    return values;
  ...
  isObjectTypeSemantic(objectType) {
    return objectType.type === 'class';
  }
}