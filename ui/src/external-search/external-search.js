...
import {SearchMediator, EVENT_BEFORE_SEARCH, EVENT_SEARCH, EVENT_SEARCH_CRITERIA_CHANGED, EVENT_BEFORE_SEARCH_CRITERIA_CHANGED} from 'search/search-mediator';
import {NO_SELECTION, MULTIPLE_SELECTION} from 'search/search-selection-modes';
 ...
 */
@Inject(NgScope, NgTimeout, ExternalSearchService, EAIService, TranslateService, PromiseAdapter, DialogService, Configuration, ActionsService, Eventbus, NotificationService)
export class ExternalSearch extends SearchCriteriaComponent {

  constructor($scope, $timeout, externalSearchService, eaiService, translateService, promiseAdapter, dialogService, configuration, actionsService, eventbus, notificationService) {
    super({
      disabled: false,
      results: {
        config: {
  ...
  buildDefaultSearchCriteria(type, properties) {
    ...
    properties.forEach((property) => {
      var rule = SearchCriteriaUtils.buildRule(property.id, property.type);
      //rule.disabled = ['field', 'operator'];
  ...
  updateSearchToolbar(searchConfiguration) {
    delete this.toolbarConfig;
    if (searchConfiguration && searchConfiguration.order) {
      // Gotta be in another digest to rebuild the toolbar
      this.$timeout(() => {
  ...
  configureResults() {
    this.searchMediator.registerListener(EVENT_SEARCH, (eventData) => {
      ...
      this.config.results.data.push(...data.map((item) => {
        ...
        return {
          id: item.dbId,
          default_header: item.headers.default_header || item.properties.title,
          data: item,
    ...
    this.dialogService.confirmation(this.translateService.translateInstant('unsaved.selection'), null, {
      ...
      onButtonClick: (buttonID, componentScope, dialogConfig) => {
        if (buttonID === 'confirm') {
          ...
          this.toolbarConfig.orderBy = this.currentOrderBy;
        }
  ...
  getPageSize() {
    return this.configuration.get('search.result.pager.pagesize');
  }
