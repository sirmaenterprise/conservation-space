...
const OTHER_USERS_LABEL = 'All Other Users';
...
export const OTHER_USERS_ID = 'sec:SYSTEM_ALL_OTHER_USERS';
...
export class Permissions {
  ...
  calculateActive(authority) {
    ...
    if (authority.active === NO_PERMISSION && authority.id === OTHER_USERS_ID) {
      authority.active = NO_PERMISSIONS_LABEL;
      authority.newSpecialPermission = NO_PERMISSIONS_LABEL;
    }
  ...
  extractPermissions(permissions) {
    ...
    this.authorities = permissions.map((permission) => {
      ...
      if (authority.id === OTHER_USERS_ID) {
        authority.name = OTHER_USERS_LABEL;
      }
      else {
  ...
  showAuthorities() {
    for (let i = 0; i < this.authorities.length; i++) {
      if (this.authorities[i].id === OTHER_USERS_ID) {
        this.systemOthersIndex = i;
      }
  ...
  processEditedAuthorities(authorities, editedAuthorities) {
    for (let authority of authorities) {
      if (this.isValidProperty(authority.id) && this.isValidProperty(authority.newSpecialPermission)) {
        ...
        if (authority.id !== OTHER_USERS_ID && this.isValidProperty(authority.id)) {
          this.addAuthorityToHidden(authority);
        }
      }
    }
  }
   ...
   * The system others should be always on last place, so the method extracts it from the array, sorts the remaining authorities
   * and pushes it on the last place.
   */
  sort() {
    let systemOtherAuthorities = this.authorities[this.systemOthersIndex];
    this.authorities.splice(this.systemOthersIndex, 1);
    ...
    this.authorities.push(systemOtherAuthorities);
    this.systemOthersIndex = this.authorities.length - 1;
  }
