...
class Comment {
  ...
  collapseComment() {
    this.expanded = false;
    this.commentContainer.find(COMMENT_DESCRIPTION_CLASS).addClass('collapsed');
    this.commentContainer.addClass('expanded');
    this.commentContainer.find(COMMENT_REPLY_TOOLBAR_CLASS).css({'display': 'none'});
    this.commentContainer.find(COMMENT_DESCRIPTION_CLASS).dotdotdot({
      ellipsis: '... ',
      wrap: 'word'
    });
  }
  ...
  expandComment() {
    if (!this.expanded && !this.comment.isReply()) {
      ...
      this.commentContainer.find(COMMENT_DESCRIPTION_CLASS).trigger('destroy');
      this.commentContainer.find(COMMENT_DESCRIPTION_CLASS).removeClass('collapsed');
