...
export class ObjectSelectorHelper {
  ...
  searchForObjects(criteria, context, searchArguments, searchMode) {
    ...
    var mediator = new SearchMediator(this.searchService, new QueryBuilder(criteria), context, searchArguments, searchMode);
  ...
  groupSelectedObjects(config, context, groupBy) {
    let searchArguments = {
      groupBy: groupBy
    };
    ...
    return this.promiseAdapter.promise((resolve, reject) => {
      var mediator = new SearchMediator(this.searchService, new QueryBuilder(config.criteria), context, searchArguments, config.searchMode);
      return mediator.search().then((searchResult) => {
        if (_.isEmpty(searchResult.response.data.aggregated[groupBy])) {
          reject(EMPTY_RESULTS);
        }
        if (searchResult.response.data.aggregated) {
          resolve({
