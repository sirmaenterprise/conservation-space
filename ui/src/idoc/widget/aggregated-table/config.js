...
export class AggregatedTableConfig extends Configurable {
      ...
      if (groupBySelectorConfig.definitions && groupBySelectorConfig.definitions.length > 0) {
        ...
        groupByProperties = firstDefinitionProperties.filter((property) => {
          if (property.codelist) {
            property.codelists = [property.codelist];
          }
          let isGroupByProperty = true;
          for (let i = 0; i < groupBySelectorConfig.definitions.length && isGroupByProperty; i++) {
            isGroupByProperty = this.isGroupByProperty(groupBySelectorConfig.definitions[i], property.name, property.codelists);
          }
  ...
  isGroupByProperty(definition, propertyName, codelists) {
    ...
    return properties.some((property) => {
      if ((property.controlDefinition && property.controlDefinition.identifier === PROPERTY_TYPE_PICKER) || property.codelist) {
        if (property.name === propertyName && codelists && codelists[0] !== property.codelist) {
          codelists.push(property.codelist);
        }
  ...
  onGroupByChanged(propertyName) {
    if (propertyName) {
      ...
      if (property) {
        this.config.groupBy = {name: property.id, label: property.text, codelist: property.codelist};
      } else {
