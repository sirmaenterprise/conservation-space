...
@Inject(NgScope, NgElement, NgInterval, ObjectSelectorHelper, CodelistRestService, InstanceRestService, Logger, TranslateService, Eventbus, PromiseAdapter)
export class AggregatedTable {

  constructor($scope, $element, $interval, objectSelectorHelper, codelistRestService, instanceRestService, logger, translateService, eventbus, promiseAdapter) {
    this.$scope = $scope;
    ...
    $scope.$watch(() => {
      ...
      if (value && this.existResultsInVersionData(value)) {
        this.groupObjects(value.name);
      } else {
        this.clearTable();
        this.setError(ERROR_MESSAGE_KEY);
  ...
  handleWidgetReady(groupsCount) {
    if (groupsCount === 0) {
      ...
      this.interval = this.$interval(() => {
        if (this.$element.find('.form-content').length === groupsCount) {
  ...
  convertResponseToModel(groupedByObjectsResponse) {
    ...
    if (this.config.groupBy.codelist) {
  ...
  createCodelistGroupingModel(groupedByObjectsResponse, name) {
    let promises = [];
    // for compatibility with old widgets
    if (!Array.isArray(this.config.groupBy.codelist)) {
      this.config.groupBy.codelist = [this.config.groupBy.codelist];
    }
    this.config.groupBy.codelist.forEach((number) => {
