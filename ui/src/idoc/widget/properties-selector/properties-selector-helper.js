...
export class PropertiesSelectorHelper {
  ...
  loadDefinitions(context, config) {
    if (config.selectObjectMode === SELECT_OBJECT_CURRENT || config.selectObjectMode === SELECT_OBJECT_MANUALLY) {
   ...
   * @returns {*} array with selected properties names or undefined
   */
  static getSelectedPropertiesArray(models, selectedPropertiesMap) {
    ...
    if (selectedProperties === undefined && semanticHierarchy) {
      ...
      semanticHierarchy.value.forEach((semanticClass) => {
        selectedProperties = _.union(selectedProperties, selectedPropertiesMap[semanticClass]);
      });
    }
