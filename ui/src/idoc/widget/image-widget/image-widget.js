...
export class ImageWidget {
  ...
  constructor($scope, element, $compile, imageService, objectSelectorHelper, eventbus, localStorageService, promiseAdapter, $timeout,
    ...
    this.events = [
      ...
      this.eventbus.subscribe(MiradorLoadedEvent, this.appendImageComments.bind(this)),
  ...
  annotationListLoadedHandler() {
    ...
    if (this.numberOfSlots === undefined) {
      this.numberOfSlots = this.element.find('.image-widget-viewer iframe').contents().find('.layout-slot').length;
    }
    this.numberOfSlots--;
  ...
  handlePrintMode() {
    ...
    let slotsReadyToPrintPromises = $.map(miradorIframe.contents().find('.layout-slot'), (slotEl) => {
      ...
      if (slot.find('.view-container .image-view').length > 0) {
        return this.promiseAdapter.promise((resolve) => {
          var annToolbarBtnPooler = this.$interval(() => {
            ...
            if (annotationsButton.length > 0) {
              ...
              var drawingCanvasPooler = this.$interval(() => {
                let drawingCanvas = slot.find('[id^="draw_canvas"]');
                if (drawingCanvas.length > 0 && drawingCanvas.css('display') !== 'none') {
                  this.$interval.cancel(drawingCanvasPooler);
                  this.imageViewPrintMode(miradorIframe, slotId).then(() => {
                    resolve();
                  }).catch((error) => {
        ...
        return this.thumbnailViewPrintMode(miradorIframe, slotId).catch((error) => {
          this.logger.error(error);
        });
      }
    });
  ...
  thumbnailViewPrintMode(miradorIframe, slotId) {
    return this.promiseAdapter.promise((resolve) => {
      let innerJQuery = miradorIframe.eq(0)[0].contentWindow.$ || $;
      let container = innerJQuery(miradorIframe.eq(0)[0].contentDocument).find(`div[data-layout-slot-id='${slotId}'] .thumbnail-view`).detach();
  ...
  imageViewPrintMode(miradorIframe, slotId) {
    return this.promiseAdapter.promise((resolve) => {
  ...
  ngOnDestroy() {
    ...
    if (this.miradorViewer && this.miradorViewer.eventsAdapter) {
      this.miradorViewer.eventsAdapter.destroy();
    }
  }
...
ImageWidget.TEMP_SAVE_ATTRIBUTE = 'temp-save';
  ...
  constructor(imageWidget) {
    ...
    this.viewerFrame.addEventListener('load', () => {
      ...
      this.viewerFrame.contentWindow.init({
        config: _.cloneDeep(this.currentConfig),
