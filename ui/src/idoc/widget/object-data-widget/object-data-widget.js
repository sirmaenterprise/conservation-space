...
import objectDataWidgetTemplate from 'idoc/widget/object-data-widget/object-data-widget.html!text';

export const NO_LINK = 'none';

@Widget
@View({
  template: objectDataWidgetTemplate
})
...
export class ObjectDataWidget extends Refreshable {
  ...
  constructor($scope, promiseAdapter, objectSelectorHelper, eventbus, $compile, $element, propertiesSelectorHelper) {
    ...
    if (!this.context.isModeling()) {
      $scope.$watch(() => {
        ...
        // reset it when switching to edit mode
    ...
    $scope.$watch(() => {
      ...
      if (newValues) {
        this.formContentReadyWatcher = setInterval(() => {
          if (this.$element.find('.form-content').length === 1) {
            clearInterval(this.formContentReadyWatcher);
  ...
  updatePropertyModel() {
    var flatViewModel = _.get(this.formConfig, 'models.viewModel.flatModelMap');
    if (flatViewModel) {
      for (var propertyId in flatViewModel) {
  ...
  getControlTypeFromPropertyData(property) {
    var definitionId = this.formConfig.models.definitionId;
    var propertiesData = this.config.selectedPropertiesData && this.config.selectedPropertiesData[definitionId];
    if (propertiesData) {
  ...
  /**
   * Add control to specific property. The control identifier is a form-wrapper component and
   * specify the way of property visualization.
   * @param property
   */
  addCodelistControl(property) {
    if (property.codelist) {
      var controlType = this.getControlTypeFromPropertyData(property);
      if (controlType) {
        property.control = {
      ...
      } else if (property.control && property.control.identifier === CONTROL_TYPE.CODELIST_LIST) {
        property.control = undefined;
        this.reloadModel = true;
      }
    }
  }
  ...
  isPreviewMode() {
    return this.config.formViewMode === FormWrapper.FORM_VIEW_MODE_PREVIEW;
  }
  ...
  static getFirstTypeWithSelectedProperties(selectedProperties) {
    ...
    Object.keys(selectedProperties).some((key) => {
      ...
      if (selectedProperties[key].length > 0) {
        result[key] = selectedProperties[key];
        return true;
      }
      return false;
    });
  ...
  compileInstanceHeader() {
    ...
    let instanceHeader = $(`<seip-static-instance-header header-type="objectDataWidget.config.instanceLinkType"
      ...
      instance-type="objectDataWidget.selectedObject.instanceType"
      thumbnail="objectDataWidget.selectedObject.getThumbnail()"></seip-static-instance-header>`);
  ...
  ngOnDestroy() {
    ...
    if (this.formContentReadyWatcher) {
      clearInterval(this.formContentReadyWatcher)
    }
