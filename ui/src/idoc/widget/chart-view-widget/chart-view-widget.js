...
export class ChartViewWidget {
  constructor($element, $scope, $compile, eventbus, pluginsService, codelistRestService, instanceRestService, promiseAdapter, objectSelectorHelper, propertiesSelectorHelper, translateService) {
    ...
    $scope.$watch(() => {
      ...
      if (!this.config.groupBy) {
        this.errorMessage = translateService.translateInstant('search.select.group.by.placeholder');
      } else {
  ...
  loadChartData() {
    return this.promiseAdapter.all([this.objectSelectorHelper.groupSelectedObjects(this.config, this.context, this.config.groupBy), this.getGroupByCodelists()]).then((results) => {
      let aggregatedResult = results[0].aggregated[this.config.groupBy];
      let labelsLoader;
      if (results[1]) {
        labelsLoader = this.getLabelsForCodelist(results[1]);
      } else {
  ...
  widgetShouldBeEmpty() {
    return this.context.isModeling() && this.config.selectObjectMode === SELECT_OBJECT_AUTOMATICALLY;
  }
