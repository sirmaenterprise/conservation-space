...
@Injectable()
@Inject(CreatePanelService, ModelsService, NamespaceService, WindowAdapter)
export class CreateInstanceAction extends ActionHandler {

  constructor(createPanelService, modelsService, namespaceService, windowAdapter) {
    super();
  ...
  execute(action, context) {
    ...
    if (actionConfig.predefinedTypes.length === 1) {
      ...
      this.modelsService.getModels('create', null, null, null, actionConfig.predefinedTypes, actionConfig.predefinedSubTypes).then((data) => {
        ...
        this.openDialogWithPredefinedType(actionConfig, context);
      });
    } else {
      actionConfig.predefinedSubTypes = [];
      this.createPanelService.openCreateInstanceDialog(this.initDialogOptionsFromActionConfig(actionConfig, undefined, context));
    }
  }
   ...
   * @param context
   */
  openDialogWithPredefinedType(actionConfig, context) {
    this.modelsService.getClassInfo(actionConfig.predefinedTypes).then((response) => {
      ...
      this.namespaceService.toFullURI(actionConfig.predefinedTypes).then((fullUrisMap) => {
        ...
        this.createPanelService.openCreateInstanceDialog(this.initDialogOptionsFromActionConfig(actionConfig, initialInstanceType, context, exclusions));
      });
    });
  }
   ...
   */
  initDialogOptionsFromActionConfig(actionConfig, initialInstanceType, context, exclusions) {
    return {
